...
void NetUpdate(void)
{
  ...
  // Queue packet to be processed when its tic time is reached
				queuedpacket = Z_Realloc(
				  queuedpacket, ++numqueuedpackets * sizeof *queuedpacket, PU_STATIC, NULL);
...
static void CheckQueuedPackets(void)
{
	...
	for (i = 0; (unsigned)i < numqueuedpackets; i++)
		if (doom_ntohl(queuedpacket[i]->tic) <= gametic) switch (queuedpacket[i]->type)
			{
	...
	{ // Requeue remaining packets
		...
		for (i = 0; (unsigned)i < numqueuedpackets; i++)
			...
			{
				newqueue = Z_Realloc(newqueue, ++newnum * sizeof *newqueue, PU_STATIC, NULL);
