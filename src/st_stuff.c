...
const char *ammo_colour_behaviour_list[ammo_colour_behaviour_max] = {"no", "full ammo only", "yes"};
...
static void ST_refreshBackground(void)
{
	...
	if (st_statusbaron)
	{
      ...
      // Applies palette to backfill
		...
		if (!deathmatch) V_DrawNumPatch(ST_ARMSBGX, y, BG, armsbg.lumpnum, CR_DEFAULT, flags);
      ...
      // killough 3/7/98: make face background change with displayplayer
		...
		V_CopyRect(BG, FG, ST_X + ST_SCALED_OFFSETX, SCREENHEIGHT - ST_SCALED_HEIGHT,
		...
		if (plyr->damagecount && plyr->attacker && plyr->attacker != plyr->mo)
		{
          // being attacked
           ...
           // e6y: compatibility optioned
			if ((default_comp[comp_ouchface] ? (plyr->health - st_oldhealth)
...
static void ST_doPaletteStuff(void)
{
	...
	else if (palette_onpowers
	         && (plyr->powers[pw_ironfeet] > 4 * 32 || plyr->powers[pw_ironfeet] & 8))
  ...
  // attacking flemoid.
	...
	if (palette_onpowers && gamemission == chex && palette >= STARTREDPALS
...
static void ST_drawWidgets(dboolean refresh)
{
  ...
  // jff 2/16/98 make color of ammo depend on amount
	...
	else
	{
		...
		if (plyr->backpack && ammo_colour_behaviour != ammo_colour_behaviour_yes) ammopct *= 2;
		if (ammopct < ammo_red)
...
static void ST_loadData(void) { ST_loadGraphics(true); }

static void ST_unloadData(void) { ST_loadGraphics(false); }
...
static void ST_createWidgets(void)
{
  ...
  // health percentage
	STlib_initPercent(
	  &w_health, ST_HEALTHX, ST_HEALTHY, tallnum, &plyr->health, &st_statusbaron, &tallpercent);
  ...
  // armor percentage - should be colored later
	STlib_initPercent(
	  &w_armor, ST_ARMORX, ST_ARMORY, tallnum, &plyr->armorpoints, &st_statusbaron, &tallpercent);
