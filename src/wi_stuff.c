...
static anim_t epsd1animinfo[] = {{ANIM_LEVEL, TICRATE / 3, 1, {128, 136}, 1},
  {ANIM_LEVEL, TICRATE / 3, 1, {128, 136}, 2}, {ANIM_LEVEL, TICRATE / 3, 1, {128, 136}, 3},
  {ANIM_LEVEL, TICRATE / 3, 1, {128, 136}, 4}, {ANIM_LEVEL, TICRATE / 3, 1, {128, 136}, 5},
  {ANIM_LEVEL, TICRATE / 3, 1, {128, 136}, 6}, {ANIM_LEVEL, TICRATE / 3, 1, {128, 136}, 7},
  {ANIM_LEVEL, TICRATE / 3, 3, {192, 144}, 8}, {ANIM_LEVEL, TICRATE / 3, 1, {128, 136}, 8}};
...
static int NUMANIMS[NUMEPISODES] = {sizeof(epsd0animinfo) / sizeof(anim_t),
  sizeof(epsd1animinfo) / sizeof(anim_t), sizeof(epsd2animinfo) / sizeof(anim_t)};
...
dboolean WI_Responder(event_t *ev) { return false; }
...
void WI_drawLF(void)
{
	...
	else
	{
      ...
      /* cph - get the graphic lump name and use it */
		...
		if (W_CheckNumForName(lname) == -1) return;
  ...
  // CPhipps - patch drawing updated
	V_DrawNamePatch(
	  (320 - V_NamePatchWidth(finished)) / 2, y, FB, finished, CR_DEFAULT, VPT_STRETCH);
}
...
void WI_drawEL(void)
{
	...
	// CPhipps - patch drawing updated
	V_DrawNamePatch(
	  (320 - V_NamePatchWidth(entering)) / 2, y, FB, entering, CR_DEFAULT, VPT_STRETCH);
  ...
  (int n, const char *const c[])
{
    ...
    // CPhipps - patch drawing updated
		V_DrawNamePatch(
		  lnodes[wbs->epsd][n].x, lnodes[wbs->epsd][n].y, FB, c[i], CR_DEFAULT, VPT_STRETCH);
	}
	else
	{
    // DEBUG
...
void WI_initAnimatedBack(int entering)
{
	...
	if (wbs->epsd < 0 || wbs->epsd > 2) return;
...
static void WI_drawTimeStats(int cnt_time, int cnt_total_time, int cnt_par)
{
  ...
  // Ty 03/17/98: unless pars changed with deh patch
	...
	if (!(modifiedgame && !deh_pars && !um_pars) || (gamemission == pack_nerve && singleplayer))
	{
...
void WI_drawShowNextLoc(void)
{
	...
	if (gamemapinfo != NULL && gamemapinfo->endpic[0] && strcmp(gamemapinfo->endpic, "-") != 0)
		return;
  ...
  // custom interpic.
    ...
    // draw a splat on taken cities.
		for (i = 0; i <= last; i++) WI_drawOnLnode(i, splat);

    // splat the secret level?
		if (wbs->didsecret) WI_drawOnLnode(8, splat);

    // draw flashing ptr
		if (snl_pointeron) WI_drawOnLnode(wbs->next, yah);
	}

	if (gamemission == pack_nerve && singleplayer && wbs->last == 7) return; // MAP08 end game

  // draws which level you are entering..
	if ((gamemode != commercial) || wbs->next != 30)  // check for MAP30 end game
		WI_drawEL();
}
...
{
	int i;
	for (i = 0; i < MAXPLAYERS; i++) free(dm_frags[i]);
...
void WI_updateDeathmatchStats(void)
{
	...
	if (dm_state == 2)
	{
		if (!(bcnt & 3)) S_StartSound(0, sfx_pistol);  // noise while counting
			...
			if (playeringame[i])
			{
				for (j = 0; j < MAXPLAYERS; j++)
				{
					if (playeringame[j] && dm_frags[i][j] != plrs[i].frags[j])
					{
						...
						if (dm_frags[i][j] < -999) dm_frags[i][j] = -999;
				...
				if (dm_totals[i] > 999) dm_totals[i] = 999;

				if (dm_totals[i] < -999) dm_totals[i] = -999; // Ty 03/17/98 end 3-digit frag count
			}
		}
...
void WI_drawDeathmatchStats(void)
{
	...
	V_DrawNamePatch(DM_TOTALSX - V_NamePatchWidth(total) / 2, DM_MATRIXY - WI_SPACINGY + 10, FB,
	  total, CR_DEFAULT, VPT_STRETCH);
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
		if (playeringame[i])
		{
			...
			V_DrawNamePatch(x - halfface, DM_MATRIXY - WI_SPACINGY, FB, facebackp,
			  i ? CR_LIMIT + i : CR_DEFAULT, VPT_STRETCH | (i ? VPT_TRANS : 0));
			V_DrawNamePatch(DM_MATRIXX - halfface, y, FB, facebackp, i ? CR_LIMIT + i : CR_DEFAULT,
			  VPT_STRETCH | (i ? VPT_TRANS : 0));

			if (i == me)
			{
				V_DrawNamePatch(
				  x - halfface, DM_MATRIXY - WI_SPACINGY, FB, bstar, CR_DEFAULT, VPT_STRETCH);
				V_DrawNamePatch(DM_MATRIXX - halfface, y, FB, star, CR_DEFAULT, VPT_STRETCH);
			}
		}
		x += DM_SPACINGX;
		y += WI_SPACINGY;
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
		...
		if (playeringame[i])
		{
			for (j = 0; j < MAXPLAYERS; j++)
			{
				if (playeringame[j]) WI_drawNum(x + w, y, dm_frags[i][j], 2);
...
void WI_initNetgameStats(void)
{
	...
	for (i = 0; i < MAXPLAYERS; i++)
		if (playeringame[i]) dofrags += WI_fragSum(i);
...
void WI_updateNetgameStats(void)
{
	...
	if (acceleratestage && ng_state != 10)
	{
		...
		for (i = 0; i < MAXPLAYERS; i++)
		{
			...
			if (dofrags) cnt_frags[i] = WI_fragSum(i); // we had frags
		}
		S_StartSound(0, sfx_barexp); // bang
		ng_state = 10;
	}
	...
	if (ng_state == 2)
	{
		if (!(bcnt & 3)) S_StartSound(0, sfx_pistol); // pop
	...
	{
		if (!(bcnt & 3)) S_StartSound(0, sfx_pistol);
	...
	else if (ng_state == 6)
	{
		...
		for (i = 0; i < MAXPLAYERS; i++)
		{
			...
			if (cnt_secret[i] >= (wbs->maxsecret ? (plrs[i].ssecret * 100) / wbs->maxsecret
...
void WI_drawNetgameStats(void)
{
	...
	// draw stat titles (top line)
	V_DrawNamePatch(NG_STATSX + NG_SPACINGX - V_NamePatchWidth(kills), NG_STATSY, FB, kills,
	  CR_DEFAULT, VPT_STRETCH);

	V_DrawNamePatch(NG_STATSX + 2 * NG_SPACINGX - V_NamePatchWidth(items), NG_STATSY, FB, items,
	  CR_DEFAULT, VPT_STRETCH);

	V_DrawNamePatch(NG_STATSX + 3 * NG_SPACINGX - V_NamePatchWidth(secret), NG_STATSY, FB, secret,
	  CR_DEFAULT, VPT_STRETCH);

	if (dofrags)
		V_DrawNamePatch(NG_STATSX + 4 * NG_SPACINGX - V_NamePatchWidth(frags), NG_STATSY, FB, frags,
		  CR_DEFAULT, VPT_STRETCH);
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
		...
		x = NG_STATSX;
		V_DrawNamePatch(x - fwidth, y, FB, facebackp, i ? CR_LIMIT + i : CR_DEFAULT,
		  VPT_STRETCH | (i ? VPT_TRANS : 0));

		if (i == me) V_DrawNamePatch(x - fwidth, y, FB, star, CR_DEFAULT, VPT_STRETCH);

		x += NG_SPACINGX;
		if (cnt_kills) WI_drawPercent(x - pwidth, y + 10, cnt_kills[i]);
		x += NG_SPACINGX;
		if (cnt_items) WI_drawPercent(x - pwidth, y + 10, cnt_items[i]);
		x += NG_SPACINGX;
		if (cnt_secret) WI_drawPercent(x - pwidth, y + 10, cnt_secret[i]);
		x += NG_SPACINGX;

		if (dofrags && cnt_frags) WI_drawNum(x, y + 10, cnt_frags[i], -1);
	...
	if (y <= SP_TIMEY)
		...
		WI_drawTimeStats(
		  plrs[me].stime / TICRATE, wbs->totaltimes / TICRATE, wbs->partime / TICRATE);
}
...
void WI_initStats(void)
{
	...
	*(cnt_kills = malloc(sizeof(*cnt_kills))) = *(cnt_items = malloc(sizeof(*cnt_items)))
...
void WI_updateStats(void)
{
	...
	{
		if (!(bcnt & 3)
		    && play_early_explosion) // e6y: do not play count sound after explosion sound
		...
		if (cnt_total_time >= wbs->totaltimes / TICRATE) cnt_total_time = wbs->totaltimes / TICRATE;
...
void WI_drawStats(void)
{
	...
	if (cnt_kills) WI_drawPercent(320 - SP_STATSX, SP_STATSY, cnt_kills[0]);

	V_DrawNamePatch(SP_STATSX, SP_STATSY + lh, FB, items, CR_DEFAULT, VPT_STRETCH);
	if (cnt_items) WI_drawPercent(320 - SP_STATSX, SP_STATSY + lh, cnt_items[0]);

	V_DrawNamePatch(SP_STATSX, SP_STATSY + 2 * lh, FB, sp_secret, CR_DEFAULT, VPT_STRETCH);
	if (cnt_secret) WI_drawPercent(320 - SP_STATSX, SP_STATSY + 2 * lh, cnt_secret[0]);
...
void WI_initVariables(wbstartstruct_t *wbstartstruct)
{
	...
	if (!wbs->maxkills) wbs->maxkills = 1; // probably only useful in MAP30
...
void WI_Start(wbstartstruct_t *wbstartstruct)
{
	...
	enterpic
	  = (wbs->nextmapinfo && wbs->nextmapinfo->enterpic[0]) ? wbs->nextmapinfo->enterpic : NULL;
