...
void P_CalcHeight(player_t *player)
{
  ...
  /* killough 10/98: Make bobbing depend only on player-applied motion.
	...
	if ((player->mo->flags & MF_FLY) && !onground) player->bob = FRACUNIT / 2;
	...
	if (mbf_features)
	{
		...
		{
			player->bob
			  = (FixedMul(player->momx, player->momx) + FixedMul(player->momy, player->momy)) >> 2;
		}
	}
	...
	// e6y
	if (!prboom_comp[PC_PRBOOM_FRICTION].state && compatibility_level >= boom_202_compatibility
...
void P_MovePlayer(player_t *player)
{
	...
	{
		if (upmove > 0 && onground && player == &players[consoleplayer]
  ...
  // e6y
	    ...
	    || (cmd->forwardmove | cmd->sidemove)) // killough 10/98
	{
		...
		else if (comperr(comperr_allowjump))
		{
			if (!onground)
			{
				if (cmd->forwardmove) P_Thrust(player, mo->angle, FRACUNIT >> 8);
				if (cmd->sidemove) P_Thrust(player, mo->angle, FRACUNIT >> 8);
			}
		}
...
void P_PlayerThink(player_t *player)
{
	...
	if (movement_smooth)
	{
		...
		if (&players[displayplayer] == player) P_ResetWalkcam();
	}
