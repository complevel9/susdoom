...
void R_LoadTrigTables(void)
{
	int lump;
	{
		...
		if (W_LumpLength(lump) != sizeof(finesine)) I_Error("R_LoadTrigTables: Invalid SINETABL");
	...
	}
	{
		...
		if (lump == -1) I_Error("Failed to locate trig tables");
		if (W_LumpLength(lump) != sizeof(tantoangle)) I_Error("R_LoadTrigTables: Invalid TANTOANG");
