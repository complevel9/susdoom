...
static int addsfx(int sfxid, int channel, const unsigned char *data, size_t len)
{
    ...
    // ci->channels = data[22] | (data[23] << 8);
		...
		ci->data = data + 44;
		ci->enddata
		  = data + 44 + (data[40] | (data[41] << 8) | (data[42] << 16) | (data[43] << 24));
...
static int getSliceSize(void)
{
	...
	if (snd_samplecount >= 32) return snd_samplecount * snd_samplerate / 11025;
  ...
  // as well as to use SAMPLERATE as defined above, not to assume 11025 Hz
	if (pitched_sounds)
		channelinfo[slot].step
		  = (unsigned int)(((uint64_t)channelinfo[slot].samplerate * steptable[pitch])
		                   / snd_samplerate);
	else
...
void I_SetChannels(void)
{
  ...
  // Okay, reset internal mixing channels to zero.
	for (i = 0; i < MAX_CHANNELS; i++) memset(&channelinfo[i], 0, sizeof(channel_info_t));

  // This table provides step widths for pitch parameters.
  // I fail to see that this is currently used.
	for (i = -128; i < 128; i++) steptablemid[i] = (int)(pow(1.2, (double)i / 64.0) * 65536.0);
...
int I_StartSound(int id, int channel, int vol, int sep, int pitch, int priority)
{
	...
	if (snd_pcspeaker) return I_PCS_StartSound(id, channel, vol, sep, pitch, priority);
...
{
#ifdef RANGECHECK
	if ((handle < 0) || (handle >= MAX_CHANNELS)) I_Error("I_StopSound: handle out of range");
#endif
...
{
#ifdef RANGECHECK
	if ((handle < 0) || (handle >= MAX_CHANNELS)) I_Error("I_SoundIsPlaying: handle out of range");
#endif
...
static void I_UpdateSound(void *unused, Uint8 *stream, int len)
{
  ...
  // Mixing channel index.
	...
	if (snd_midiplayer
	    == NULL) // This is but a temporary fix. Please do remove after a more definitive one!
		memset(stream, 0, len);
  ...
  //  that is 512 values for two channels.
    ...
    //  as well. Thus loop those  channels.
      ...
      // Check channel, if active.
        ...
        // linear filtering
				// the old SRC did linear interpolation back into 8 bit, and then expanded to 16
				// bit. this does interpolation and 8->16 at same time, allowing slightly higher
				// quality
				if (ci->bits == 16)
				{
				...
				else
				{
					s = (ci->data[0] * (0x10000 - ci->stepremainder))
					  + (ci->data[1] * (ci->stepremainder)) - 0x800000; // convert to signed
				}
...
void I_InitSound(void)
{
	...
	if (!use_experimental_music)
	{
		...
		if (Mix_OpenAudioDevice(audio_rate, MIX_DEFAULT_FORMAT, audio_channels, audio_buffers, NULL,
...
void I_SetSoundCap(void) { dumping_sound = 1; }
...
{ // assumes 16 bit signed interleaved stereo
	...
	{
		*sout++
		  = ((unsigned)sin[j + 0] * (0x10000 - remainder) + (unsigned)sin[j + 2] * remainder) >> 16;
		*sout++
		  = ((unsigned)sin[j + 1] * (0x10000 - remainder) + (unsigned)sin[j + 3] * remainder) >> 16;
		remainder += step;
		j += remainder >> 16 << 1;
		remainder &= 0xffff;
	}
...
void I_ShutdownMusic(void)
{
	...
	if (music_tmp)
	{
		...
		for (i = 0; i < MUSIC_TMP_EXT; i++)
		{
			...
			if (!unlink(name)) lprintf(LO_DEBUG, "I_ShutdownMusic: removed %s\n", name);
			free(name);
		}
		free(music_tmp);
		music_tmp = NULL;
...
void I_InitMusic(void)
{
	...
	return;
#endif
	lprintf(LO_INFO, "I_InitMusic: Was compiled without SDL_Mixer support.  You should enable "
	                 "experimental music.\n");
}
...
int I_RegisterSong(const void *data, size_t len)
{
	...
	if (use_experimental_music) return Exp_RegisterSong(data, len);
#ifdef HAVE_MIXER
	...
	if (len > 4 && memcmp(data, "MUS", 3) != 0)
	{
		...
		for (i = 0; i < MUSIC_TMP_EXT; i++)
		{
			...
			if (strlen(music_tmp_ext[i]) == 0)
			{
				...
				if (rw_midi) music[0] = Mix_LoadMUS_RW(rw_midi, SDL_FALSE);
			}

			if (!music[0])
			{
				io_errors = (M_WriteFile(name, data, len) == 0);
				if (!io_errors) music[0] = Mix_LoadMUS(name);
			}
...
{
	if (use_experimental_music) return Exp_RegisterMusic(filename, song);
...
static int Exp_RegisterSongEx(const void *data, size_t len, int try_mus2mid)
{
		...
		for (j = 0; j < NUM_MUS_PLAYERS; j++)
		{
			...
			for (i = 0; music_players[i]; i++)
			{
				if (strcmp(music_players[i]->name(), music_player_order[j]) == 0)
				{
					...
					if (music_player_was_init[i])
					{
						...
						lprintf(LO_INFO,
						  "Exp_RegisterSongEx: Music player %s on preferred list but it failed to "
						  "init\n",
						  music_players[i]->name());
				}
			}
			if (!found)
				lprintf(LO_INFO,
				  ...
				  "support not included at compile time)\n",
	...
	if (len > 4 && try_mus2mid)
	{
		...
		if (result == 0)
		{
			...
			if (song_data) return Exp_RegisterSongEx(song_data, outbuf_len, 0);
		}
	}
...
void M_ChangeMIDIPlayer(void)
{
	...
	if (!strcasecmp(snd_midiplayer, midiplayers[midi_player_sdl])) { experimental_music = false; }
	else
	{
...
#if 1
	if (use_experimental_music == -1) { use_experimental_music = experimental_music; }
