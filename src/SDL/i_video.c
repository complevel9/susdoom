...
static int VanillaTranslateKey(SDL_Keysym *key)
{
	...
	if (sc > 3 && sc < sizeof(scancode_map) / sizeof(scancode_map[0])) rc = scancode_map[sc];
  // Key is mapped..
	if (rc) return rc;
...
static int I_TranslateKey(SDL_Keysym *key)
{
	...
	if (vanilla_keymap) return VanillaTranslateKey(key);
...
{
	return 0 | (buttonstate & SDL_BUTTON(1) ? 1 : 0) | (buttonstate & SDL_BUTTON(2) ? 2 : 0)
	     | (buttonstate & SDL_BUTTON(3) ? 4 : 0) | (buttonstate & SDL_BUTTON(6) ? 8 : 0)
	     | (buttonstate & SDL_BUTTON(7) ? 16 : 0) | (buttonstate & SDL_BUTTON(4) ? 32 : 0)
	     | (buttonstate & SDL_BUTTON(5) ? 64 : 0) | (buttonstate & SDL_BUTTON(8) ? 128 : 0);
}
...
static void I_GetEvent(void)
{
	...
	while (SDL_PollEvent(Event))
	{
      ...
      // Prevent executing action on Alt-Tab
				if (Event->key.keysym.sym == SDLK_TAB) { break; }
...
static void I_InitInputs(void)
{
  ...
  // Create an empty cursor
	...
	if (mouse_enabled) MouseAccelChanging();
...
// I_SetPalette
//
void I_SetPalette(int pal) { newpal = pal; }
...
void I_PreInitGraphics(void)
{
  ...
  // Initialize SDL
	...
	if (!(M_CheckParm("-nodraw") && M_CheckParm("-nosound"))) flags = SDL_INIT_VIDEO;
#ifdef PRBOOM_DEBUG
	flags |= SDL_INIT_NOPARACHUTE;
#endif

	p = SDL_Init(flags);
	if (p < 0) I_Error("Could not initialize SDL [%s]", SDL_GetError());
...
static void I_FillScreenResolutionsList(void)
{
  ...
  // do it only once
	if (screen_resolutions_list[0]) return;

	if (desired_screenwidth == 0 || desired_screenheight == 0) I_GetScreenResolution();
    ...
    // -2 for the desired resolution and for NULL
      ...
      // make sure the canonical resolutions are always available
        ...
        // no hard-coded resolutions for mode-changing fullscreen
			...
			else { SDL_GetDisplayMode(display_index, i, &mode); }
  ...
  // [FG] sort the list
	SDL_qsort(
	  screen_resolutions_list, list_size, sizeof(*screen_resolutions_list), cmp_resolutions);
...
// e6y
// GLBoom use this function for trying to set the closest supported resolution if the requested mode
// can't be set correctly. For example glboom.exe -geom 1025x768 -nowindow will set 1024x768. It
// should be used only for fullscreen modes.
static void I_ClosestResolution(int *width, int *height)
{
	...
	if (count > 0)
	{
		for (i = 0; i < count; ++i)
		{
			...
			dist
			  = (twidth - *width) * (twidth - *width) + (theight - *height) * (theight - *height);
...
static void I_CalculateRes(int width, int height)
{
	...
	if (V_GetMode() == VID_MODEGL)
	{
		if (desired_fullscreen) I_ClosestResolution(&width, &height);
		SCREENWIDTH = width;
		SCREENHEIGHT = height;
	...
	else
	{
		...
		if (desired_fullscreen && exclusive_fullscreen) I_ClosestResolution(&width, &height);
		...
		else { SCREENPITCH = SCREENWIDTH * V_GetPixelDepth(); }
	}
}
...
#ifndef GL_DOOM
	if (mode == VID_MODEGL) mode = (video_mode_t)I_GetModeFromString(default_videomode = "8bit");
#endif
...
void I_SetWindowCaption(void) { SDL_SetWindowTitle(NULL, PACKAGE_NAME " " PACKAGE_VERSION); }
...
void I_SetWindowIcon(void)
{
	...
	if (!surface)
	{
		surface = SDL_CreateRGBSurfaceFrom(
		  icon_data, icon_w, icon_h, 32, icon_w * 4, 0xff << 0, 0xff << 8, 0xff << 16, 0xff << 24);
	}

	if (surface) SDL_SetWindowIcon(NULL, surface);
}
...
void I_UpdateVideoMode(void)
{
	...
	int screen_multiply;
	int actualheight;
	const dboolean novsync = M_CheckParm("-timedemo") || M_CheckParm("-fastdemo");
	...
	if (V_GetMode() == VID_MODEGL) init_flags = SDL_WINDOW_OPENGL;
...
#ifndef MACOSX
	if (!desired_fullscreen && V_GetMode() != VID_MODEGL) init_flags |= SDL_WINDOW_RESIZABLE;
#endif
	...
	if (V_GetMode() == VID_MODEGL)
	{
		...
		sdl_window = SDL_CreateWindow(PACKAGE_NAME " " PACKAGE_VERSION, SDL_WINDOWPOS_CENTERED,
	...
	else
	{
		...
		{
			SDL_SetWindowSize(
			  sdl_window, screen_multiply * SCREENWIDTH, screen_multiply * actualheight);
		}
		...
		screen
		  = SDL_CreateRGBSurface(0, SCREENWIDTH, SCREENHEIGHT, V_GetNumPixelBits(), 0, 0, 0, 0);
		...
		{
			I_Error(
			  "Couldn't set %dx%d video mode [%s]", SCREENWIDTH, SCREENHEIGHT, SDL_GetError());
		}
	}
...
#ifdef GL_DOOM
	if (V_GetMode() == VID_MODEGL) SDL_GL_SetSwapInterval(((render_vsync && !novsync) ? 1 : 0));
#endif
	...
	if (V_GetMode() != VID_MODEGL)
	{
		...
		else { screens[0].not_on_heap = false; }
...
static void DeactivateMouse(void) { SDL_SetRelativeMouseMode(SDL_FALSE); }
...
static void I_ReadMouse(void)
{
	...
	if (!mouse_currently_grabbed && !desired_fullscreen) mouse_currently_grabbed = true;
}
...
static dboolean MouseShouldBeGrabbed()
{
	...
	if (walkcamera.type) return (demoplayback && gamestate == GS_LEVEL && !menuactive);
...
static void UpdateFocus(void)
{
	...
	if (V_GetMode() == VID_MODEGL)
	{
		if (gl_hardware_gamma)
		{
			...
			else { gld_SetGammaRamp(useglgamma); }
		}
	}
#endif
...
void UpdateGrab(void)
{
	...
	if (grab && !currently_grabbed) ActivateMouse();

	if (!grab && currently_grabbed) DeactivateMouse();
