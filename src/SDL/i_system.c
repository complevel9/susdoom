...
void I_EndDisplay(void) { InDisplay = false; }
...
void I_Read(int fd, void *vbuf, size_t sz)
{
	...
	while (sz)
	{
		...
		if (rc <= 0) I_Error("I_Read: read failed: %s", rc ? strerror(errno) : "EOF");
		sz -= rc;
		buf += rc;
	}
}
...
int I_Filelength(int handle)
{
	...
	if (fstat(handle, &fileinfo) == -1) I_Error("I_Filelength: %s", strerror(errno));
...
void I_SwitchToWindow(HWND hwnd)
{
	...
	if (SwitchToThisWindow)
	{
		...
		if (IsWindowVisible(hwndLastActive)) hwnd = hwndLastActive;
...
const char *I_DoomExeDir(void)
{
	...
	if (!base)        // cache multiple requests
	{
		...
		if (strlen(base) < 2 || M_access(base, W_OK) != 0)
		{
			...
			if (!M_getcwd(base, 1024) || M_access(base, W_OK) != 0) strcpy(base, current_dir_dummy);
		}
	}
	return base;
}
...
const char *I_GetTempDir(void)
{
	...
	if (tmp_path[0] == 0) GetTempPath(sizeof(tmp_path), tmp_path);
...
char *I_FindFileInternal(const char *wfname, const char *ext, dboolean isStatic)
{
  ...
  /* Precalculate a length we will need in the loop */
    ...
    // switch replaced with lookup table
		if (search[i].env)
		{
			if (!(d = M_getenv(search[i].env))) continue;
		}
		...
		if (!isStatic) p = (char *)malloc((d ? strlen(d) : 0) + (s ? strlen(s) : 0) + pl);
		sprintf(p, "%s%s%s%s%s", d ? d : "", (d && !HasTrailingSlash(d)) ? "/" : "", s ? s : "",
		  (s && !HasTrailingSlash(s)) ? "/" : "", wfname);

		if (ext && M_access(p, F_OK)) strcat(p, ext);
		if (!M_access(p, F_OK))
		{
