...
static int main_getcmdline(void)
{
	...
	if (argvw == NULL) return OutOfMemory();

	/* Parse it into argv and argc */
	argv = (char **)SDL_calloc(argc + 1, sizeof(*argv));
	if (!argv) return OutOfMemory();
	for (i = 0; i < argc; ++i)
	{
		argv[i] = WIN_WStringToUTF8(argvw[i]);
		if (!argv[i]) return OutOfMemory();
	}
	argv[i] = NULL;
	LocalFree(argvw);
...
int console_ansi_main(int argc, char *argv[]) { return main_getcmdline(); }


#if UNICODE
/* This is where execution begins [console apps, unicode] */
int console_wmain(int argc, wchar_t *wargv[], wchar_t *wenvp) { return main_getcmdline(); }
#endif
