...
//
void STlib_initNum(
  st_number_t *n, int x, int y, const patchnum_t *pl, int *num, dboolean *on, int width)
{
	n->x = x;
	n->y = y;
	n->oldnum = 0;
...
#ifdef RANGECHECK
	if (n->y - ST_Y < 0) I_Error("STlib_drawNum: n->y - ST_Y < 0");
#endif
...
// Returns nothing.
//
void STlib_initPercent(st_percent_t *p, int x, int y, const patchnum_t *pl, int *num, dboolean *on,
...
void STlib_updatePercent(st_percent_t *per, int cm, int refresh)
{
    ...
    // CPhipps - patch drawing updated
		V_DrawNumPatch(per->n.x, per->n.y, FG, per->p->lumpnum, sts_pct_always_gray ? CR_GRAY : cm,
...
void STlib_initMultIcon(
  st_multicon_t *i, int x, int y, const patchnum_t *il, int *inum, dboolean *on)
{
	i->x = x;
	i->y = y;
	i->oldinum = -1;
...
void STlib_updateMultIcon(st_multicon_t *mi, dboolean refresh)
{
	...
	if (*mi->on && (mi->oldinum != *mi->inum || refresh))
	{
...
#ifdef RANGECHECK
			if (y - ST_Y < 0) I_Error("STlib_updateMultIcon: y - ST_Y < 0");
#endif
		...
		if (*mi->inum != -1)  // killough 2/16/98: redraw only if != -1
			V_DrawNumPatch(
			  mi->x, mi->y, FG, mi->p[*mi->inum].lumpnum, CR_DEFAULT, VPT_ALIGN_BOTTOM);
		mi->oldinum = *mi->inum;
	}
}
...
void STlib_initBinIcon(
  st_binicon_t *b, int x, int y, const patchnum_t *i, dboolean *val, dboolean *on)
{
	b->x = x;
	b->y = y;
	b->oldval = 0;
