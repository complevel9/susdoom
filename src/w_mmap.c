...
void W_PrintLump(FILE *fp, void *p)
{
	...
	for (i = 0; i < numlumps; i++)
		...
		{
			fprintf(fp, " %8.8s %6u %2d %6d", lumpinfo[i].name, W_LumpLength(i), cachelump[i].locks,
...
static void W_ReportLocks(void)
{
	...
	if (cachelump)
	{
		for (i = 0; i < numlumps; i++)
		{
			...
			{
				lprintf(LO_DEBUG, "%8.8s %6u %2d   %6d\n", lumpinfo[i].name, W_LumpLength(i),
  ...
  // set up caching
	...
	memset(mapped_wad, 0, sizeof(mmap_info_t) * numwadfiles);
	{
		...
		for (i = 0; i < numlumps; i++)
		{
			...
			if (!lumpinfo[i].wadfile) continue;
#ifdef RANGECHECK
			...
			if (!mapped_wad[wad_index].data)
			{
				...
				mapped_wad[wad_index].hnd = CreateFileW(wname, GENERIC_READ,
				  FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
				free(wname);
				if (mapped_wad[wad_index].hnd == INVALID_HANDLE_VALUE)
					I_Error("W_InitCache: CreateFile for memory mapping failed (LastError %i)",
					  GetLastError());
				mapped_wad[wad_index].hnd_map
				  = CreateFileMapping(mapped_wad[wad_index].hnd, NULL, PAGE_READONLY, 0, 0, NULL);
				if (mapped_wad[wad_index].hnd_map == NULL)
					I_Error(
					  "W_InitCache: CreateFileMapping for memory mapping failed (LastError %i)",
					  GetLastError());
				mapped_wad[wad_index].data
				  = MapViewOfFile(mapped_wad[wad_index].hnd_map, FILE_MAP_READ, 0, 0, 0);
				if (mapped_wad[wad_index].data == NULL)
					I_Error("W_InitCache: MapViewOfFile for memory mapping failed (LastError %i)",
					  GetLastError());
			}
		}
	}
...
const void *W_CacheLumpNum(int lump)
{
	...
	if ((unsigned)lump >= (unsigned)numlumps) I_Error("W_CacheLumpNum: %i >= numlumps", lump);
#endif
	if (!lumpinfo[lump].wadfile) return NULL;
	...
	mapped_wad = calloc(maxfd + 1, sizeof *mapped_wad);
	{
		...
		for (i = 0; i < numlumps; i++)
		{
			...
			if (lumpinfo[i].wadfile)
			{
				...
				if (!mapped_wad[fd])
					if ((mapped_wad[fd]
					      = mmap(NULL, I_Filelength(fd), PROT_READ, MAP_SHARED, fd, 0))
					    == MAP_FAILED)
...
const void *W_CacheLumpNum(int lump)
{
	...
	return (const void *)(((const byte *)(mapped_wad[lumpinfo[lump].wadfile->handle]))
...
{
	if (cachelump[lump].locks == -1) return; // this lump is memory mapped

	cachelump[lump].locks -= 1;
  /* cph - Note: must only tell z_zone to make purgeable if currently locked,
	 * else it might already have been purged
	 */
	if (cachelump[lump].locks == 0) Z_ChangeTag(cachelump[lump].cache, PU_CACHE);
}
