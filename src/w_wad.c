...
// killough 1/31/98: static, const
{
	...
	if (wadfile->src == source_skip) return;
  ...
  // open the file and add to directory
	...
	if (wadfile->handle == -1 && strlen(wadfile->name) > 4 && wadfile->src == source_pwad
...
static int W_CoalesceMarkedResource(
  const char *start_marker, const char *end_marker, li_namespace_e li_namespace)
{
	int result = 0;
...
int(W_FindNumFromName)(const char *name, int li_namespace, int i)
{
  ...
  // Doom wads.
		...
		while (
		  i >= 0
		  && (strncasecmp(lumpinfo[i].name, name, 8) || lumpinfo[i].li_namespace != li_namespace))
			i = lumpinfo[i].next;
	}
...
void W_HashLumps(void)
{
	...
	for (i = 0; i < numlumps; i++) lumpinfo[i].index = -1;                     // mark slots empty
...
int W_GetNumForName(const char *name)     // killough -- const added
{
	...
	if (i == -1) I_Error("W_GetNumForName: %.8s not found", name);
	return i;
}

const lumpinfo_t *W_GetLumpInfoByNum(int lump)
{
	if (lump < 0 || lump >= numlumps) I_Error("W_GetLumpInfoByNum: lump num %d out of range", lump);
...
int W_CheckNumForNameInternal(const char *name)
{
	...
	for (p = -1; (p = W_ListNumFromName(name, p)) >= 0;)
		if (lumpinfo[p].flags == LUMP_PRBOOM) return p;
	return -1;
}
...
void W_Init(void)
{
  // CPhipps - start with nothing
    ...
    // open all the files, load headers, and count lumps
		int i;
		for (i = 0; (size_t)i < numwadfiles; i++) W_AddFile(&wadfiles[i]);
	}

	if (!numlumps) I_Error("W_Init: No files found");
