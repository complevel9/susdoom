...
static dboolean IsDirectory(char *dir, struct dirent *de)
{
	...
	if (de->d_type != DT_UNKNOWN && de->d_type != DT_LNK) { return de->d_type == DT_DIR; }
	else
#endif
	{
		char *filename;
...
{
	int i;
	for (i = 0; i < num_globs; ++i) free(globs[i]);
	free(globs);
}
...
glob_t *I_StartMultiGlob(const char *directory, int flags, const char *glob, ...)
{
	...
	for (;;)
	{
		...
		if (new_globs == NULL) FreeStringList(globs, num_globs);
		globs = new_globs;
...
static dboolean MatchesGlob(const char *name, const char *glob, int flags)
{
	...
	while (*glob != '\0')
	{
		...
		if (g == '*')
		{
			...
			while (*name != '\0')
			{
				if (MatchesGlob(name, glob + 1, flags)) return true;
				++name;
			}
			return glob[1] == '\0';
		}
...
static dboolean MatchesAnyGlob(const char *name, glob_t *glob)
{
	int i;

	for (i = 0; i < glob->num_globs; ++i)
		if (MatchesGlob(name, glob->globs[i], glob->flags)) return true;
	return false;
}
...
const char *I_NextGlob(glob_t *glob)
{
	...
	if (glob->next_index >= glob->filenames_len) return NULL;
