...
GLenum gl_fuzzdfactors[]
  = {GL_ONE_MINUS_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, GL_ONE};
...
void SetFrameTextureMode(void)
{
	...
	if (SceneInTexture) { glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE); }
	else
#endif
...
void gld_InitTextureParams(void)
{
	...
	tex_format_t tex_formats[] = {{GL_RGBA2, "GL_RGBA2"}, {GL_RGBA4, "GL_RGBA4"},
	  {GL_RGB5_A1, "GL_RGB5_A1"}, {GL_RGBA8, "GL_RGBA8"}, {GL_RGBA, "GL_RGBA"}, {0, NULL}};
	...
	if (tex_filter[MIP_TEXTURE].mipmap) gl_shared_texture_palette = false;
...
void gld_MultisamplingCheck(void)
{
	if (render_multisampling)
	{
		...
		if (test != render_multisampling)
		{
			...
			M_SaveDefaults();
			I_Error(
			  "Couldn't set %dX multisamples for %dx%d video mode", i, SCREENWIDTH, SCREENHEIGHT);
		}
	}
}
...
void gld_MultisamplingSet(void)
{
	if (render_multisampling)
	{
		int use_multisampling
		  = map_use_multisamling || (!(automapmode & am_active) || (automapmode & am_overlay));
...
void gld_Init(int width, int height)
{
  ...
  // Create FBO object and associated render targets
	...
	if (!gld_LoadGLDefs("GLBDEFS")) gld_LoadGLDefs("GLDEFS");
...
void gld_ResetTexturedAutomap(void) { visible_subsectors_count_prev = -1; }

void gld_MapDrawSubsectors(
  player_t *plr, int fx, int fy, fixed_t mx, fixed_t my, int fw, int fh, fixed_t scale)
{
	...
	alpha = (float)((automapmode & am_overlay) ? map_textured_overlay_trans : map_textured_trans)
	      / 100.0f;
	if (alpha == 0) return;
	...
	if (numsubsectors > visible_subsectors_size)
	{
		...
		visible_subsectors
		  = realloc(visible_subsectors, visible_subsectors_size * sizeof(visible_subsectors[0]));
	}
	...
	if (ddt_cheating)
		...
		for (i = 0; i < numsubsectors; i++) visible_subsectors_count += map_subsectors[i];
  ...
  // if there are no new visible subsectors.
    ...
    // sort subsectors by texture
		qsort(visible_subsectors, visible_subsectors_count, sizeof(visible_subsectors[0]),
		  dicmp_visible_subsectors_by_pic);
	}
	...
	glTranslatef((float)fx - (float)mx / (float)FRACUNIT * (float)scale / (float)FRACUNIT,
	  (float)fy + (float)fh + (float)my / (float)FRACUNIT * (float)scale / (float)FRACUNIT, 0);
      ...
      // Find texture origin.
        ...
        // go through all vertexes of this loop
				for (vertexnum = currentloop->vertexindex;
				  vertexnum < (currentloop->vertexindex + currentloop->vertexcount); vertexnum++)
				{
					glTexCoord2f(
					  flats_vbo[vertexnum].u + floor_uoffs, flats_vbo[vertexnum].v + floor_voffs);
...
void gld_DrawNumPatch_f(float x, float y, int lump, int cm, enum patch_translation_e flags)
{
  ...
  // [FG] automatically center wide patches without horizontal offset
	if (gltexture->width > 320 && leftoffset == 0) x -= (float)(gltexture->width - 320) / 2;
	...
	bFakeColormap = (gltexture->flags & GLTEXTURE_HIRES) && (lumpinfo[lump].flags & LUMP_CM2RGB);
	if (bFakeColormap)
	{
		if (!(flags & VPT_TRANS) && (cm != CR_GRAY)) cm = CR_RED;
	...
	if (bFakeColormap) glColor3f(1.0f, 1.0f, 1.0f);
}
...
void gld_DrawLine_f(float x0, float y0, float x1, float y1, int BaseColor)
{
	...
	glColor4f((float)playpal[3 * BaseColor] / 255.0f, (float)playpal[3 * BaseColor + 1] / 255.0f,
	  (float)playpal[3 * BaseColor + 2] / 255.0f, alpha);
	glBegin(GL_LINES);
	glVertex2f(x0, y0);
	glVertex2f(x1, y1);
	glEnd();
...
void gld_DrawWeapon(int weaponlump, vissprite_t *vis, int lightlevel)
{
  ...
  // There is no more line of graphics under certain weapons.
	...
	y1
	  = viewwindowy + centery - (int)(((float)vis->texturemid / (float)FRACUNIT) * pspriteyscale_f);
  ...
  // when invisibility is about to go
	...
	{
		glBlendFunc(
		  gl_fuzzsfactors[gl_weaponspritefuzzmode], gl_fuzzdfactors[gl_weaponspritefuzzmode]);
...
void gld_FillBlock(int x, int y, int width, int height, int col)
{
	...
	glColor3f((float)playpal[3 * col] / 255.0f, (float)playpal[3 * col + 1] / 255.0f,
...
void gld_SetPalette(int palette)
{
	...
	if (gl_shared_texture_palette)
	{
		...
		GLEXT_glColorTableEXT(
		  GL_SHARED_TEXTURE_PALETTE_EXT, GL_RGBA, 256, GL_RGBA, GL_UNSIGNED_BYTE, pal);
	}
	else
	{
		if (palette > 0)
...
{ // NSM convert to static
	...
	if (buffer && scr)
	{
		...
		for (i = 0; i < SCREENHEIGHT / 2; i++)
		{
			...
			memcpy(&scr[i * SCREENWIDTH * 3], &scr[(SCREENHEIGHT - (i + 1)) * SCREENWIDTH * 3],
			  SCREENWIDTH * 3);
...
GLvoid gld_Set2DMode(void)
{
	...
	glLoadIdentity();
	glOrtho((GLdouble)0, (GLdouble)SCREENWIDTH, (GLdouble)SCREENHEIGHT, (GLdouble)0, (GLdouble)-1.0,
	  (GLdouble)1.0);
	glDisable(GL_DEPTH_TEST);
}

void gld_InitDrawScene(void) { gld_ResetDrawInfo(); }

void gld_Finish(void)
{
	gld_Set2DMode();
	if (gl_finish && !render_vsync && 0) glFinish();
...
void gld_StartDrawScene(void)
{
  ...
  // e6y: fog in frame
	gl_use_fog = !gl_compatibility && (gl_fog || gl_lightmode == gl_lightmode_fogbased)
...
// e6y
	...
	gl_mask_sprite_threshold_f
	  = (gl_sprite_blend ? (float)gl_mask_sprite_threshold / 100.0f : 0.5f);
	...
	if (players[displayplayer].fixedcolormap == 32)
	{
		...
		else if (gl_version >= OPENGL_VERSION_1_3)
	...
	SceneInTexture
	  = (gl_ext_framebuffer_object) && ((invul_method & INVUL_BW) || (motion_blur.enabled));

  // Vortex: Set FBO object
	if (SceneInTexture) GLEXT_glBindFramebufferEXT(GL_FRAMEBUFFER_EXT, glSceneImageFBOTexID);
#endif
  ...
  // Vortex: Black and white effect
    ...
    // e6y: motion bloor effect for strafe50
		if (motion_blur.enabled)
		{
			...
			{
				motionblur_alpha
				  = (float)((atan(-renderer_fps / motion_blur.att_a)) / motion_blur.att_b)
				  + motion_blur.att_c;
			}
		...
		if (motion_blur.enabled) glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
...
#endif
	{
		if (invul_method & INVUL_INV) gld_InvertScene();
		if (invul_method & INVUL_BW) glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
	}
...
static void gld_AddDrawWallItem(GLDrawItemType itemtype, void *itemdata)
{
	if (gl_blend_animations)
	{
		...
		case GLDIT_FWALL:
			...
			{
				wall->alpha = 1.0f
				            - ((float)tic_vars.frac + ((leveltime - 1) % anim->speed) * 65536.0f)
				                / (65536.0f * anim->speed);
...
static void gld_DrawWall(GLWall *wall)
{
	...
	has_detail = scene_has_details && gl_arb_multitexture && (wall->flag < GLDWF_SKY)
	          && (wall->gltexture->detail)
	          && gld_IsDetailVisible(xCamera, yCamera, wall->glseg->x1, wall->glseg->z1,
  ...
  // to avoid visual glitches for textures with holes
	...
	if (!wall->gltexture) glColor4f(1.0f, 0.0f, 0.0f, 1.0f);

	if (has_detail) { gld_DrawWallWithDetail(wall); }
	else if ((wall->flag == GLDWF_TOPFLUD) || (wall->flag == GLDWF_BOTFLUD))
	{
		gl_strip_coords_t c;
...
void gld_AddWall(seg_t *seg)
{
  ...
  // for walls exactly vertical/horizontal on the map
	...
	wall.fogdensity = gld_CalcFogDensity(frontsector,
	  frontsector->lightlevel + (gl_lightmode == gl_lightmode_fogbased ? rellight : 0), GLDIT_WALL);
	wall.alpha = 1.0f;
	wall.gltexture = NULL;
	wall.seg = seg; // e6y
	...
	if (!seg->backsector) /* onesided */
	{
		...
		if (temptex && frontsector->ceilingheight > frontsector->floorheight)
		{
			...
			CALC_TEX_VALUES_MIDDLE1S(
			  wall, seg, backseg, (LINE->flags & ML_DONTPEGBOTTOM) > 0, linelength, lineheight);
    ...
    /* toptexture */
		...
		if (frontsector->ceilingpic == skyflatnum)// || backsector->ceilingpic==skyflatnum)
		{
          ...
          // e6y
			  ...
			  && (backsector->ceilingheight <= backsector->floorheight))
			{
				...
				wall.ybottom
				  = ((float)(backsector->floorheight
				             + (seg->sidedef->rowoffset > 0 ? seg->sidedef->rowoffset : 0)))
				  / MAP_SCALE;
				gld_AddSkyTexture(&wall, frontsector->sky, backsector->sky, SKY_CEILING);
			}
			else if (bs->ceilingpic == skyflatnum && fs->ceilingpic != skyflatnum
			         && toptexture == NO_TEXTURE && midtexture == NO_TEXTURE)
			{
				wall.ybottom = (float)min_ceiling / MAP_SCALE;
				if (bs->ceilingheight < fs->floorheight) fix_sky_bleed = true;
				gld_AddSkyTexture(&wall, frontsector->sky, backsector->sky, SKY_CEILING);
			}
			else if (((backsector->ceilingpic != skyflatnum && toptexture != NO_TEXTURE)
			           && midtexture == NO_TEXTURE)
			         || backsector->ceilingpic != skyflatnum
			         || backsector->ceilingheight <= frontsector->floorheight)
			{
				...
				else { wall.ybottom = (float)max_ceiling / MAP_SCALE; }
				gld_AddSkyTexture(&wall, frontsector->sky, backsector->sky, SKY_CEILING);
			}
		}
		if (floor_height < ceiling_height)
		{
			      ...
			      && (backsector->ceilingpic == skyflatnum)))
			{
				...
				if (!temptex && gl_use_stencil && backsector
				    ...
				    /*frontsector->ceilingpic != skyflatnum && */ backsector->ceilingpic
				      != skyflatnum
				    && !(backsector->flags & NULL_SECTOR))
				{
					...
					if (wall.ybottom >= zCamera)
					{
						...
						temptex
						  = gld_RegisterFlat(flattranslation[seg->backsector->ceilingpic], true);
						if (temptex)
						{
							wall.gltexture = temptex;
				...
				else if (temptex)
				{
					...
					CALC_TEX_VALUES_TOP(wall, seg, backseg,
					  (LINE->flags & (/*e6y ML_DONTPEGBOTTOM | */ ML_DONTPEGTOP)) == 0, linelength,
					  lineheight);
					gld_AddDrawWallItem(GLDIT_WALL, &wall);
				}
			}
		    ...
		    && backsector->ceilingheight > frontsector->floorheight)
		{
			...
			else
			{
				...
				ceiling_height
				  = MIN(seg->frontsector->ceilingheight, seg->backsector->ceilingheight)
				  + (seg->sidedef->rowoffset);
			...
			if (seg->frontsector != seg->backsector || seg->frontsector->heightsec != -1)
			{
				sector_t *f, *b;
...
bottomtexture:
		...
		if (frontsector->floorpic == skyflatnum)
		{
			...
			else if (bottomtexture == NO_TEXTURE && midtexture == NO_TEXTURE)
			{
		...
		if (floor_height < ceiling_height)
		{
			...
			if (!temptex && gl_use_stencil && backsector && !(seg->linedef->r_flags & RF_ISOLATED)
			    &&
			...
			else if (temptex)
			{
				...
				if (fix_sky_bleed)
				{
					...
					seg->sidedef->rowoffset
					  += (MAX(frontsector->floorheight, backsector->floorheight) - min_ceiling);
				}
				CALC_Y_VALUES(wall, lineheight, floor_height, ceiling_height);
				CALC_TEX_VALUES_BOTTOM(wall, seg, backseg, (LINE->flags & ML_DONTPEGBOTTOM) > 0,
				  linelength, lineheight, floor_height - frontsector->ceilingheight);
				gld_AddDrawWallItem(GLDIT_WALL, &wall);
...
static void gld_DrawFlat(GLFlat *flat)
{
	...
	has_detail = scene_has_details && gl_arb_multitexture && flat->gltexture->detail;
        ...
        // set texture coordinate of this vertex
				if (has_detail)
				{
					GLEXT_glMultiTexCoord2fvARB(
					  GL_TEXTURE0_ARB, (GLfloat *)&flats_vbo[vertexnum].u);
					GLEXT_glMultiTexCoord2fvARB(
					  GL_TEXTURE1_ARB, (GLfloat *)&flats_vbo[vertexnum].u);
				}
				else { glTexCoord2fv((GLfloat *)&flats_vbo[vertexnum].u); }
...
static void gld_AddFlat(int sectornum, dboolean ceiling, visplane_t *plane)
{
	...
	sector = R_FakeFlat(
	  sector, &tempsec, &floorlightlevel, &ceilinglightlevel, false); // for boom effects
...
void gld_AddPlane(int subsectornum, visplane_t *floor, visplane_t *ceiling)
{
	...
	// render the floor
	if (floor && floor->height < viewz) gld_AddFlat(subsector->sector->iSectorID, false, floor);
	// render the ceiling
...
static void gld_DrawSprite(GLSprite *sprite)
{
	...
	if (!(sprite->flags & MF_NO_DEPTH_TEST))
	{
		if (sprite->flags & MF_SHADOW)
		{
			...
			glBlendFunc(
			  gl_fuzzsfactors[gl_thingspritefuzzmode], gl_fuzzdfactors[gl_thingspritefuzzmode]);
			// glColor4f(0.2f,0.2f,0.2f,(float)tran_filter_pct/100.0f);
			glAlphaFunc(GL_GEQUAL, 0.1f);
			glColor4f(0.2f, 0.2f, 0.2f, 0.33f);
			restore = 1;
...
void gld_ProjectSprite(mobj_t *thing, int lightlevel)
{
	...
	if (heightsec != -1) // only clip things which are in special sectors
	{
		...
		if (phs != -1 && viewz < sectors[phs].floorheight ? fz >= sectors[heightsec].floorheight
	...
	// e6y FIXME!!!
	if (thing == players[displayplayer].mo && walkcamera.type != 2) goto unlock_patch;
	...
	if ((gl_spriteclip != spriteclip_const) && (sprite.y2 < 0)
	    && (sprite.y2 >= (float)(-gl_spriteclip_threshold_f))
	...
	if (frustum_culling)
	{
		if (!gld_SphereInFrustum(sprite.x + cos_inv_yaw * (sprite.x1 + sprite.x2) / 2.0f,
	...
	if (thing->flags & MF_COLOREDBLOOD) sprite.cm = thing->bloodcolor;
	...
	if (sprite.flags & MF_NO_DEPTH_TEST) { gld_AddDrawItem(GLDIT_ASPRITE, &sprite); }
	else
	{
		gld_AddDrawItem(
		  (gl_sprite_blend || (sprite.flags & (MF_SHADOW | MF_TRANSLUCENT)) ? GLDIT_TSPRITE
		                                                                    : GLDIT_SPRITE),
		  &sprite);
		gld_ProcessThingShadow(thing);
	}

	if (health_bar) gld_AddHealthBar(thing, &sprite);
...
static void gld_DrawItemsSort(
  GLDrawItemType itemtype, int(C_DECL *PtFuncCompare)(const void *, const void *))
{
	qsort(gld_drawinfo.items[itemtype], gld_drawinfo.num_items[itemtype],
		...
		if (!no_overlapped_sprites)
		{
			...
			while (i < count)
			{
				...
				if (sprite1->xy == sprite2->xy)
				{
					...
					i++;
					while (
					  i < count && gld_drawinfo.items[itemtype][i].item.sprite->xy == sprite1->xy)
					{
			...
			if (gl_use_fog)
			{
				...
				if (wall->seg->backsector)
				{
					wall->fogdensity = gld_CalcFogDensity(
					  wall->seg->frontsector, wall->seg->backsector->lightlevel, itemtype);
				}
...
void gld_InitDisplayLists(void)
{
	...
	if (gl_use_display_lists)
	{
		...
		if (gl_ext_arb_vertex_buffer_object) GLEXT_glBindBufferARB(GL_ARRAY_BUFFER, flats_vbo_id);
		...
		if (details_count && gl_arb_multitexture)
		{
			...
			for (i = 0; i < flats_display_list_size; i++)
			{
				...
				for (loopnum = 0; loopnum < sectorloops[i].loopcount; loopnum++)
				{
					...
					currentloop = &sectorloops[i].loops[loopnum];
					glDrawArrays(
					  currentloop->mode, currentloop->vertexindex, currentloop->vertexcount);
				}
...
void gld_DrawScene(player_t *player)
{
	...
	// e6y: skybox
	skybox = 0;
	if (gl_drawskys != skytype_none) skybox = gld_DrawBoxSkyBox();

	if (!skybox)
	{
		if (gl_drawskys == skytype_skydome) gld_DrawDomeSkyBox();
		// e6y: 3d emulation of screen quad
		if (gl_drawskys == skytype_screen) gld_DrawScreenSkybox();
	}
	...
	if (show_alive)
	{
		...
		if (step > period) step = period * 2 - step;
