...
void R_InitSkyMap(void)
{
	if (!GetMouseLook())
	{
		...
		{
			skyiscale
			  = (fixed_t)(((uint_64_t)FRACUNIT * SCREENWIDTH * 200) / (viewwidth * SCREENHEIGHT));
		}
	}
	else
	{
    ...
    //                  of the texture is at the top of the screen when looking fully up.
		...
		else if (skyheight > 200) { skytexturemid = (200 - skyheight) << FRACBITS; }
      ...
      // skyiscale = (fixed_t)((int_64_t)skyiscale * FieldOfView / 2048);
		...
		else { skytexturemid = 100 * FRACUNIT; }
	}
}
