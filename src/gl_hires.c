...
void gld_ProgressUpdate(const char *text, int progress, int total)
{
  ...
  // do not do it often
	...
	if (len > 4) V_FillRect(0, 2, SCREENHEIGHT - 3, len - 4, 2, 31);
...
typedef struct _DDRAW_H_DDPIXELFORMAT
{
	...
	union
	{
		...
		unsigned long
		  dwPrivateFormatBitCount; // Bits per pixel of private driver formats. Only valid in
	...
	union
	{
		...
		unsigned long
		  dwStencilBitDepth; // how many stencil bits (note: dwZBufferBitDepth-dwStencilBitDepth is
		                     // total Z-only bits)
	...
	union
	{
		...
		{
			unsigned short
			  wFlipMSTypes; // Multisample methods supported via flip for this D3DFORMAT
...
GLGenericImage *ReadDDSFile(const char *filename, int *bufsize, int *numMipmaps)
{
	...
	if (fp != NULL)
	{
		if ((fread(filecode, 4, 1, fp) == 1) && (strncmp(filecode, "DDS ", 4) == 0)
		    ...
		    (fread(&ddsd, sizeof(ddsd), 1, fp) == 1)) // get the surface desc
		{
			...
			if (genericImage)
			{
				...
				if (factor != -1)
				{
					...
					*bufsize = ddsd.u2.dwMipMapCount > 1 ? ddsd.u1.dwLinearSize * factor
					...
					if (fread(genericImage->pixels, 1, *bufsize, fp) > 0)
					{
						...
						genericImage->components
						  = (ddsd.u4.ddpfPixelFormat.dwFourCC == DDRAW_H_FOURCC_DXT1) ? 3 : 4;
...
static int gld_HiRes_GetExternalName(GLTexture *gltexture, char *img_path, char *dds_path)
{
	...
	static hires_path_t hires_paths[] = {
	  {doom, GLDT_TEXTURE,
	    {{"%stextures/doom/doom1/%s.%s", -1}, {"%stextures/doom/doom1/%s-ck.%s", -1},
	      {"%stextures/doom1/%s.%s", -1}, {"%stextures/doom/%s.%s", -1},
	      {"%stextures/doom/%s-ck.%s", -1}, {"%stextures/%s.%s", -1}, {"%stextures/%s-ck.%s", -1},
	      {NULL, 0}}},
	  {doom, GLDT_FLAT,
	    {{"%sflats/doom/doom1/%s.%s", -1}, {"%stextures/doom/doom1/flat-%s.%s", -1},
	      {"%stextures/doom1/flat-%s.%s", -1}, {"%sflats/doom/%s.%s", -1},
	      {"%stextures/doom/flat-%s.%s", -1}, {"%sflats/%s.%s", -1}, {"%stextures/flat-%s.%s", -1},
	      {NULL}}},
	  {doom, GLDT_PATCH,
	    {{"%spatches/doom/doom1/%s.%s", -1}, {"%spatches/doom1-ultimate/%s.%s", -1},
	      {"%spatches/doom/%s.%s", -1}, {"%spatches/%s.%s", -1}, {NULL}}},

	  {doom2, GLDT_TEXTURE,
	    {{"%stextures/doom/doom2/%s.%s", -1}, {"%stextures/doom/doom2/%s-ck.%s", -1},
	      {"%stextures/doom/%s.%s", -1}, {"%stextures/doom/%s-ck.%s", -1}, {"%stextures/%s.%s", -1},
	      {"%stextures/%s-ck.%s", -1}, {NULL}}},
	  {doom2, GLDT_FLAT,
	    {{"%sflats/doom/doom2/%s.%s", -1}, {"%stextures/doom/doom2/flat-%s.%s", -1},
	      {"%sflats/doom/%s.%s", -1}, {"%stextures/doom/flat-%s.%s", -1}, {"%sflats/%s.%s", -1},
	      {"%stextures/flat-%s.%s", -1}, {NULL}}},
	  {doom2, GLDT_PATCH,
	    {{"%spatches/doom/doom2/%s.%s", -1}, {"%spatches/doom2/%s.%s", -1},
	      {"%spatches/doom/%s.%s", -1}, {"%spatches/%s.%s", -1}, {NULL}}},

	  {pack_tnt, GLDT_TEXTURE,
	    {{"%stextures/doom/tnt/%s.%s", -1}, {"%stextures/doom/tnt/%s-ck.%s", -1},
	      {"%stextures/doom2-tnt/%s.%s", -1}, {"%stextures/doom/doom2-tnt/%s.%s", -1},
	      {"%stextures/doom/doom2-tnt/%s-ck.%s", -1}, {"%stextures/doom/%s.%s", -1},
	      {"%stextures/doom/%s-ck.%s", -1}, {"%stextures/%s.%s", -1}, {"%stextures/%s-ck.%s", -1},
	      {NULL}}},
	  {pack_tnt, GLDT_FLAT,
	    {{"%sflats/doom/tnt/%s.%s", -1}, {"%stextures/doom/tnt/flat-%s.%s", -1},
	      {"%sflats/doom/doom2-tnt/%s.%s", -1}, {"%stextures/doom/doom2-tnt/flat-%s.%s", -1},
	      {"%stextures/doom2-tnt/flat-%s.%s", -1}, {"%sflats/doom/%s.%s", -1},
	      {"%stextures/doom/flat-%s.%s", -1}, {"%sflats/%s.%s", -1}, {"%stextures/flat-%s.%s", -1},
	      {NULL}}},
	  {pack_tnt, GLDT_PATCH,
	    {{"%spatches/doom/tnt/%s.%s", -1}, {"%spatches/doom2-tnt/%s.%s", -1},
	      {"%spatches/tnt/%s.%s", -1}, {"%spatches/doom/%s.%s", -1}, {"%spatches/%s.%s", -1},
	      {NULL}}},

	  {pack_plut, GLDT_TEXTURE,
	    {{"%stextures/doom/plut/%s.%s", -1}, {"%stextures/doom/plut/%s-ck.%s", -1},
	      {"%stextures/doom2-plut/%s.%s", -1}, {"%stextures/doom/doom2-plut/%s.%s", -1},
	      {"%stextures/doom/doom2-plut/%s-ck.%s", -1}, {"%stextures/doom/%s.%s", -1},
	      {"%stextures/doom/%s-ck.%s", -1}, {"%stextures/%s.%s", -1}, {"%stextures/%s-ck.%s", -1},
	      {NULL}}},
	  {pack_plut, GLDT_FLAT,
	    {{"%sflats/doom/plut/%s.%s", -1}, {"%stextures/doom/plut/flat-%s.%s", -1},
	      {"%sflats/doom/doom2-plut/%s.%s", -1}, {"%stextures/doom/doom2-plut/flat-%s.%s", -1},
	      {"%stextures/doom2-plut/flat-%s.%s", -1}, {"%sflats/doom/%s.%s", -1},
	      {"%stextures/doom/flat-%s.%s", -1}, {"%sflats/%s.%s", -1}, {"%stextures/flat-%s.%s", -1},
	      {NULL}}},
	  {pack_plut, GLDT_PATCH,
	    {{"%spatches/doom/plut/%s.%s", -1}, {"%spatches/doom2-plut/%s.%s", -1},
	      {"%spatches/plutonia/%s.%s", -1}, {"%spatches/doom/%s.%s", -1}, {"%spatches/%s.%s", -1},
	      {NULL}}},

	  {hacx, GLDT_TEXTURE,
	    {{"%stextures/hacx/%s.%s", -1}, {"%stextures/hacx/%s-ck.%s", -1}, {"%stextures/%s.%s", -1},
	      {"%stextures/%s-ck.%s", -1}, {NULL}}},
	  {hacx, GLDT_FLAT,
	    {{"%sflats/hacx/%s.%s", -1}, {"%stextures/hacx/flat-%s.%s", -1}, {"%sflats/%s.%s", -1},
	      {"%stextures/flat-%s.%s", -1}, {NULL}}},
	  {hacx, GLDT_PATCH, {{"%spatches/hacx/%s.%s", -1}, {"%spatches/%s.%s", -1}, {NULL}}},

	  {chex, GLDT_TEXTURE,
	    {{"%stextures/chex/%s.%s", -1}, {"%stextures/chex/%s-ck.%s", -1}, {"%stextures/%s.%s", -1},
	      {"%stextures/%s-ck.%s", -1}, {NULL}}},
	  {chex, GLDT_FLAT,
	    {{"%sflats/chex/%s.%s", -1}, {"%stextures/chex/flat-%s.%s", -1}, {"%sflats/%s.%s", -1},
	      {"%stextures/flat-%s.%s", -1}, {NULL}}},
	  {chex, GLDT_PATCH, {{"%spatches/chex/%s.%s", -1}, {"%spatches/%s.%s", -1}, {NULL}}},
	...
	case GLDT_TEXTURE:
	{
		...
		if (!gl_hires_override_pwads)
		{
			for (i = 0; i < texture->patchcount; i++)
				if (lumpinfo[texture->patches[i].patch].source != source_iwad) return false;
		}
	...
	do {
		...
		if (checklist->exists == 1) // dir exists
		{
			...
			{
				doom_snprintf(
				  checkName, sizeof(checkName), checklist->path, hiresdir, texname, "dds");
				if (!M_access(checkName, F_OK)) strcpy(dds_path, checkName);
			}

			for (extp = extensions; *extp; extp++)
			{
				doom_snprintf(
				  checkName, sizeof(checkName), checklist->path, hiresdir, texname, *extp);
...
void gld_HiRes_ProcessColormap(unsigned char *buffer, int bufSize)
{
	...
	for (pos = 0; pos < bufSize; pos += 4)
	{
		...
		if (gl_hires_24bit_colormap)
			...
			color = RGB2PAL[((buffer[pos + 0] >> 3) << 10) + ((buffer[pos + 1] >> 3) << 5)
			                + (buffer[pos + 2] >> 3)];
	...
	if (1 || M_CheckParm("-" RGB2PAL_NAME))
	{
		...
		if (ok)
		{
			...
			for (r = 0; r < numcolors_per_chanel; r++)
			{
				...
				{
					int xy[256];
					for (color = 0; color < 256; color++) xy[color] = x[r][color] + y[g][color];
	...
	if (GLEXT_glCompressedTexImage2DARB)
	{
		...
		if (ddsimage)
		{
			...
			if (tex_width == ddsimage->width && tex_height == ddsimage->height)
			{
				...
				/* load the mipmaps */
				for (i = 0;
				  i < (numMipmaps ? numMipmaps : 1) && (ddsimage->width || ddsimage->height); i++)
				{
					...
					GLEXT_glCompressedTexImage2DARB(GL_TEXTURE_2D, i, ddsimage->format,
					  ddsimage->width, ddsimage->height, 0, size, ddsimage->pixels + offset);
...
static int gld_HiRes_WriteCache(GLTexture *gltexture, GLuint *texid, const char *img_path)
{
	...
	if (!result) lprintf(LO_WARN, "gld_HiRes_WriteCache: error writing '%s'.\n", cache_filename);
...
static int gld_HiRes_LoadFromFile(GLTexture *gltexture, GLuint *texid, const char *img_path)
{
	...
	if (!surf_tmp) { lprintf(LO_WARN, "gld_HiRes_LoadExternal: %s\n", SDL_GetError()); }
	else
	{
...
int gld_LoadHiresTex(GLTexture *gltexture, int cm)
{
	    ...
	    && !(gltexture->flags & GLTEXTURE_HASNOHIRES))
	{
		...
		if ((*texid == 0) || (gltexture->flags & GLTEXTURE_HIRES))
		{
			...
			if (*texid == 0 && gl_texture_internal_hires)
			{
				...
				if (lumpname)
				{
					...
					if (lump != -1)
					{
						...
						if (!surf_tmp) surf_tmp = IMG_LoadTyped_RW(rw_data, false, "TGA");
			...
			if (*texid)
			{
				...
				if (last_glTexID == gltexture->texid_p) { result = true; }
				else if (texid == gltexture->texid_p)
				{
				...
				else
				{
					...
					if (boom_cm && use_boom_cm && gl_boom_colormaps)
					{
						...
						if (*gltexture->texid_p == 0)
						{
							...
							if (gld_BuildTexture(gltexture, buf, true, w, h)) result = true;
						}
						else
						{
							gld_HiRes_Bind(gltexture, gltexture->texid_p);
	...
	if (result) { last_glTexID = gltexture->texid_p; }
	else
	{
...
{
	static const char *staticpatches[] = {"INTERPIC", // "TITLEPIC",

	  // doom's M_*
	  "M_DETAIL", "M_DISOPT", "M_DISP", "M_DOOM", "M_ENDGAM", "M_EPI1", "M_EPI2", "M_EPI3",
	  "M_EPISOD", "M_GDHIGH", "M_GDLOW", "M_HURT", "M_JKILL", "M_LGTTL", "M_LOADG", "M_LSCNTR",
	  "M_LSLEFT", "M_LSRGHT", "M_MESSG", "M_MSENS", "M_MSGOFF", "M_MSGON", "M_MUSVOL", "M_NEWG",
	  "M_NGAME", "M_NMARE", "M_OPTION", "M_OPTTTL", "M_PAUSE", "M_QUITG", "M_RDTHIS", "M_ROUGH",
	  "M_SAVEG", "M_SCRNSZ", "M_SFXVOL", "M_SGTTL", "M_SKILL", "M_SKULL1", "M_SKULL2", "M_SVOL",
	  "M_THERML", "M_THERMM", "M_THERMO", "M_THERMR", "M_ULTRA", "M_EPI4",

	  // prboom's M_*
	  "M_ACCEL", "M_AUTO", "M_BUTT1", "M_BUTT2", "M_CHAT", "M_COLORS", "M_COMPAT", "M_COMPAT",
	  "M_ENEM", "M_GENERL", "M_HORSEN", "M_KEYBND", "M_KEYBND", "M_LOKSEN", "M_MESS", "M_PALNO",
	  "M_PALSEL", "M_SETUP", "M_STAT", "M_STAT", "M_VBOX", "M_VERSEN", "M_WEAP",
