...
void R_InitVisplanesRes(void)
{
	...
	for (i = 0; i < MAXVISPLANES; i++) visplanes[i] = 0;
}
...
static void R_MapPlane(int y, int x1, int x2, draw_span_vars_t *dsvars)
{
  ...
  // killough 2/28/98: Add offsets
	...
	if (V_GetMode() != VID_MODEGL) R_DrawSpan(dsvars);
}
...
visplane_t *R_DupPlane(const visplane_t *pl, int start, int stop)
{
	...
	new_pl->minx = start;
	new_pl->maxx = stop;
	for (i = 0; i != SCREENWIDTH; i++) new_pl->top[i] = SHRT_MAX;
	return new_pl;
}
//
// R_FindPlane
...
visplane_t *R_FindPlane(fixed_t height, int picnum, int lightlevel, fixed_t xoffs, fixed_t yoffs)
{
  ...
  // New visplane algorithm uses hash table -- killough
...
#endif
	{
		...
		for (i = 0; i != SCREENWIDTH; i++) check->top[i] = SHRT_MAX;
	}
...
{
	for (; t1 < t2 && t1 <= b1; t1++) R_MapPlane(t1, spanstart[t1], x - 1, dsvars);
	for (; b1 > b2 && b1 >= t1; b1--) R_MapPlane(b1, spanstart[b1], x - 1, dsvars);
	while (t2 < t1 && t2 <= b2) spanstart[t2++] = x;
	while (b2 > b1 && b2 >= t2) spanstart[b2--] = x;
}
      ...
      // to use info lumps.
        ...
        // allow old sky textures to be used.
				...
				if (skystretch)
				{
					...
					dcvars.texturemid
					  = (int)((int_64_t)dcvars.texturemid * skyheight / SKYSTRETCH_HEIGHT);
				}
			}
			else
  ...
  // killough 10/98: Use sky scrolling offset, and possibly flip picture
			for (x = pl->minx; (dcvars.x = x) <= pl->maxx; x++)
				if ((dcvars.yl = pl->top[x]) != SHRT_MAX
				    && dcvars.yl <= (dcvars.yh = pl->bottom[x])) // dropoff overflow
				{
					dcvars.source = R_GetTextureColumn(
					  tex_patch, ((an + xtoviewangle[x]) ^ flip) >> ANGLETOSKYSHIFT);
					dcvars.prevsource = R_GetTextureColumn(
					  tex_patch, ((an + xtoviewangle[x - 1]) ^ flip) >> ANGLETOSKYSHIFT);
      ...
      // SoM 10/19/02: deep water colormap fix
			...
			for (x = pl->minx; x <= stop; x++)
				R_MakeSpans(
				  x, pl->top[x - 1], pl->bottom[x - 1], pl->top[x], pl->bottom[x], &dsvars);
...
void R_DrawPlanes(void)
{
	...
	for (i = 0; i < MAXVISPLANES; i++)
		for (pl = visplanes[i]; pl; pl = pl->next, rendered_visplanes++) R_DoDrawPlane(pl);
}
