...
static R_DrawColumn_f drawcolumnfuncs[VID_MODEMAX][RDRAW_FILTER_MAXFILTERS][RDRAW_FILTER_MAXFILTERS]
...
R_DrawColumn_f R_GetDrawColumnFunc(
  enum column_pipeline_e type, enum draw_filter_type_e filter, enum draw_filter_type_e filterz)
{
...
R_DrawSpan_f R_GetDrawSpanFunc(enum draw_filter_type_e filter, enum draw_filter_type_e filterz)
{
	...
	if (result == NULL) I_Error("R_GetDrawSpanFunc: undefined function (%d, %d)", filter, filterz);
	return result;
}
...
void R_InitBuffer(int width, int height)
{
  ...
  // Same with base row offset.
	...
	drawvars.short_topleft
	  = (unsigned short *)(screens[0].data) + viewwindowy * screens[0].short_pitch + viewwindowx;
	drawvars.int_topleft
	  = (unsigned int *)(screens[0].data) + viewwindowy * screens[0].int_pitch + viewwindowx;
	...
	if (V_GetMode() == VID_MODE8)
		for (i = 0; i < FUZZTABLE; i++) fuzzoffset[i] = fuzzoffset_org[i] * screens[0].byte_pitch;
	else if ((V_GetMode() == VID_MODE15) || (V_GetMode() == VID_MODE16))
		for (i = 0; i < FUZZTABLE; i++) fuzzoffset[i] = fuzzoffset_org[i] * screens[0].short_pitch;
	else if (V_GetMode() == VID_MODE32)
		for (i = 0; i < FUZZTABLE; i++) fuzzoffset[i] = fuzzoffset_org[i] * screens[0].int_pitch;
}
...
void R_FillBackScreen(void)
{
  ...
  // e6y: wide-res
	if (ratio_multiplier != ratio_scale || wide_offsety)
	{
		...
		if (only_stbar && ST_SCALED_OFFSETX > 0)
		{
			...
			V_FillFlat(
			  grnrock.lumpnum, 1, 0, stbar_top, ST_SCALED_OFFSETX, ST_SCALED_HEIGHT, VPT_NONE);
			V_FillFlat(grnrock.lumpnum, 1, SCREENWIDTH - ST_SCALED_OFFSETX, stbar_top,
			  ST_SCALED_OFFSETX, ST_SCALED_HEIGHT, VPT_NONE);

      // line between view and status bar
			V_FillPatch(
			  brdr_b.lumpnum, 1, 0, stbar_top, ST_SCALED_OFFSETX, brdr_b.height, VPT_NONE);
			V_FillPatch(brdr_b.lumpnum, 1, SCREENWIDTH - ST_SCALED_OFFSETX, stbar_top,
			  ST_SCALED_OFFSETX, brdr_b.height, VPT_NONE);
  ...
  // line between view and status bar
	...
	{
		V_FillPatch(brdr_b.lumpnum, 1, 0, SCREENHEIGHT - ST_SCALED_HEIGHT, SCREENWIDTH,
		  brdr_b.height, VPT_NONE);
	}

	V_FillPatch(
	  brdr_t.lumpnum, 1, viewwindowx, viewwindowy - 8, scaledviewwidth, brdr_t.height, VPT_NONE);

	V_FillPatch(brdr_b.lumpnum, 1, viewwindowx, viewwindowy + viewheight, scaledviewwidth,
	  brdr_b.height, VPT_NONE);

	V_FillPatch(
	  brdr_l.lumpnum, 1, viewwindowx - 8, viewwindowy, brdr_l.width, viewheight, VPT_NONE);

	V_FillPatch(brdr_r.lumpnum, 1, viewwindowx + scaledviewwidth, viewwindowy, brdr_r.width,
	  viewheight, VPT_NONE);
  ...
  // Draw beveled edge.
	...
	V_DrawNumPatch(
	  viewwindowx + scaledviewwidth, viewwindowy - 8, 1, brdr_tr.lumpnum, CR_DEFAULT, VPT_NONE);

	V_DrawNumPatch(
	  viewwindowx - 8, viewwindowy + viewheight, 1, brdr_bl.lumpnum, CR_DEFAULT, VPT_NONE);

	V_DrawNumPatch(viewwindowx + scaledviewwidth, viewwindowy + viewheight, 1, brdr_br.lumpnum,
	  CR_DEFAULT, VPT_NONE);
}
...
void R_DrawViewBorder(void)
{
  ...
  // e6y: wide-res
	if ((ratio_multiplier != ratio_scale || wide_offsety) && ST_SCALED_OFFSETX > 0
