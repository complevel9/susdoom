...
void P_TouchSpecialThing(mobj_t *special, mobj_t *toucher)
{
  ...
  // Can happen with a sliding player corpse.
	...
	case SPR_SOUL:
		...
		if (compatibility_level > doom_12_compatibility) sound = sfx_getpow;
		break;
...
static void P_KillMobj(mobj_t *source, mobj_t *target)
{
    ...
    !demorecording && !demoplayback)
  {
	...
	if (!((target->flags ^ MF_COUNTKILL) & (MF_FRIEND | MF_COUNTKILL))) totallive--;
...
#ifdef GL_DOOM
	if (target->momx == 0 && target->momy == 0) target->flags |= MF_FOREGROUND;
#endif
}
...
void P_DamageMobj(mobj_t *target, mobj_t *inflictor, mobj_t *source, int damage)
{
  ...
  /* killough 8/31/98: allow bouncers to take damage */
	if (!(target->flags & (MF_SHOOTABLE | MF_BOUNCES))) return; // shouldn't happen...
	...
	if (player && gameskill == sk_baby) damage >>= 1;   // take half damage in trainer mode
  ...
  // thus kick away unless using the chainsaw.
	    ...
	    && (!source || !source->player || source->player->readyweapon != wp_chainsaw))
	{
      ...
      /* killough 11/98: thrust objects hanging off ledges */
		if (target->intflags & MIF_FALLING && target->gear >= MAXGEAR) target->gear = 0;
	}
  ...
  // player specific
	...
	if (source && target) CheckGivenDamageTracer(source, damage);
  ...
  // e6y: Monsters could commit suicide in Doom v1.2 if they damaged themselves by exploding a
  // barrel
	if (source && (source != target || compatibility_level == doom_12_compatibility)
	    && source->type != MT_VILE && (!target->threshold || target->type == MT_VILE)
	    && ((source->flags ^ target->flags) & MF_FRIEND || monster_infighting || !mbf_features))
	{
      /* if not intent on another player, chase after this one
		...
		if (!target->lastenemy || target->lastenemy->health <= 0
		    || (!mbf_features ? !target->lastenemy->player
