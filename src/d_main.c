  ...
  // Allow only sensible keys during skipping
	if (doSkip)
	{
        ...
        // key_use is used for seeing the current frame
				if (ev->data1 != key_use && ev->data1 != key_demo_skip) return;
			}
		}
	}

	M_Responder(ev)
	  || (gamestate == GS_LEVEL && (HU_Responder(ev) || ST_Responder(ev) || AM_Responder(ev)))
	  || G_Responder(ev);
}
...
static void D_Wipe(void)
{
	...
	do {
		...
		if (capturing_video && !doSkip && cap_wipescreen) I_CaptureFrame();
	} while (!done);
}
...
void D_Display(fixed_t frac)
{
  ...
  // e6y
	...
	if (doSkip)
	{
		if (HU_DrawDemoProgress(false)) I_FinishUpdate();
		if (!gamekeydown[key_use]) return;
  ...
  // save the current screen if about to wipe
    ...
    // Work out if the player view is visible, and if there is a border
      ...
      // not only if viewactive is true
		...
		if (redrawborderstuff || (V_GetMode() == VID_MODEGL)) R_DrawViewBorder();
    ...
    // Now do the drawing
		if (viewactive || map_always_updates) R_RenderPlayerView(&players[displayplayer]);
    ...
    // but should NOT be applied for automap, statusbar and HUD
		...
		if (automapmode & am_active) AM_Drawer();
		...
		if (V_GetMode() != VID_MODEGL) R_DrawViewBorder();
		HU_Drawer();

#ifdef GL_DOOM
		if (V_GetMode() == VID_MODEGL) gld_ProcessExtraAlpha();
#endif
	}
  ...
  // draw pause pic
    ...
    // Simplified the "logic" here and no need for x-coord caching - POPE
		V_DrawNamePatch(
		  (320 - V_NamePatchWidth("M_PAUSE")) / 2, 4, 0, "M_PAUSE", CR_DEFAULT, VPT_STRETCH);
	}
  ...
  // Don't thrash cpu during pausing or if the window doesnt have focus
	if ((paused && !walkcamera.type) || (!window_focused)) I_uSleep(5000);
...
{
	if (quickstart_window_ms > 0) I_uSleep(quickstart_window_ms * 1000);
  ...
  // There can only be one iwad source!
	if (source == source_iwad)
	{
		...
		for (i = 0; i < numwadfiles; ++i)
			if (wadfiles[i].src == source_iwad) wadfiles[i].src = source_skip;
	}
      ...
      // read IWAD header
        ...
        // scan directory for levelname lumps
				while (length--)
				{
					...
					if (!strncmp(fileinfo[length].name, "DMENUPIC", 8)) bfgedition++;
					if (!strncmp(fileinfo[length].name, "HACX", 4)) hx++;
				}
				free(fileinfo);
			}
...
void AddIWAD(const char *iwad)
{
  ...
  /* jff 8/23/98 set gamemission global appropriately in all cases
	...
	case shareware:
		gamemission = doom;
		if (i >= 8 && !strnicmp(iwad + i - 8, "chex.wad", 8)) gamemission = chex;
		break;
	case commercial:
		gamemission = doom2;
...
static char *FindIWADFile(void)
{
	...
	if (i && (++i < myargc))
		...
		for (i = 0; !iwad && i < nstandard_iwads; i++) iwad = I_FindFile(standard_iwads[i], ".wad");
	return iwad;
}
...
static void DoLooseFiles(void)
{
	...
	struct
	{
		...
		char ***list;
		int *count;
	} looses[] = {{".wad", &wads, &wadcount}, {".lmp", &lmps, &lmpcount},
	  {".deh", &dehs, &dehcount}, {".bex", &dehs, &dehcount},
	...
	struct
	{
		...
		char ***list;
		int *count;
	} params[] = {{"-file", &wads, &wadcount}, {"-deh", &dehs, &dehcount},
	  {"-playdemo", &lmps, &lmpcount}, {0}};
	...
	for (i = 0; i < myargc; i++) skip[i] = false;
...
static void L_SetupConsoleMasks(void)
{
	int p;
	int i;
	const char *cena = "ICWEFDA",
	           *pos;  // jff 9/3/98 use this for parsing console masks // CPhipps - const char*'s
...
static char *GetAutoloadBaseDir(unsigned int iter)
{
	...
	if (M_CheckParm("-noload")) return NULL;
...
const char *BaseName(const char *filename)
{
	...
	while (basename > filename && *basename != '/' && *basename != '\\') basename--;
...
const char *IWADBaseName(void)
{
	...
	for (i = 0; i < numwadfiles; i++)
		if (wadfiles[i].src == source_iwad) break;

	if (i == numwadfiles) I_Error("IWADBaseName: IWAD not found\n");
  ...
  // e6y: DEH files preloaded in wrong order
	...
	// Ty 08/29/98 - moved this check later so we can have -warp alone: && p < myargc-1)
	{
		...
		if (gamemode == commercial)
		{
			...
			{
				int map;
				if (sscanf(myargv[p + 1], "%d", &map) == 1) startmap = map;
				warpmap = startmap;
			}
		}
			...
			if (p < myargc - 1)
			{
				...
				if (sscanf(myargv[p + 1], "%d", &episode) == 1)
				{
					...
					startmap = 1;
					if (p < myargc - 2 && sscanf(myargv[p + 2], "%d", &map) == 1) startmap = map;
	...
	if ((p = M_CheckParm("-file")))
	{
		...
		while (++p != myargc && *myargv[p] != '-')
		{
			...
			if (!file && D_TryGetWad(myargv[p])) file = I_FindFile(myargv[p], ".wad");
			if (file)
			{
	...
	if (p && p < myargc - 1)
	{
		...
		if ((p = M_CheckParm("-ffmap")) && p < myargc - 1) ffmap = atoi(myargv[p + 1]);
		free(file);
	}
	...
	// e6y
	{
		...
		if (!demo_footer) demo_footer = CheckAutoDemo();
#ifdef USE_WINDOWS_LAUNCHER
	...
	if (!M_CheckParm("-nodeh"))
	{
		...
		if (bfgedition)
		{
			...
			if (lump != -1) ProcessDehFile(NULL, D_dehout(), lump);
		}
	...
	if (p)
	{
		...
		while (++p != myargc && *myargv[p] != '-')
		{
			...
			else { I_Error("D_DoomMainSetup: Cannot find .deh or .bex file named %s", myargv[p]); }
		}
	}
	...
	if (!(M_CheckParm("-nodraw") && M_CheckParm("-nosound"))) I_InitGraphics();

	// NSM
	if ((p = M_CheckParm("-viddump")) && (p < myargc - 1)) I_CapturePrep(myargv[p + 1]);
	...
	if ((p = M_CheckParm("-checksum")) && ++p < myargc) P_RecordChecksum(myargv[p]);
	...
	{ /* killough 12/98 */
		if (autostart || netgame)
		{
			...
			if (autostart) GetFirstMap(&startepisode, &startmap);
