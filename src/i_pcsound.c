...
static dboolean CachePCSLump(int sound_id)
{
	...
	if (current_sound_lump[0] != 0x00 || current_sound_lump[1] != 0x00) return false;

	headerlen = (current_sound_lump[3] << 8) | current_sound_lump[2];

	if (headerlen > lumplen - 4) return false;
...
int I_PCS_StartSound(int id, int channel, int vol, int sep, int pitch, int priority)
{
	int result;

	if (!pcs_initialised) return -1;
	...
	if (id == sfx_posact || id == sfx_bgact || id == sfx_dmact || id == sfx_dmpain
	    || id == sfx_popain || id == sfx_sawidl)
	{
		return -1;
	}

	if (SDL_LockMutex(sound_lock) < 0) return -1;

	result = CachePCSLump(id);

	if (result) current_sound_handle = channel;
...
{
	if (!pcs_initialised) return;

	if (SDL_LockMutex(sound_lock) < 0) return;

	// If this is the channel currently playing, immediately end it.

	if (current_sound_handle == handle) current_sound_remaining = 0;
...
{
	if (!pcs_initialised) return false;

	if (handle != current_sound_handle) return false;
