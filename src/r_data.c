  ...
  // clean up malloc-ing to use sizeof
	...
	for (i = 0; i < numtextures; i++, directory++)
	{
		...
		texture = textures[i] = Z_Malloc(
		  sizeof(texture_t) + sizeof(texpatch_t) * (LittleShort(mtexture->patchcount) - 1),
		  PU_STATIC, 0);
...
/*
		...
		*/
		{
			size_t j;
			for (j = 0; j < sizeof(texture->name); j++) texture->name[j] = mtexture->name[j];
		}
/* #endif */
...
int R_ColormapNumForName(const char *name)
{
	...
	if (strncasecmp(name, "COLORMAP", 8))     // COLORMAP predefined to return 0
		if ((i = (W_CheckNumForName)(name, ns_colormaps)) != -1) i -= firstcolormaplump;
	return i;
}
  ...
  // If a tranlucency filter map lump is present, use it
	...
	{   // Compose a default transparent filter map based on PLAYPAL.
      ...
      // Use cached translucency filter if it's available
		...
		if (!cachefp || fread(&cache, 1, sizeof cache, cachefp) != sizeof cache
		    || cache.pct != tran_filter_pct || memcmp(cache.playpal, playpal, sizeof cache.playpal)
...
int R_FlatNumForName(const char *name)    // killough -- const added
{
    ...
    // Unknown flats will be replaced with "NO TEXTURE" preset from prboom-plus.wad
		...
		if (i == -1) I_Error("R_FlatNumForName: -N0_TEX- not found");
	}
	return i - firstflat;
}
...
int PUREFUNC R_CheckTextureNumForName(const char *name)
{
	...
	if (*name != '-')     // "NoTexture" marker.
	{
		...
		while (i >= 0 && strncasecmp(textures[i]->name, name, 8)) i = textures[i]->next;
	}
	return i;
}
