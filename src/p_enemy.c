...
{
	if (target != NULL && target->player && (target->player->cheats & CF_NOTARGET)) return;
...
static dboolean P_HitFriend(mobj_t *actor)
{
	return actor->flags & MF_FRIEND && actor->target
	    && (P_AimLineAttack(actor,
	          ...
	          P_AproxDistance(actor->x - actor->target->x, actor->y - actor->target->y), 0),
	      linetarget)
	    && linetarget != actor->target && !((linetarget->flags ^ actor->flags) & MF_FRIEND);
}
...
static dboolean P_CheckMissileRange(mobj_t *actor)
{
  ...
  /* killough 7/18/98: friendly monsters don't attack other friendly
	 ...
	 */
	if (actor->flags & actor->target->flags & MF_FRIEND) return false;
  ...
  // OPTIMIZE: get this from a global checksight
	...
	if (P_Random(pr_missrange) < dist) return false;

	if (P_HitFriend(actor)) return false;
...
static dboolean P_IsOnLift(const mobj_t *actor)
{
  ...
  // Short-circuit: it's on a lift which is active.
	if (sec->floordata && ((thinker_t *)sec->floordata)->function == T_PlatRaise) return true;

  // Check to see if it's in a sector which can be activated as a lift.
	if ((line.tag = sec->tag))
		for (l = -1; (l = P_FindLineFromLineTag(&line, l)) >= 0;) switch (lines[l].special)
			{
			case 10:
			case 14:
			case 15:
...
static dboolean P_Move(mobj_t *actor, dboolean dropoff) /* killough 9/12/98 */
{
...
#ifdef RANGECHECK
	if ((unsigned)actor->movedir >= 8) I_Error("P_Move: Weird actor->movedir!");
#endif

  // killough 10/98: make monsters get affected by ice and sludge too:

	if (monster_friction) movefactor = P_GetMoveFactor(actor, &friction);
  ...
  /* killough 11/98: fall more slowly, under gravity, if felldown==true */
	if (!(actor->flags & MF_FLOAT) && (!felldown || !mbf_features)) actor->z = actor->floorz;
...
static dboolean P_SmartMove(mobj_t *actor)
{
  ...
  // haleyjd: allow all friends of HelperType to also jump down
	...
	if (!P_Move(actor, dropoff)) return false;
...
{
	if (!P_SmartMove(actor)) return false;
...
static void P_DoNewChaseDir(mobj_t *actor, fixed_t deltax, fixed_t deltay)
{
  ...
  // there is no direct path to the player, so pick another direction.
	if (olddir != DI_NODIR && (actor->movedir = olddir, P_TryWalk(actor))) return;

  // randomly determine direction of search
	if (P_Random(pr_newchasedir) & 1)
	{
		for (tdir = DI_EAST; tdir <= DI_SOUTHEAST; tdir++)
			if (tdir != turnaround && (actor->movedir = tdir, P_TryWalk(actor))) return;
	}
	else
		for (tdir = DI_SOUTHEAST; tdir != DI_EAST - 1; tdir--)
			if (tdir != turnaround && (actor->movedir = tdir, P_TryWalk(actor))) return;

	if ((actor->movedir = turnaround) != DI_NODIR && !P_TryWalk(actor)) actor->movedir = DI_NODIR;
}
...
static dboolean PIT_AvoidDropoff(line_t *line)
{
	if (line->backsector && // Ignore one-sided linedefs
	    tmbbox[BOXRIGHT] > line->bbox[BOXLEFT] && tmbbox[BOXLEFT] < line->bbox[BOXRIGHT]
...
static void P_NewChaseDir(mobj_t *actor)
{
  ...
  // 2) Take advantage over an enemy without missiles, by keeping distance
	...
	if (mbf_features)
	{
		if (actor->floorz - actor->dropoffz > FRACUNIT * 24 && actor->z <= actor->floorz
		    && !(actor->flags & (MF_DROPOFF | MF_FLOAT)) && !comp[comp_dropoff]
  ...
  // in certain situations (e.g. a crowded lift)
			...
			if (actor->flags & target->flags & MF_FRIEND && distfriend << FRACBITS > dist
			...
			{   // Live enemy target
				if (monster_backing && actor->info->missilestate && actor->type != MT_SKULL
  ...
  // logic still works the same, except in the strafing part
	...
	if (actor->strafecount) actor->movecount = actor->strafecount;
}
  ...
  // with a healthy friend, don't attack around 60% the time
	{
		...
		if (targ && targ->target == mo && P_Random(pr_skiptarget) > 100
		    && (targ->flags ^ mo->flags) & MF_FRIEND && targ->health * 2 >= targ->info->spawnhealth)
			return true;
	}

	if (!P_IsVisible(actor, mo, current_allaround)) return true;
...
static dboolean P_LookForPlayers(mobj_t *actor, dboolean allaround)
{
  ...
  // Change mask of 3 to (MAXPLAYERS-1) -- killough 2/15/98:
	...
	stopc = !mbf_features && !demo_compatibility && monsters_remember ? MAXPLAYERS
	                                                                  : 2; // killough 9/9/98
	...
	for (;; actor->lastlook = (actor->lastlook + 1) & (MAXPLAYERS - 1))
	{
		...
		if (player->cheats & CF_NOTARGET) continue; // no target

		if (player->health <= 0) continue; // dead

		if (!P_IsVisible(actor, player->mo, allaround)) continue;
		 ...
		 */
		if (!comp[comp_pursuit]) actor->threshold = 60;
...
static dboolean P_LookForMonsters(mobj_t *actor, dboolean allaround)
{
	...
	if (demo_compatibility) return false;
	...
	if (cap->cnext != cap) // Empty list? bail out early
	{
		...
		if (!P_BlockThingsIterator(x, y, PIT_FindTarget)) return true;
...
static dboolean P_HelpFriend(mobj_t *actor)
{
	...
	if (actor->health * 3 < actor->info->spawnhealth) return false;
	...
	for (th = cap->cnext; th != cap; th = th->cnext)
		...
		{
			if (P_Random(pr_helpfriend) < 180) break;
		}
		else if (((mobj_t *)th)->flags & MF_JUSTHIT && ((mobj_t *)th)->target
...
void A_Look(mobj_t *actor)
{
	...
	if (targ && targ->player && (targ->player->cheats & CF_NOTARGET)) return;
	...
	if (!(actor->flags & MF_FRIEND && P_LookForTargets(actor, false))
	    && !((targ = actor->subsector->sector->soundtarget) && targ->flags & MF_SHOOTABLE
	...
	if (actor->info->seesound)
	{
		...
		else
		{
			...
			if (full_sounds) S_UnlinkSound(actor);
		}
	}
	P_SetMobjState(actor, actor->info->seestate);
}
...
static void A_KeepChasing(mobj_t *actor)
{
	...
	{
		actor->movecount--;
		if (actor->strafecount) actor->strafecount--;
		P_SmartMove(actor);
	}
}
...
void A_Chase(mobj_t *actor)
{
	...
	if (actor->info->meleestate && P_CheckMeleeRange(actor))
	{
		 ...
		 * cph - DEMOSYNC? */
		if (!actor->info->missilestate) actor->flags |= MF_JUSTHIT;
		return;
	}
	...
	if (--actor->movecount < 0 || !P_SmartMove(actor)) P_NewChaseDir(actor);
...
void A_CPosRefire(mobj_t *actor)
{
	...
	if (P_HitFriend(actor)) goto stop;
...
void A_SpidRefire(mobj_t *actor)
{
	...
	if (P_Random(pr_spidrefire) < 10) return;

	// killough 11/98: prevent refiring on friends continuously
	if (!actor->target || actor->target->health <= 0
	    || actor->flags & actor->target->flags & MF_FRIEND || !P_CheckSight(actor, actor->target))
stop:
		P_SetMobjState(actor, actor->info->seestate);
}
...
static void A_PainShootSkull(mobj_t *actor, angle_t angle)
{
	...
	else //   V
	{
		...
		if (Check_Sides(actor, x, y)) return;
...
void A_BossDeath(mobj_t *mo)
{
	...
	if (gamemapinfo && gamemapinfo->numbossactions != 0)
	{
		...
		for (i = 0; i < gamemapinfo->numbossactions; i++)
			if (gamemapinfo->bossactions[i].type == mo->type) break;
		if (i >= gamemapinfo->numbossactions) return; // no matches found
		...
		for (i = 0; i < gamemapinfo->numbossactions; i++)
		{
			if (gamemapinfo->bossactions[i].type == mo->type)
			{
				...
				if (!P_UseSpecialLine(mo, &junk, 0, true)) P_CrossSpecialLine(&junk, 0, mo, true);
			}
		}
	...
	else
	{
		...
		if (comp[comp_666] && gameepisode < 4)
		{
			...
			if (gamemap != 8) return;
			if (mo->type == MT_BRUISER && gameepisode != 1) return;
		}
		else
		{
			switch (gameepisode)
			...
			{ // killough 2/7/98: remove limit on icon landings:
				if (numbraintargets >= numbraintargets_alloc)
					braintargets = realloc(
					  braintargets, (numbraintargets_alloc
					                  = numbraintargets_alloc ? numbraintargets_alloc * 2 : 32)
					                  * sizeof *braintargets);
...
void A_BrainSpit(mobj_t *mo)
{
	...
	if (gameskill <= sk_easy && !brain.easy) return;
...
void A_RandomJump(mobj_t *mo)
{
	...
	if (P_Random(pr_randomjump) < mo->state->misc2) P_SetMobjState(mo, mo->state->misc1);
}
...
void A_LineEffect(mobj_t *mo)
{
	...
	if (!junk.special) return;
	junk.tag = (short)mo->state->misc2;
	if (!P_UseSpecialLine(mo, &junk, 0, false)) P_CrossSpecialLine(&junk, 0, mo, false);
