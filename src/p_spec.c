...
void P_InitPicAnims(void)
{
  ...
  // jff 3/23/98 read from predefined or wad lump instead of table
    ...
    // 1/11/98 killough -- removed limit by array-doubling
		...
		{
      // different episode ?
			if (R_CheckTextureNumForName(animdefs[i].startname) == -1) continue;
...
fixed_t P_FindNextHighestFloor(sector_t *sec, int currentheight)
{
  ...
  // Adapted for prboom's complevels
    ...
    // 20 adjoining sectors max!
		if (!MAX_ADJOINING_SECTORS) MAX_ADJOINING_SECTORS = M_CheckParm("-doom95") ? 500 : 20;
        ...
        // 21: can be emulated;
				...
				{
					lprintf(LO_WARN,
					  "P_FindNextHighestFloor: Overflow of heightlist[%d] array is detected.\n",
					  MAX_ADJOINING_SECTORS);
					lprintf(LO_WARN, " Sector %d, line %d, heightlist index %d: ", sec->iSectorID,
					  sec->lines[i]->iLineID, h);
...
sector_t *P_FindModelFloorSector(fixed_t floordestheight, int secnum)
{
	...
	for (i = 0; i < (demo_compatibility && sec->linecount < linecount ? sec->linecount : linecount);
	  i++)
	{
		if (twoSided(secnum, i))
		{
			...
			if (sec->floorheight == floordestheight) return sec;
		}
	}
	return NULL;
}
...
sector_t *P_FindModelCeilingSector(fixed_t ceildestheight, int secnum)
{
	  ...
	  i++)
	{
		if (twoSided(secnum, i))
		{
			...
			if (sec->ceilingheight == ceildestheight) return sec;
		}
	}
	return NULL;
}
...
int P_FindSectorFromLineTag(const line_t *line, int start)
{
	...
	while (start >= 0 && sectors[start].tag != line->tag) start = sectors[start].nexttag;
	return start;
}
...
int P_FindLineFromLineTag(const line_t *line, int start)
{
	...
	while (start >= 0 && lines[start].tag != line->tag) start = lines[start].nexttag;
	return start;
}
...
dboolean P_CanUnlockGenDoor(line_t *line, player_t *player)
{
	...
	{
	case AnyKey:
		if (!player->cards[it_redcard] && !player->cards[it_redskull] && !player->cards[it_bluecard]
		    && !player->cards[it_blueskull] && !player->cards[it_yellowcard]
	...
	case YCard:
		...
		{
			player->message
			  = skulliscard ? s_PD_YELLOWK : s_PD_YELLOWC; // Ty 03/27/98 - externalized
			S_StartSound(player->mo, sfx_oof); // killough 3/20/98
			return false;
		}
		break;
	...
	case YSkull:
		...
		{
			player->message
			  = skulliscard ? s_PD_YELLOWK : s_PD_YELLOWS; // Ty 03/27/98 - externalized
			S_StartSound(player->mo, sfx_oof); // killough 3/20/98
			return false;
		}
		break;
...
void P_CrossSpecialLine(line_t *line, int side, mobj_t *thing, dboolean bossaction)
{
	...
	{
		if (line->special > 98 && line->special != 104) return;
	}
	else if (!thing->player && !bossaction)
	{
	...
	if (!demo_compatibility) // generalized types not recognized if old demo
	{
		...
		else if ((unsigned)line->special >= GenDoorBase)
		{
			if (!thing->player && !bossaction)
			{
				if (!(line->special & DoorMonster)) return; // monsters disallowed from this door
		...
		else if ((unsigned)line->special >= GenLockedBase)
		{
			...
			{ // jff 4/1/98 check for being a walk type before reporting door type
				if (!P_CanUnlockGenDoor(line, thing->player)) return;
			}
			else
				return;
		...
		{
			if (!thing->player && !bossaction)
				if (!(line->special & LiftMonster)) return; // monsters disallowed
			if (!comperr(comperr_zerotag)
			    && !line->tag) // e6y //jff 2/27/98 all walk generalized types require tag
				return;
		...
		{
			if (!thing->player && !bossaction)
				if (!(line->special & StairMonster)) return; // monsters disallowed
			if (!comperr(comperr_zerotag)
			    && !line->tag) // e6y //jff 2/27/98 all walk generalized types require tag
				return;
		...
		if (linefunc) // if it was a valid generalized type
			...
			{
			case WalkOnce:
				if (linefunc(line)) line->special = 0; // clear special if a walk once type
				return;
			case WalkMany:
				linefunc(line);
				return;
	...
	case 2:
		// Open Door
		if (EV_DoDoor(line, openDoor) || demo_compatibility) line->special = 0;
		break;

	case 3:
		// Close Door
		if (EV_DoDoor(line, closeDoor) || demo_compatibility) line->special = 0;
		break;

	case 4:
		// Raise Door
		if (EV_DoDoor(line, normal) || demo_compatibility) line->special = 0;
		break;

	case 5:
		// Raise Floor
		if (EV_DoFloor(line, raiseFloor) || demo_compatibility) line->special = 0;
		break;

	case 6:
		// Fast Ceiling Crush & Raise
		if (EV_DoCeiling(line, fastCrushAndRaise) || demo_compatibility) line->special = 0;
		break;

	case 8:
		// Build Stairs
		if (EV_BuildStairs(line, build8) || demo_compatibility) line->special = 0;
		break;

	case 10:
		// PlatDownWaitUp
		if (EV_DoPlat(line, downWaitUpStay, 0) || demo_compatibility) line->special = 0;
		break;

	case 12:
		// Light Turn On - brightest near
		if (EV_LightTurnOn(line, 0) || demo_compatibility) line->special = 0;
		break;

	case 13:
		// Light Turn On 255
		if (EV_LightTurnOn(line, 255) || demo_compatibility) line->special = 0;
		break;

	case 16:
		// Close Door 30
		if (EV_DoDoor(line, close30ThenOpen) || demo_compatibility) line->special = 0;
		break;

	case 17:
		// Start Light Strobing
		if (EV_StartLightStrobing(line) || demo_compatibility) line->special = 0;
		break;

	case 19:
		// Lower Floor
		if (EV_DoFloor(line, lowerFloor) || demo_compatibility) line->special = 0;
		break;

	case 22:
		// Raise floor to nearest height and change texture
		if (EV_DoPlat(line, raiseToNearestAndChange, 0) || demo_compatibility) line->special = 0;
		break;

	case 25:
		// Ceiling Crush and Raise
		if (EV_DoCeiling(line, crushAndRaise) || demo_compatibility) line->special = 0;
		break;

	case 30:
		// Raise floor to shortest texture height
		//  on either side of lines.
		if (EV_DoFloor(line, raiseToTexture) || demo_compatibility) line->special = 0;
		break;

	case 35:
		// Lights Very Dark
		if (EV_LightTurnOn(line, 35) || demo_compatibility) line->special = 0;
		break;

	case 36:
		// Lower Floor (TURBO)
		if (EV_DoFloor(line, turboLower) || demo_compatibility) line->special = 0;
		break;

	case 37:
		// LowerAndChange
		if (EV_DoFloor(line, lowerAndChange) || demo_compatibility) line->special = 0;
		break;

	case 38:
		// Lower Floor To Lowest
		if (EV_DoFloor(line, lowerFloorToLowest) || demo_compatibility) line->special = 0;
		break;

	case 39:
		// TELEPORT! //jff 02/09/98 fix using up with wrong side crossing
		if (EV_Teleport(line, side, thing) || demo_compatibility) line->special = 0;
		break;
	...
	case 44:
		// Ceiling Crush
		if (EV_DoCeiling(line, lowerAndCrush) || demo_compatibility) line->special = 0;
		break;
	...
	case 53:
		...
		if (EV_DoPlat(line, perpetualRaise, 0) || demo_compatibility) line->special = 0;
		break;

	case 54:
		// Platform Stop
		if (EV_StopPlat(line) || demo_compatibility) line->special = 0;
		break;

	case 56:
		// Raise Floor Crush
		if (EV_DoFloor(line, raiseFloorCrush) || demo_compatibility) line->special = 0;
		break;

	case 57:
		// Ceiling Crush Stop
		if (EV_CeilingCrushStop(line) || demo_compatibility) line->special = 0;
		break;

	case 58:
		// Raise Floor 24
		if (EV_DoFloor(line, raiseFloor24) || demo_compatibility) line->special = 0;
		break;

	case 59:
		// Raise Floor 24 And Change
		if (EV_DoFloor(line, raiseFloor24AndChange) || demo_compatibility) line->special = 0;
		break;

	case 100:
		// Build Stairs Turbo 16
		if (EV_BuildStairs(line, turbo16) || demo_compatibility) line->special = 0;
		break;

	case 104:
		// Turn lights off in sector(tag)
		if (EV_TurnTagLightsOff(line) || demo_compatibility) line->special = 0;
		break;

	case 108:
		// Blazing Door Raise (faster than TURBO!)
		if (EV_DoDoor(line, blazeRaise) || demo_compatibility) line->special = 0;
		break;

	case 109:
		// Blazing Door Open (faster than TURBO!)
		if (EV_DoDoor(line, blazeOpen) || demo_compatibility) line->special = 0;
		break;

	case 110:
		// Blazing Door Close (faster than TURBO!)
		if (EV_DoDoor(line, blazeClose) || demo_compatibility) line->special = 0;
		break;

	case 119:
		// Raise floor to nearest surr. floor
		if (EV_DoFloor(line, raiseFloorToNearest) || demo_compatibility) line->special = 0;
		break;

	case 121:
		// Blazing PlatDownWaitUpStay
		if (EV_DoPlat(line, blazeDWUS, 0) || demo_compatibility) line->special = 0;
		break;
	...
	case 125:
		...
		if (!thing->player && (EV_Teleport(line, side, thing) || demo_compatibility))
			line->special = 0;
		break;

	case 130:
		// Raise Floor Turbo
		if (EV_DoFloor(line, raiseFloorTurbo) || demo_compatibility) line->special = 0;
		break;

	case 141:
		// Silent Ceiling Crush & Raise
		if (EV_DoCeiling(line, silentCrushAndRaise) || demo_compatibility) line->special = 0;
		break;
	...
	default:
		if (!demo_compatibility) switch (line->special)
			{
			...
			case 142:
				...
				if (EV_DoFloor(line, raiseFloor512)) line->special = 0;
				break;

			case 143:
				// Raise Floor 24 and change
				// 143 W1  EV_DoPlat(raiseAndChange,24)
				if (EV_DoPlat(line, raiseAndChange, 24)) line->special = 0;
				break;

			case 144:
				// Raise Floor 32 and change
				// 144 W1  EV_DoPlat(raiseAndChange,32)
				if (EV_DoPlat(line, raiseAndChange, 32)) line->special = 0;
				break;

			case 145:
				// Lower Ceiling to Floor
				// 145 W1  EV_DoCeiling(lowerToFloor)
				if (EV_DoCeiling(line, lowerToFloor)) line->special = 0;
				break;

			case 146:
				// Lower Pillar, Raise Donut
				// 146 W1  EV_DoDonut()
				if (EV_DoDonut(line)) line->special = 0;
				break;

			case 199:
				// Lower ceiling to lowest surrounding ceiling
				// 199 W1 EV_DoCeiling(lowerToLowest)
				if (EV_DoCeiling(line, lowerToLowest)) line->special = 0;
				break;

			case 200:
				// Lower ceiling to highest surrounding floor
				// 200 W1 EV_DoCeiling(lowerToMaxFloor)
				if (EV_DoCeiling(line, lowerToMaxFloor)) line->special = 0;
				break;
			...
			case 153: // jff 3/15/98 create texture change no motion type
				...
				if (EV_DoChange(line, trigChangeOnly)) line->special = 0;
				break;

			case 239: // jff 3/15/98 create texture change no motion type
				// Texture/Type Change Only (Numeric)
				// 239 W1 Change Texture/Type Only
				if (EV_DoChange(line, numChangeOnly)) line->special = 0;
				break;

			case 219:
				// Lower floor to next lower neighbor
				// 219 W1 Lower Floor Next Lower Neighbor
				if (EV_DoFloor(line, lowerFloorToNearest)) line->special = 0;
				break;

			case 227:
				// Raise elevator next floor
				// 227 W1 Raise Elevator next floor
				if (EV_DoElevator(line, elevateUp)) line->special = 0;
				break;

			case 231:
				// Lower elevator next floor
				// 231 W1 Lower Elevator next floor
				if (EV_DoElevator(line, elevateDown)) line->special = 0;
				break;

			case 235:
				// Elevator to current floor
				// 235 W1 Elevator to current floor
				if (EV_DoElevator(line, elevateCurrent)) line->special = 0;
				break;
			...
			case 268: // jff 4/14/98 add monster-only silent
				if (!thing->player && EV_SilentTeleport(line, side, thing)) line->special = 0;
				break;
...
void P_ShootSpecialLine(mobj_t *thing, line_t *line)
{
	...
	if (!demo_compatibility)
	{
		...
		else if ((unsigned)line->special >= GenDoorBase)
		{
			...
			if (!comperr(comperr_zerotag)
			    && !line->tag) // e6y //jff 3/2/98 all gun generalized types require tag
				return;
			linefunc = EV_DoGenDoor;
		}
		else if ((unsigned)line->special >= GenLockedBase)
		{
			if (!thing->player) return; // monsters disallowed from unlocking doors
		...
		if (linefunc) switch ((line->special & TriggerType) >> TriggerTypeShift)
			{
			case GunOnce:
				if (linefunc(line)) P_ChangeSwitchTexture(line, 0);
				return;
			case GunMany:
	...
	case 24:
		...
		if (EV_DoFloor(line, raiseFloor) || demo_compatibility) P_ChangeSwitchTexture(line, 0);
		break;
	...
	default:
		if (!demo_compatibility) switch (line->special)
			...
			case 197:
				// Exit to next level
				// killough 10/98: prevent zombies from exiting levels
				if (thing->player && thing->player->health <= 0 && !comp[comp_zombie]) break;
				P_ChangeSwitchTexture(line, 0);
				G_ExitLevel();
				break;
	...
	if (sector->special < 32) // regular sector specials
	{
		...
		case 9:
			...
			if (hudadd_secretarea)
			{
				...
				SetCustomMessage(
				  player - players, STSTR_SECRETFOUND, 0, 2 * TICRATE, CR_GOLD, sfx_id);
			}
...
void P_SpawnSpecials(void)
{
	...
	if (comp_skytransfers || !demo_compatibility)
		for (i = 0; i < numlines; i++) switch (lines[i].special)
			{
				int s;
				// killough 10/98:
				//
...
static void Add_WallScroller(fixed_t dx, fixed_t dy, const line_t *l, int control, int accel)
{
	...
	{
		x = (fixed_t)(((int_64_t)dy * -(int_64_t)l->dy - (int_64_t)dx * (int_64_t)l->dx)
		              / (int_64_t)d); // killough 10/98:
		y = (fixed_t)(((int_64_t)dy * (int_64_t)l->dx - (int_64_t)dx * (int_64_t)l->dy)
		...
		case 254:
			...
			else
			{
				for (s = -1; (s = P_FindLineFromLineTag(l, s)) >= 0;)
					if (s != i) Add_WallScroller(dx, dy, lines + s, control, accel);
			}
			break;
...
static dboolean PIT_PushThing(mobj_t *thing)
{
	      ...
	      : (sentient(thing) || thing->flags & MF_SHOOTABLE) && !(thing->flags & MF_NOCLIP))
	{
		...
		if (speed > 0 && P_CheckSight(thing, tmpusher->source))
		{
			...
			if (tmpusher->source->type == MT_PUSH) pushangle += ANG180; // away
...
void T_Pusher(pusher_t *p)
{
	...
	if (!(sec->special & PUSH_MASK)) return;
	...
	if (p->type == p_push)
	{
		...
		for (bx = xl; bx <= xh; bx++)
			for (by = yl; by <= yh; by++) P_BlockThingsIterator(bx, by, PIT_PushThing);
		return;
	}
	...
	{
		thing = node->m_thing;
		if (!thing->player || (thing->flags & (MF_NOGRAVITY | MF_NOCLIP))) continue;
		if (p->type == p_wind)
		{
...
static void P_SpawnPushers(void)
{
	...
	for (i = 0; i < numlines; i++, l++) switch (l->special)
		{
		case 224: // wind
			for (s = -1; (s = P_FindSectorFromLineTag(l, s)) >= 0;)
