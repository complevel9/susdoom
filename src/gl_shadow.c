...
simple_shadow_params_t simple_shadows = {0, 0, -1, 0, 0, 80, 1000, 0.5f, 0.0044f};
...
void gld_ProcessThingShadow(mobj_t *mo)
{
	...
	if (!simple_shadows.enable || !simple_shadows.loaded) return;

  // Should this mobj have a shadow?
	if (mo->flags & (MF_SHADOW | MF_NOBLOCKMAP | MF_NOSECTOR)) return;

	if (mo->frame & FF_FULLBRIGHT) return;

  // Don't render mobj shadows on sky floors.
	if (mo->subsector->sector->floorpic == skyflatnum) return;

	if (sectorloops[sec->iSectorID].loopcount <= 0) return;

  // Is this too far?
	dist = P_AproxDistance((mo->x >> 16) - (viewx >> 16), (mo->y >> 16) - (viewy >> 16));
	if (dist > simple_shadows.max_dist) return;
  ...
  // Calculate the strength of the shadow.
	...
	halfmoh = moh * 0.5f;
	if (height > halfmoh) shadow.light *= 1 - (height - halfmoh) / (moh - halfmoh);

  // Can't be seen.
	if (shadow.light <= 0) return;

	if (shadow.light > 1) shadow.light = 1;

  // Calculate the radius of the shadow.
	radius = mo->info->radius >> 16;
	if (radius > mo->patch_width >> 1) radius = mo->patch_width >> 1;
	if (radius > simple_shadows.max_radius) radius = simple_shadows.max_radius;
	if (!radius) return;
...
void gld_RenderShadows(void)
{
	...
	if (gld_drawinfo.num_items[GLDIT_SHADOW] <= 0) return;

	if (!gl_ztrick) glDepthRange(simple_shadows.bias, 1);
