...
static GLLight gld_light[gl_lightmode_last] = {
  // gl_lightmode_glboom
  {false, 16, gld_InitLightTable_glboom, gld_CalcLightLevel_glboom, gld_CalcLightLevel_glboom,
    gld_CalcFogDensity_glboom},

   // gl_lightmode_gzdoom
  {true, 8, gld_InitLightTable_gzdoom, gld_CalcLightLevel_gzdoom, gld_CalcLightLevel_gzdoom,
    gld_CalcFogDensity_gzdoom},

   // gl_lightmode_fogbased
  {true, 8, gld_InitLightTable_fogbased, gld_CalcLightLevel_fogbased, gld_CalcLightLevel_gzdoom,
    gld_CalcFogDensity_fogbased},

   // gl_lightmode_shaders
  {true, 16, gld_InitLightTable_glboom, gld_CalcLightLevel_shaders, gld_CalcLightLevel_glboom,
    gld_CalcFogDensity_glboom},
};
...
void M_ChangeLightMode(void)
{
	...
	{
		if (!glsl_Init()) gl_lightmode_default = gl_lightmode_glboom;
	}
	...
	if (gld_light[gl_lightmode].use_hwgamma) { gld_SetGammaRamp(useglgamma); }
	else
	{
		gld_SetGammaRamp(-1);
		gld_FlushTextures();
...
void gld_InitLightTable(void)
{
	...
	for (i = 0; i < gl_lightmode_last; i++) gld_light[i].Init();
}
...
static void gld_InitLightTable_glboom(void)
{
	...
	for (g = 0; g < 5; g++)
	{
		for (i = 0; i < 256; i++)
		{
			lighttable_glboom[g][i] = (float)((1.0f - exp(pow(i / 255.0f, 3) * gamma[g]))
			                                  / (1.0f - exp(1.0f * gamma[g])));
		}
	}
}
...
void gld_StaticLightAlpha(float light, float alpha)
{
	...
	else if (!(invul_method & INVUL_BW)) { glColor4f(1.0f, 1.0f, 1.0f, alpha); }
	else
	{
	...
	if (shaders) glsl_SetLightLevel((player->fixedcolormap ? 1.0f : light));
}
...
void M_ChangeAllowFog(void)
{
	...
	for (i = 0; i < 256; i++)
	{
		...
		{
			distfogtable[0][i]
			  = (float)((gl_distfog >> 1) - (gl_distfog >> 1) * (i - 164) / (230 - 164));
		}
		else { distfogtable[0][i] = 0.0f; }
...
static float gld_CalcFogDensity_gzdoom(sector_t *sector, int lightlevel, GLDrawItemType type)
{
	...
	else { return distfogtable[1][BETWEEN(0, 255, lightlevel)]; }
}

static float gld_CalcFogDensity_fogbased(sector_t *sector, int lightlevel, GLDrawItemType type)
{
	if (players[displayplayer].fixedcolormap) { return 0; }
	else
	{
