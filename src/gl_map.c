...
void gld_InitMapPics(void)
{
	...
	while (am_icons[i].name)
	{
		...
		if (lump != -1)
		{
			...
			surf_raw
			  = IMG_Load_RW(SDL_RWFromConstMem(W_CacheLumpNum(lump), W_LumpLength(lump)), true);
			...
			if (surf)
			{
				...
				if (gl_arb_texture_non_power_of_two)
				{
					...
					glTexImage2D(GL_TEXTURE_2D, 0, gl_tex_format, surf->w, surf->h, 0, GL_RGBA,
					  GL_UNSIGNED_BYTE, surf->pixels);
				}
				else
				{
					gluBuild2DMipmaps(GL_TEXTURE_2D, gl_tex_format, surf->w, surf->h, GL_RGBA,
					  GL_UNSIGNED_BYTE, surf->pixels);
				}
...
void gld_AddNiceThing(int type, float x, float y, float radius, float angle, unsigned char r,
...
void gld_ClearNiceThings(void)
{
	...
	for (type = 0; type < am_icon_count; type++) M_ArrayClear(&map_things[type]);
}
