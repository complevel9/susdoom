...
{
	gl_DeviceSupportsGamma = (-1
	                          != SDL_GetWindowGammaRamp(sdl_window, gl_oldHardwareGamma[0],
	                            gl_oldHardwareGamma[1], gl_oldHardwareGamma[2]));
    ...
    //
		if ((HIBYTE(gl_oldHardwareGamma[0][255]) <= HIBYTE(gl_oldHardwareGamma[0][0]))
    ...
    // if ((HIBYTE(gl_oldHardwareGamma[0][247]) == 255))
		{
			...
			lprintf(LO_WARN, "gld_CheckHardwareGamma: suspicious gamma tables, using linear ramp "
			                 "for restoration\n");
...
void gld_ResetGammaRamp(void)
{
	if (M_CheckParm("-resetgamma"))
	{
		...
		{
			lprintf(LO_WARN,
			  "gld_ResetGammaRamp: suspicious gamma tables, using linear ramp for restoration\n");
			_exit(0);
		}
	}
}
