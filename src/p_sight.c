...
dboolean P_SightBlockLinesIterator(int x, int y)
{
	...
	{
		ld = &lines[*list];
		if (ld->validcount == validcount) continue;    // line has already been checked
    ...
    // try to early out the check
		if (!ld->backsector) return false; // stop checking
...
dboolean P_SightTraverseIntercepts(void)
{
  ...
  //
	...
	while (count--)
	{
		...
		if (!PTR_SightTraverse(in)) return false;      // don't bother going farther
		in->frac = INT_MAX;
	}
...
dboolean P_SightPathTraverse(fixed_t x1, fixed_t y1, fixed_t x2, fixed_t y2)
{
	...
	if (((x1 - bmaporgx) & (MAPBLOCKSIZE - 1)) == 0) x1 += FRACUNIT; // don't side exactly on a line
	if (((y1 - bmaporgy) & (MAPBLOCKSIZE - 1)) == 0) y1 += FRACUNIT; // don't side exactly on a line
	trace.x = x1;
	trace.y = y1;
	trace.dx = x2 - x1;
	trace.dy = y2 - y1;
	...
	// each block
	if (xt1 < 0 || yt1 < 0 || xt1 >= bmapwidth || yt1 >= bmapheight || xt2 < 0 || yt2 < 0
	    || xt2 >= bmapwidth || yt2 >= bmapheight)
		return false;
...
INLINE static int P_DivlineSide(fixed_t x, fixed_t y, const divline_t *node)
{
	...
	return !node->dx ? x == node->x ? 2
	                 : x <= node->x ? node->dy > 0
	                                : node->dy < 0
	     : !node->dy ? (compatibility_level < prboom_4_compatibility ? x : y) == node->y ? 2
	                 : y <= node->y ? node->dx < 0
	                                : node->dx > 0
...
INLINE static int P_DivlineCrossed(fixed_t x1, fixed_t y1, fixed_t x2, fixed_t y2, divline_t *node)
{
	...
	if (!node->dx)
	{
		if (x1 == node->x) return (x2 == node->x);
		if (x2 == node->x) return (x1 == node->x);
		if (x1 < node->x) return (x2 < node->x);
		return (x2 > node->x);
	}
    ...
    //   return (x2 == node->y);
		if (_y1 == node->y) return (_y2 == node->y);
		if (_y2 == node->y) return (_y1 == node->y);
		if (y1 <= node->y) return (y2 < node->y);
		return (y2 > node->y);
	}

	{
		...
		if (right1 < left1) return (right2 < left2);

		if (left1 == right1) return (left2 == right2);
...
dboolean P_CrossSubsector_PrBoom(int num)
{
	...
	for (; ssline < ssline_last; ssline++)
	{
		 ...
		 */
		if (ssline->bbox[BOXLEFT] > los.bbox[BOXRIGHT] || ssline->bbox[BOXRIGHT] < los.bbox[BOXLEFT]
		...
		if (ssline->linedef->validcount == validcount) continue;
		...
		if (ssline->linedef->flags & ML_TWOSIDED)
		{
			...
			if ((opentop >= los.maxz) && (openbottom <= los.minz)) continue;
		}
		...
		{ // crosses a two sided line
			...
			if (front->floorheight != back->floorheight)
			{
				...
				if (slope > los.bottomslope) los.bottomslope = slope;
			}

			if (front->ceilingheight != back->ceilingheight)
			{
				fixed_t slope = FixedDiv(opentop - los.sightzstart, frac);
				if (slope < los.topslope) los.topslope = slope;
			}

			if (los.topslope <= los.bottomslope) return false; // stop
		}
	}
...
dboolean P_CrossSubsector_Doom(int num)
{
	...
	for (; ssline < ssline_last; ssline++)
	{
		...
		if (!(ssline->linedef->flags & ML_TWOSIDED)) return false;
		...
		if (!back) back = GetSectorAtNullAddress();
...
dboolean P_CheckSight(mobj_t *t1, mobj_t *t2)
{
	...
	if (compatibility_level == doom_12_compatibility) return P_CheckSight_12(t1, t2);
	...
	if ((t1->subsector == t2->subsector) && (compatibility_level >= mbf_compatibility)) return true;
	...
	los.topslope
	  = (los.bottomslope = t2->z - (los.sightzstart = t1->z + t1->height - (t1->height >> 2)))
	  + t2->height;
