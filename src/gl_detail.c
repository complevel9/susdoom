...
static const char *DetailItem_Keywords[TAG_DETAIL_MAX + 1] = {"walls", "flats", NULL};
...
void gld_ShutdownDetail(void)
{
	...
	if (details)
	{
		for (i = 0; i < details_count; i++) glDeleteTextures(1, &details[i].texid);
...
void gld_DrawWallDetail_NoARB(GLWall *wall)
{
	...
	if (gld_IsDetailVisible(
	      xCamera, yCamera, wall->glseg->x1, wall->glseg->z1, wall->glseg->x2, wall->glseg->z2))
	{
		float w, h, dx, dy;
		dboolean fake = (wall->flag == GLDWF_TOPFLUD) || (wall->flag == GLDWF_BOTFLUD);
		detail_t *detail = wall->gltexture->detail;
        ...
        // calculation of fog density for flooded walls
				if (wall->seg->backsector)
				{
					wall->fogdensity = gld_CalcFogDensity(
					  wall->seg->frontsector, wall->seg->backsector->lightlevel, GLDIT_FWALL);
				}
...
#if defined(USE_VERTEX_ARRAYS) || defined(USE_VBO)
		if (gl_use_display_lists) { glCallList(flats_display_list + flat->sectornum); }
		else
		{
...
void gld_DrawDetail_NoARB(void)
{
  ...
  // detail walls
	if (scene_has_wall_details)
	{
		...
		if (!gl_use_stencil) gld_DrawItemsSortByDetail(GLDIT_MWALL);
...
void gld_InitFrameDetails(void)
{
	...
	scene_has_details = (render_usedetail) && (scene_has_wall_details || scene_has_flat_details);
}
...
void gld_BindDetailARB(GLTexture *gltexture, int enable)
{
	if (scene_has_details)
	{
		...
		if (enable && gltexture->detail && gltexture->detail->texid != last_detail_texid)
		{
		...
		if (surf_raw)
		{
			...
			if (surf)
			{
				...
				gluBuild2DMipmaps(GL_TEXTURE_2D, gl_tex_format, surf->w, surf->h,
				  imageformats[surf->format->BytesPerPixel], GL_UNSIGNED_BYTE, surf->pixels);
    ...
    // get detail texture name
		...
		if (strlen(sc_String) < 9)
		{
			...
			if (detail->texid > 0)
			{
				...
				if (SC_Check() && SC_GetString() && M_StrToFloat(sc_String, &f)) detail->width = f;
				if (SC_Check() && SC_GetString() && M_StrToFloat(sc_String, &f)) detail->height = f;
...
void gld_ParseDetailItem(tag_detail_e item)
{
  // item's default values
	...
	if (SC_Check() && !SC_Compare("{")) gld_ReadDetailParams(item, &details[item]);
