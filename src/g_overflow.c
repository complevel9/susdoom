...
const char *overflow_cfgname[OVERFLOW_MAX]
  = {"overrun_spechit_emulate", "overrun_reject_emulate", "overrun_intercept_emulate",
    "overrun_playeringame_emulate", "overrun_donut_emulate", "overrun_missedbackside_emulate"};
...
static void ShowOverflowWarning(overrun_list_t overflow, int fatal, const char *params, ...)
{
	...
	if (overflows[overflow].warn && !overflows[overflow].promted)
	{
		...
		sprintf(buffer, (fatal ? str1 : (EMULATE(overflow) ? str2 : str3)), name[overflow],
...
{
	if (maxitcs < num_intercepts) maxitcs = num_intercepts;
...
int PlayeringameOverrun(const mapthing_t *mthing)
{
    ...
    // playeringame[-1] == players[3].didsecret
		...
		if (EMULATE(OVERFLOW_PLYERINGAME)) return true;
	}
	return false;
}
...
void SpechitOverrun(spechit_overrun_param_t *params)
{
	...
	if (demo_compatibility && numspechit > 8)
	{
		...
		ShowOverflowWarning(OVERFLOW_SPECHIT,
		  ...
		  spechit[0]->iLineID, spechit[1]->iLineID, spechit[2]->iLineID, spechit[3]->iLineID,
		  spechit[4]->iLineID, spechit[5]->iLineID, spechit[6]->iLineID, spechit[7]->iLineID,
		  spechit[8]->iLineID);
        ...
        //
          ...
          // baseaddr = atoi(myargv[p+1]);
				...
				else { spechit_baseaddr = DEFAULT_SPECHIT_MAGIC; }
			}
		...
		*rejectmatrix = memmove(newreject, *rejectmatrix, length);
		...
		if (demo_compatibility && PROCESS(OVERFLOW_REJECT))
		{
			...
			if (EMULATE(OVERFLOW_REJECT))
			{
				...
				for (i = 0; i < (unsigned int)(required - length) && i < 16; i++)
				{ // 16 hard-coded
	...
	if (demo_compatibility)
	{
		if (line)
		{
			ShowOverflowWarning(OVERFLOW_MISSEDBACKSIDE, 0,
			  "\n\nLinedef %d has two-sided flag set, but no second sidedef", line->iLineID);
		}
		else { ShowOverflowWarning(OVERFLOW_MISSEDBACKSIDE, 0, ""); }
	}
