...
void T_VerticalDoor(vldoor_t *door)
{
  ...
  // Is the door waiting, going up, or going down?
	...
	case -1:
      // Door is moving down
		res = T_MovePlane(
		  door->sector, door->speed, door->sector->floorheight, false, 1, door->direction);
      ...
      // e6y: "Tagged doors don't trigger special lighting" handled wrong
		...
		if (door->lighttag && door->topheight - door->sector->floorheight
			...
			EV_LightTurnOnPartway(
			  door->line, FixedDiv(door->sector->ceilingheight - door->sector->floorheight,
		...
		if (res == pastdest)
		{
			...
			case genBlazeClose:
				...
				if (comp[comp_blazing]) S_StartSound((mobj_t *)&door->sector->soundorg, sfx_bdcls);
				break;
...
int EV_VerticalDoor(line_t *line, mobj_t *thing)
{
	...
	/* If this is a repeatable line, and the door is already moving, then we can just reverse the
	 * current action. Note that in prboom 2.3.0 I erroneously removed the if-this-is-repeatable
	 * check, hence the prboom_4_compatibility clause below (foolishly assumed that already moving
	 * implies repeatable - but it could be moving due to another switch, e.g. lv19-509) */
	if (door
	    && ((compatibility_level == prboom_4_compatibility) || (line->special == 1)
	        || (line->special == 117) || (line->special == 26) || (line->special == 27)
	        || (line->special == 28)))
	{
		    ...
		    || door->thinker.function == T_VerticalDoor)
		{
			...
			if (outval)
			{
				if (door->thinker.function == T_VerticalDoor) { door->direction = outval; }
