...
void F_StartFinale(void)
{
  ...
  //  some stuff might have changed in the process.
	...
	if (gamemapinfo) FMI_StartFinale();
...
{
	return midstage ? NEWTEXTSPEED : (midstage = acceleratestage) ? acceleratestage = 0,
...
static const castinfo_t castorder[] = { // CPhipps - static const, initialised here
  {&s_CC_ZOMBIE, MT_POSSESSED}, {&s_CC_SHOTGUN, MT_SHOTGUY}, {&s_CC_HEAVY, MT_CHAINGUY},
  {&s_CC_IMP, MT_TROOP}, {&s_CC_DEMON, MT_SERGEANT}, {&s_CC_LOST, MT_SKULL}, {&s_CC_CACO, MT_HEAD},
  {&s_CC_HELL, MT_KNIGHT}, {&s_CC_BARON, MT_BRUISER}, {&s_CC_ARACH, MT_BABY}, {&s_CC_PAIN, MT_PAIN},
  {&s_CC_REVEN, MT_UNDEAD}, {&s_CC_MANCU, MT_FATSO}, {&s_CC_ARCH, MT_VILE},
  {&s_CC_SPIDER, MT_SPIDER}, {&s_CC_CYBER, MT_CYBORG}, {&s_CC_HERO, MT_PLAYER}, {NULL, 0}};
...
void F_CastDrawer(void)
{
  ...
  // CPhipps - patch drawing updated
	...
	// CPhipps - patch drawing updated
	V_DrawNumPatch(
	  160, 170, 0, lump + firstspritelump, CR_DEFAULT, VPT_STRETCH | (flip ? VPT_FLIP : 0));
}
...
void F_BunnyScroll(void)
{
...

	{
		...
		if (scrolled <= 0) { V_DrawNamePatch(0, 0, 0, pfub2, CR_DEFAULT, VPT_STRETCH); }
