    ...
    // copy start screen to main screen
		for (i = 0; i < SCREENHEIGHT; i++)
			memcpy(wipe_scr.data + i * wipe_scr.byte_pitch,
			  wipe_scr_start.data + i * wipe_scr_start.byte_pitch, SCREENWIDTH * V_GetPixelDepth());
	}
...
#ifdef GL_DOOM
	if (V_GetMode() == VID_MODEGL) gld_wipe_doMelt(ticks, y_lookup);
#endif
	return done;
}
...
int wipe_StartScreen(void)
{
  ...
  // e6y: fixed slowdown at 1024x768 on some systems
	...
	screens[SRC_SCR] = wipe_scr_start;
	V_CopyRect(
	  0, SRC_SCR, 0, 0, SCREENWIDTH, SCREENHEIGHT, VPT_NONE); // Copy start screen to buffer
	return 0;
}
