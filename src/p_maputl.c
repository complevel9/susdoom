...
int PUREFUNC P_BoxOnLineSide(const fixed_t *tmbox, const line_t *ld)
{
	...
	case ST_HORIZONTAL:
		return (tmbox[BOXBOTTOM] > ld->v1->y) == (p = tmbox[BOXTOP] > ld->v1->y) ? p ^ (ld->dx < 0)
		                                                                         : -1;
	case ST_VERTICAL:
		return (tmbox[BOXLEFT] < ld->v1->x) == (p = tmbox[BOXRIGHT] < ld->v1->x) ? p ^ (ld->dy < 0)
		                                                                         : -1;
	case ST_POSITIVE:
...
fixed_t PUREFUNC P_InterceptVector2(const divline_t *v2, const divline_t *v1)
{
	...
	return (den = FixedMul(v1->dy >> 8, v2->dx) - FixedMul(v1->dx >> 8, v2->dy))
	       ? FixedDiv(
	           FixedMul((v1->x - v2->x) >> 8, v1->dy) + FixedMul((v2->y - v1->y) >> 8, v1->dx), den)
	       : 0;
}
...
fixed_t PUREFUNC P_InterceptVector(const divline_t *v2, const divline_t *v1)
{
	...
	else
	{
		...
		if (!den) return 0;
		return (
		  fixed_t)(((int_64_t)(v1->x - v2->x) * v1->dy - (int_64_t)(v1->y - v2->y) * v1->dx) / den);
	}
}
...
dboolean P_BlockLinesIterator(int x, int y, dboolean func(line_t *))
{
	...
	for (; *list != -1; list++) // phares
	{
		line_t *ld;
#ifdef RANGECHECK
		if (*list < 0 || *list >= numlines) I_Error("P_BlockLinesIterator: index >= numlines");
#endif
		ld = &lines[*list];
...
dboolean P_PathTraverse(
  fixed_t x1, fixed_t y1, fixed_t x2, fixed_t y2, int flags, dboolean trav(intercept_t *))
{
...
dboolean P_PathNoTraverse(fixed_t x1, fixed_t y1, fixed_t x2, fixed_t y2, int flags)
{
	fixed_t xt1, yt1;
	fixed_t xt2, yt2;
	fixed_t xstep, ystep;
	...
	if (!((x1 - bmaporgx) & (MAPBLOCKSIZE - 1))) x1 += FRACUNIT; /* don't side exactly on a line */

	if (!((y1 - bmaporgy) & (MAPBLOCKSIZE - 1))) y1 += FRACUNIT; /* don't side exactly on a line */
