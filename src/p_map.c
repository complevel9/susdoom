...
int P_GetFriction(const mobj_t *mo, int *frictionfactor)
{
  ...
  /* Assign the friction value to objects on the floor, non-floating,
	...
	if (mo->flags & MF_FLY) { friction = FRICTION_FLY; }
	else
	{
		if (!(mo->flags & (MF_NOCLIP | MF_NOGRAVITY))
		    && (mbf_features || (mo->player && !compatibility)) && variable_friction)
	...
	if (frictionfactor) *frictionfactor = movefactor;
...
int P_GetMoveFactor(mobj_t *mo, int *frictionp)
{
  ...
  // p_mobj.c, the friction factors are applied as you coast and slow down.
	...
	if (frictionp) *frictionp = friction;
  ...
  PIT_CrossLine(line_t *ld)
{
	if (!(ld->flags & ML_TWOSIDED) || (ld->flags & (ML_BLOCKING | ML_BLOCKMONSTERS)))
		if (!(tmbbox[BOXLEFT] > ld->bbox[BOXRIGHT] || tmbbox[BOXRIGHT] < ld->bbox[BOXLEFT]
  ...
  PIT_CheckLine(line_t *ld)
{
	...
	if (P_BoxOnLineSide(tmbbox, ld) != -1) return true; // didn't hit it
...
static dboolean PIT_CheckThing(mobj_t *thing) // killough 3/26/98: make static
{
  ...
  // killough 11/98: add touchy things
	if (!(thing->flags & (MF_SOLID | MF_SPECIAL | MF_SHOOTABLE | MF_TOUCHY))) return true;
  ...
  // check for special pickup
	...
	if (thing->flags & MF_SPECIAL)
	{
		...
		if (tmthing->flags & MF_PICKUP) P_TouchSpecialThing(thing, tmthing); // can remove thing
		return !solid;
	}
...
dboolean Check_Sides(mobj_t *actor, int x, int y)
{
  ...
  // xl->xh, yl->yh determine the mapblock set to search
	...
	for (bx = xl; bx <= xh; bx++)
		for (by = yl; by <= yh; by++)
			if (!P_BlockLinesIterator(bx, by, PIT_CrossLine)) return true; //   ^
	return (false); //   |
} // phares
...
dboolean P_CheckPosition(mobj_t *thing, fixed_t x, fixed_t y)
{
	...
	if (tmthing->flags & MF_NOCLIP) return true;
	...
	for (bx = xl; bx <= xh; bx++)
		for (by = yl; by <= yh; by++)
			if (!P_BlockLinesIterator(bx, by, PIT_CheckLine)) return false; // doesn't fit
				...
				if ((compatibility || !dropoff
					...
					return false; // don't stand over a dropoff
			         ...
			         || (dropoff == 2 && // large jump down (e.g. dogs)
			             (tmfloorz - tmdropoffz > 128 * FRACUNIT || !thing->target
			               || thing->target->z > tmdropoffz)))
			{
				if (!monkeys || !mbf_features ? tmfloorz - tmdropoffz > 24 * FRACUNIT
...
static dboolean PIT_ApplyTorque(line_t *ld)
{
	    ...
	    && P_BoxOnLineSide(tmbbox, ld) == -1)
	{
		...
		fixed_t dist = // lever arm
		  +(ld->dx >> FRACBITS) * (mo->y >> FRACBITS) - (ld->dy >> FRACBITS) * (mo->x >> FRACBITS)
...
{
	int xl = P_GetSafeBlockX((tmbbox[BOXLEFT] = mo->x - mo->radius) - bmaporgx);
	int xh = P_GetSafeBlockX((tmbbox[BOXRIGHT] = mo->x + mo->radius) - bmaporgx);
	int yl = P_GetSafeBlockY((tmbbox[BOXBOTTOM] = mo->y - mo->radius) - bmaporgy);
	...
	for (bx = xl; bx <= xh; bx++)
		for (by = yl; by <= yh; by++) P_BlockLinesIterator(bx, by, PIT_ApplyTorque);
...
dboolean P_ThingHeightClip(mobj_t *thing)
{
	...
	if (onfloor)
	{
		...
		if (thing->intflags & MIF_FALLING && thing->gear >= MAXGEAR) thing->gear = 0;
	}
	else
	{
...
void P_HitSlideLine(line_t *ld)
{
	...
	if (mbf_features || prboom_comp[PC_PRBOOM_FRICTION].state)
	{
		icyfloor = P_AproxDistance(tmxmove, tmymove) > 4 * FRACUNIT && variable_friction
		        && // killough 8/28/98: calc friction on demand
		           slidemo->z <= slidemo->floorz && P_GetFriction(slidemo, NULL) > ORIG_FRICTION;
	}
	else
	{
		extern dboolean onground;
		icyfloor = !compatibility && variable_friction && slidemo->player && onground
...
dboolean PTR_SlideTraverse(intercept_t *in)
{
	...
	if (!in->isaline) I_Error("PTR_SlideTraverse: not a line?");

	li = in->d.line;

	if (!(li->flags & ML_TWOSIDED))
	{
		if (P_PointOnLineSide(slidemo->x, slidemo->y, li)) return true; // don't hit the back side
		goto isblocking;
	}
	...
	if (opentop - slidemo->z < slidemo->height) goto isblocking; // mobj is too high

	if (openbottom - slidemo->z > 24 * FRACUNIT) goto isblocking; // too big a step up
...
void P_SlideMove(mobj_t *mo)
{
	...
	do {
		...
		if (!--hitcount) goto stairstep; // don't loop forever
		...
		P_PathTraverse(
		  leadx, leady, leadx + mo->momx, leady + mo->momy, PT_ADDLINES, PTR_SlideTraverse);
		P_PathTraverse(
		  trailx, leady, trailx + mo->momx, leady + mo->momy, PT_ADDLINES, PTR_SlideTraverse);
		P_PathTraverse(
		  leadx, traily, leadx + mo->momx, traily + mo->momy, PT_ADDLINES, PTR_SlideTraverse);
...
stairstep:
			...
			if (!P_TryMove(mo, mo->x, mo->y + mo->momy, true))
				if (!P_TryMove(mo, mo->x + mo->momx, mo->y, true))
					if (compatibility_level == boom_201_compatibility) mo->momx = mo->momy = 0;
		...
		if ((bestslidefrac -= 0x800) > 0)
		{
			...
			if (!P_TryMove(mo, mo->x + newx, mo->y + newy, true)) goto stairstep;
		}
		...
		{
			if (D_abs(mo->player->momx) > D_abs(tmxmove)) mo->player->momx = tmxmove;
			if (D_abs(mo->player->momy) > D_abs(tmymove)) mo->player->momy = tmymove;
		}
	} // killough 3/15/98: Allow objects to drop off ledges:
...
dboolean PTR_AimTraverse(intercept_t *in)
{
	 ...
	 */
	if (th->flags & shootthing->flags & aim_flags_mask && !th->player) return true;
...
dboolean PTR_ShootTraverse(intercept_t *in)
{
	...
	if (in->isaline)
	{
		...
		{ // crosses a two sided (really 2s) line
			...
			else if ((li->frontsector->floorheight == li->backsector->floorheight
		...
		if (li->frontsector->ceilingpic == skyflatnum)
		{
			...
			if (li->backsector && li->backsector->ceilingpic == skyflatnum)
				...
				if (demo_compatibility || li->backsector->ceilingheight < z) return false;
		}
...
dboolean PIT_RadiusAttack(mobj_t *thing)
{
	...
	if (!(thing->flags & (MF_SHOOTABLE | MF_BOUNCES))) return true;
	...
	if (bombspot->flags & MF_BOUNCES ? thing->type == MT_CYBORG && bombsource->type == MT_CYBORG
...
void P_RadiusAttack(mobj_t *spot, mobj_t *source, int damage)
{
	...
	for (y = yl; y <= yh; y++)
		for (x = xl; x <= xh; x++) P_BlockThingsIterator(x, y, PIT_RadiusAttack);
}
...
dboolean PIT_ChangeSector(mobj_t *thing)
{
	...
	if (P_ThingHeightClip(thing)) return true; // keep checking
	...
	if (thing->health <= 0)
	{
		...
		if (compatibility_level != doom_12_compatibility) thing->flags &= ~MF_SOLID;
		thing->height = 0;
		thing->radius = 0;
		if (colored_blood)
		{
...
dboolean P_CheckSector(sector_t *sector, dboolean crunch)
{
	...
	for (n = sector->touching_thinglist; n; n = n->m_snext) n->visited = false;
...
inline static msecnode_t *P_GetSecnode(void) { return (msecnode_t *)Z_BMalloc(&secnodezone); }

// P_PutSecnode() returns a node to the freelist.

inline static void P_PutSecnode(msecnode_t *node) { Z_BFree(&secnodezone, node); }
...
msecnode_t *P_AddSecnode(sector_t *s, mobj_t *thing, msecnode_t *nextnode)
{
	...
	if (nextnode) nextnode->m_tprev = node; // set back link on Thing

	// Add new node at head of sector thread starting at s->touching_thinglist

	node->m_sprev = NULL; // prev node on sector thread
	node->m_snext = s->touching_thinglist; // next node on sector thread
	if (s->touching_thinglist) node->m_snext->m_sprev = node;
...
{
	while (node) node = P_DelSecnode(node);
}
...
void P_CreateSecNodeList(mobj_t *thing, fixed_t x, fixed_t y)
{
	...
	for (bx = xl; bx <= xh; bx++)
		for (by = yl; by <= yh; by++) P_BlockLinesIterator(bx, by, PIT_GetSectors);
	...
	while (node)
	{
		...
		{
			if (node == sector_list) sector_list = node->m_tnext;
...
/* cphipps 2004/08/30 -
 * Must clear tmthing at tic end, as it might contain a pointer to a removed thinker, or the level
 * might have ended/been ended and we clear the objects it was pointing too. Hopefully we don't need
 * to carry this between tics for sync. */
void P_MapStart(void)
{
	if (tmthing) I_Error("P_MapStart: tmthing set!");
}
void P_MapEnd(void) { tmthing = NULL; }
