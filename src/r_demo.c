...
int IsDemoPlayback(void)
{
	...
	if ((p = M_CheckParm("-playdemo")) && (p < myargc - 1)) return p;
	if ((p = M_CheckParm("-timedemo")) && (p < myargc - 1)) return p;
	if ((p = M_CheckParm("-fastdemo")) && (p < myargc - 1)) return p;
...
int IsDemoContinue(void)
{
	...
	if ((p = M_CheckParm("-recordfromto")) && (p < myargc - 2) && I_FindFile(myargv[p + 1], ".lmp"))
		return p;
...
int LoadDemo(const char *name, const byte **buffer, int *length, int *lump)
{
  ...
  // check ns_demos namespace first, then ns_global
	...
	if (num < 0) num = W_CheckNumForName(basename);
...
void R_SmoothPlaying_Reset(player_t *player)
{
	...
	{
		if (!player) player = &players[displayplayer];
...
{
	if (demo_extendedformat == -1) demo_extendedformat = demo_extendedformat_default;
...
angle_t R_DemoEx_ReadMLook(void)
{
	...
	if (!use_demoex_info || !(demoplayback || democontinue)) return 0;
  ...
  // mlook data must be initialised here
	...
	pitch = 0;
	if (mlook_lump.data && mlook_lump.tick < mlook_lump.maxtick && consoleplayer == displayplayer
	    && !walkcamera.type)
	{
...
void R_DemoEx_ResetMLook(void) { mlook_lump.tick = 0; }

void R_DemoEx_WriteMLook(angle_t pitch)
{
	if (!use_demoex_info || !demorecording) return;
	...
	if (mlook_lump.tick >= mlook_lump.maxtick)
	{
		...
		if (mlook_lump.tick >= mlook_lump.maxtick) mlook_lump.maxtick = mlook_lump.tick * 2;
		mlook_lump.data = realloc(mlook_lump.data, mlook_lump.maxtick * sizeof(mlook_lump.data[0]));
		memset(
		  mlook_lump.data + ticks, 0, (mlook_lump.maxtick - ticks) * sizeof(mlook_lump.data[0]));
	}
...
static int R_DemoEx_GetVersion(void)
{
	...
	if (lump != -1)
	{
		...
		if (size > 0)
		{
			...
			if (sscanf(str_ver, "%d", &ver) == 1) result = ver;
		}
		W_UnlockLumpNum(lump);
	}
...
static void R_DemoEx_GetParams(const byte *pwad_p, waddata_t *waddata)
{
	...
	if (params)
	{
		struct
		{
			const char *param;
			wad_source_t source;
		} files[] = {{"-iwad", source_iwad}, {"-file", source_pwad}, {"-deh", source_deh}, {NULL}};

		M_ParseCmdLine(
		  str, params, ((char *)params) + sizeof(char *) * paramscount, &paramscount, &i);
		...
		if (!M_CheckParm("-iwad") && !M_CheckParm("-file"))
		{
            ...
            // something is wrong here
						...
						if (!filename) filename = strdup(params[p]);
    ...
    // for doom 1.2
		if (!M_CheckParm("-nomonsters"))
		{
			...
			if (p >= 0) M_AddParam("-nomonsters");
		}

		p = M_CheckParmEx("-spechit", params, paramscount);
		if (p >= 0 && p < (int)paramscount - 1) spechit_baseaddr = atoi(params[p + 1]);
...
static void R_DemoEx_AddParams(wadtbl_t *wadtbl)
{
  ...
  // iwad and pwads
	for (i = 0; i < numwadfiles; i++)
	{
		...
		while (fileext_p != filename_p && *(fileext_p - 1) != '.') fileext_p--;
		if (fileext_p == filename_p) continue;

		item = NULL;

		if (wadfiles[i].src == source_iwad && !iwad && !strcasecmp(fileext_p, "wad")) item = &iwad;

		if (wadfiles[i].src == source_pwad && !strcasecmp(fileext_p, "wad")) item = &pwads;
  ...
  // overflows
	{
		...
		for (overflow = 0; overflow < OVERFLOW_MAX; overflow++)
		{
			...
			{
				sprintf(
				  buf, "-set %s=%d ", overflow_cfgname[overflow], overflows[overflow].emulate);
				AddString(&files, buf);
			}
		}
	}

	if (files) W_AddLump(wadtbl, DEMOEX_PARAMS_LUMPNAME, (const byte *)files, strlen(files));
}
...
byte *G_GetDemoFooter(const char *filename, const byte **footer, size_t *size)
{
  ...
  // get demo size in bytes
    ...
    // skip demo data
		while (p < buffer + file_size && *p != DEMOMARKER) p += bytes_per_tic;
...
void G_SetDemoFooter(const char *filename, wadtbl_t *wadtbl)
{
	...
	if (buffer)
	{
      ...
      // write pwad header, all data and lookup table to the end of a demo
			if (fwrite(buffer, demosize, 1, hfile) != 1
			    || fwrite(&wadtbl->header, headersize, 1, hfile) != 1
			    || fwrite(wadtbl->data, datasize, 1, hfile) != 1
			    || fwrite(wadtbl->lumps, lumpssize, 1, hfile) != 1 || false)
			{
	...
	if (buffer && size > sizeof(*header))
	{
		    ...
		    || strncmp(header->identification, "PWAD", 4) == 0)
		{
			...
			if (header->infotableofs + length <= size)
			{
				...
				{
					if (fileinfo->filepos < 0 || fileinfo->filepos > header->infotableofs
...
static int G_ReadDemoFooter(const char *filename)
{
	...
	if (!use_demoex_info) return result;
	...
	else
	{
		...
		if (tmp_dir && *tmp_dir != '\0')
		{
			...
			if (!HasTrailingSlash(tmp_dir)) strcat(tmp_path, "/");

			doom_snprintf(demoex_filename, sizeof(demoex_filename), template_format, tmp_path);
#ifdef HAVE_MKSTEMP
			if (mkstemp(demoex_filename) == -1) demoex_filename[0] = 0;
#else
	...
	else
	{
      ...
      // write an additional info from a demo to demoex.wad
        ...
        // add demoex.wad to the wads list
					...
					for (i = 0; i < numwadfiles; i++)
					{
						if (wadfiles[i].src == source_auto_load || wadfiles[i].src == source_pre
...
void G_WriteDemoFooter(FILE *file)
{
	...
	W_AddLump(
	  &demoex, DEMOEX_VERSION_LUMPNAME, (const byte *)DEMOEX_VERSION, strlen(DEMOEX_VERSION));
	W_AddLump(&demoex, NULL, (const byte *)DEMOEX_SEPARATOR, strlen(DEMOEX_SEPARATOR));
	...
	// process port name
	W_AddLump(&demoex, DEMOEX_PORTNAME_LUMPNAME, (const byte *)(PACKAGE_NAME " " PACKAGE_VERSION),
	...
	// write pwad header, all data and lookup table to the end of a demo
	if (fwrite(&demoex.header, sizeof(demoex.header), 1, file) != 1
...
{
	if (!waddata || !filename) return false;
...
int ParseDemoPattern(const char *str, waddata_t *waddata, char **missed, dboolean trytodownload)
{
	...
	for (; (pToken = strtok(pToken, "|")); pToken = NULL)
	{
		...
		if (trytodownload && !I_FindFile2(pToken, ".wad")) D_TryGetWad(pToken);
#ifdef _MSC_VER
...
#endif
		{
			...
			if (pToken == pStr) { wadfiles[numwadfiles].src = source_iwad; }
			else
			{
				char *p = (char *)wadfiles[numwadfiles].name;
				int len = strlen(p);
				if (!strcasecmp(&p[len - 4], ".wad")) wadfiles[numwadfiles].src = source_pwad;
...
int DemoNameToWadData(const char *demoname, waddata_t *waddata, patterndata_t *patterndata)
{
	...
	for (i = 0; i < demo_patterns_count; i++)
		if (strlen(demo_patterns_list[i]) > maxlen) maxlen = strlen(demo_patterns_list[i]);
	...
	for (i = 0; i < demo_patterns_count; i++)
	{
		...
		else
		{
			...
			if (result != 0)
			{
				regerror(result, &preg, errbuf, sizeof(errbuf));
				lprintf(LO_WARN,
				  "Incorrect regular expressions in the <%s%d = \"%s\"> config entry - %s\n",
			...
			else
			{
				    ...
				    && demo_match[0].rm_eo == (int)strlen(demofilename))
				{
					...
					waddata->wadfiles
					  = realloc(waddata->wadfiles, sizeof(*wadfiles) * (waddata->numwadfiles + 1));
					...
					{
						len = MIN(
						  pmatch[1].rm_eo - pmatch[1].rm_so, sizeof(patterndata->pattern_name) - 1);
...
void WadDataToWadFiles(waddata_t *waddata)
{
	...
	if (iwadindex == -1) I_Error("WadDataToWadFiles: IWAD not found\n");
...
int CheckAutoDemo(void)
{
	...
	if (M_CheckParm("-auto"))
		...
		I_Error("Cannot process -auto - " PACKAGE_NAME " was compiled without LIBPCRE support");
#else
	{
		int i;
		waddata_t waddata;
...
dboolean D_TryGetWad(const char *name)
{
	...
	char *wadname_p = NULL;
	char *msg = NULL;
	const char *format = "The necessary wad has not been found\n"
	                     "Do you want to search for \'%s\'?\n\n"
	                     "Command line:\n%s\n\n"
	                     "Be careful! Execution of an unknown program is unsafe.";

	if (!getwad_cmdline || !name || !(*getwad_cmdline) || !(*name)) return false;
	...
	else { sprintf(cmdline, "%s %s", getwad_cmdline, wadname); }
