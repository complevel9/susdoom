...
static const crdef_t crdefs[] = {{"CRBRICK", &colrngs[CR_BRICK]}, {"CRTAN", &colrngs[CR_TAN]},
  {"CRGRAY", &colrngs[CR_GRAY]}, {"CRGREEN", &colrngs[CR_GREEN]}, {"CRBROWN", &colrngs[CR_BROWN]},
  {"CRGOLD", &colrngs[CR_GOLD]}, {"CRRED", &colrngs[CR_RED]}, {"CRBLUE", &colrngs[CR_BLUE]},
  {"CRORANGE", &colrngs[CR_ORANGE]}, {"CRYELLOW", &colrngs[CR_YELLOW]},
  {"CRBLUE2", &colrngs[CR_BLUE2]}, {"CRBLACK", &colrngs[CR_BLACK]},
  {"CRPURPLE", &colrngs[CR_PURPLE]}, {"CRWHITE", &colrngs[CR_WHITE]}, {NULL}};
...
void V_InitFlexTranTable(void)
{
    ...
    // build RGB table
		for (r = 0; r < 32; r++)
		{
			for (g = 0; g < 32; g++)
			{
				...
				{
					RGB32k[r][g][b]
					  = V_BestColor(palette, MAKECOLOR(r), MAKECOLOR(g), MAKECOLOR(b));
				}
			}
		}
    ...
    // build lookup table
		for (x = 0; x < 65; x++)
		{
			...
			for (y = 0; y < 256; y++)
			{
				Col2RGB8[x][y] = (((palette[pos + 0] * x) >> 4) << 20)
				               | ((palette[pos + 1] * x) >> 4)
...
void V_InitColorTranslation(void)
{
	...
	for (p = crdefs; p->name; p++)
	{
		*p->map = W_CacheLumpName(p->name);
		if (p - crdefs == CR_DEFAULT) continue;
...
// No return.
//
static void FUNC_V_CopyRect(
  int srcscrn, int destscrn, int x, int y, int width, int height, enum patch_translation_e flags)
{
	byte *src;
	byte *dest;
...
static void FUNC_V_FillFlat(
  int lump, int scrn, int x, int y, int width, int height, enum patch_translation_e flags)
{
  ...
  int lump, int scrn, int x, int y, int width, int height, enum patch_translation_e flags)
{
	...
	for (sy = y; sy < y + height; sy += h)
		for (sx = x; sx < x + width; sx += w) V_DrawNumPatch(sx, sy, scrn, lump, CR_DEFAULT, flags);
}
...
//
static void V_DrawMemPatch(
  int x, int y, int scrn, const rpatch_t *patch, int cm, enum patch_translation_e flags)
{
	const byte *trans;
  ...
  // CPhipps - auto-no-stretch if not high-res
	if ((flags & VPT_STRETCH_MASK) && SCREEN_320x200) flags &= ~VPT_STRETCH_MASK;

  // e6y: wide-res
	params = &stretch_params[flags & VPT_ALIGN_MASK];

  // CPhipps - null translation pointer => no translation
	if (!trans) flags &= ~VPT_TRANS;

  // [FG] automatically center wide patches without horizontal offset
	if (patch->width > 320 && patch->leftoffset == 0) x -= (patch->width - 320) / 2;
	...
	if (V_GetMode() == VID_MODE8 && !(flags & VPT_STRETCH_MASK))
	{
      ...
      // killough 1/19/98: improved error message:
			lprintf(LO_WARN,
			  "V_DrawMemPatch8: Patch (%d,%d)-(%d,%d) exceeds LFB in vertical direction "
			  "(horizontal is clipped)\n"
    ...
    //         - reformat initialisers, move variables into inner blocks
		...
		{
			colfunc
			  = R_GetDrawColumnFunc(RDC_PIPELINE_STANDARD, drawvars.filterpatch, RDRAW_FILTER_NONE);
		}
    ...
    // e6y: predefined arrays are used
      ...
      // FIXME: Is it needed only for F_BunnyScroll?
			...
			left
			  = (x < 0 || x > 320 ? (x * params->video->width) / 320 : params->video->x1lookup[x]);
			top
			  = (y < 0 || y > 200 ? (y * params->video->height) / 200 : params->video->y1lookup[y]);
      ...
      // ignore this column if it's to the left of our clampRect
			if (dcvars.x < 0) continue;
			if (dcvars.x >= SCREENWIDTH) break;

			dcvars.texu = ((flags & VPT_FLIP) ? ((patch->width << FRACBITS) - col) : col)
      ...
      // step through the posts in a column
        ...
        // e6y
				if (!(flags & VPT_STRETCH_MASK))
				{
					...
					dcvars.yh = ((((y + post->topdelta + post->length) << 16) - (FRACUNIT >> 1))
					             >> FRACBITS);
				}
				else
				{
          ...
          // of the top and bottom screen coordinates of a column.
				...
				if ((dcvars.yh < 0) || (dcvars.yh < top)) continue;
				if ((dcvars.yl >= SCREENHEIGHT) || (dcvars.yl >= bottom)) continue;
          ...
          // dcvars.yh = bottom-1;
				...
				dcvars.prevsource
				  = prevcolumn ? prevcolumn->pixels + post->topdelta + yoffset : dcvars.source;
				dcvars.nextsource
				  = nextcolumn ? nextcolumn->pixels + post->topdelta + yoffset : dcvars.source;
...
static void FUNC_V_DrawNumPatch(
  int x, int y, int scrn, int lump, int cm, enum patch_translation_e flags)
{
	V_DrawMemPatch(x, y, scrn, R_CachePatchNum(lump), cm, flags);
	R_UnlockPatchNum(lump);
}

static void FUNC_V_DrawNumPatchPrecise(
  float x, float y, int scrn, int lump, int cm, enum patch_translation_e flags)
{
...
void V_UpdateTrueColorPalette(video_mode_t mode)
{
  ...
  // opengl doesn't use the gamma
	const byte *const gtable
	  = (const byte *)W_CacheLumpNum(gtlump) + (V_GetMode() == VID_MODEGL ? 0 : 256 * (usegamma));
...
void V_SetPalette(int pal)
{
	...
	else
	{
      ...
      // we've loaded any wads, which prevents us from reading the palette - POPE
			if (W_CheckNumForName("PLAYPAL") >= 0) V_UpdateTrueColorPalette(V_GetMode());
		}
	}
}
...
static void WRAP_gld_FillFlat(
  int lump, int n, int x, int y, int width, int height, enum patch_translation_e flags)
{
	gld_FillFlat(lump, x, y, width, height, flags);
}
static void WRAP_gld_FillPatch(
  int lump, int n, int x, int y, int width, int height, enum patch_translation_e flags)
{
	gld_FillPatch(lump, x, y, width, height, flags);
}
static void WRAP_gld_DrawNumPatch(
  int x, int y, int scrn, int lump, int cm, enum patch_translation_e flags)
{
	gld_DrawNumPatch(x, y, lump, cm, flags);
}
static void WRAP_gld_DrawNumPatchPrecise(
  float x, float y, int scrn, int lump, int cm, enum patch_translation_e flags)
{
	gld_DrawNumPatch_f(x, y, lump, cm, flags);
}
static void WRAP_gld_DrawBlock(
  int x, int y, int scrn, int width, int height, const byte *src, enum patch_translation_e flags)
{
}
...
static void NULL_CopyRect(
  int srcscrn, int destscrn, int x, int y, int width, int height, enum patch_translation_e flags)
{
}
static void NULL_FillFlat(
  int lump, int n, int x, int y, int width, int height, enum patch_translation_e flags)
{
}
static void NULL_FillPatch(
  int lump, int n, int x, int y, int width, int height, enum patch_translation_e flags)
{
}
static void NULL_DrawBackground(const char *flatname, int n) {}
static void NULL_DrawNumPatch(
  int x, int y, int scrn, int lump, int cm, enum patch_translation_e flags)
{
}
static void NULL_DrawNumPatchPrecise(
  float x, float y, int scrn, int lump, int cm, enum patch_translation_e flags)
{
}
static void NULL_DrawBlock(
  int x, int y, int scrn, int width, int height, const byte *src, enum patch_translation_e flags)
{
}
...
// V_GetMode
//
video_mode_t V_GetMode(void) { return current_videomode; }
...
// V_GetPixelDepth
//
int V_GetPixelDepth(void) { return V_GetModePixelDepth(current_videomode); }
...
void V_AllocScreens(void)
{
	...
	for (i = 0; i < NUM_SCREENS; i++) V_AllocScreen(&screens[i]);
}
...
void V_FreeScreens(void)
{
	...
	for (i = 0; i < NUM_SCREENS; i++) V_FreeScreen(&screens[i]);
}
...
static void WRAP_V_DrawLine(fline_t *fl, int color)
{
  ...
  // For debugging only
	if (fl->a.x < 0 || fl->a.x >= SCREENWIDTH || fl->a.y < 0 || fl->a.y >= SCREENHEIGHT
	    || fl->b.x < 0 || fl->b.x >= SCREENWIDTH || fl->b.y < 0 || fl->b.y >= SCREENHEIGHT)
	{
  ...
  // draw first pixel
    ...
    // line is y-axis major.
      ...
      // if error has overflown, advance x coordinate
			if (erroracc <= erroracctmp) x += xdir;

			y += 1; // advance y

      // the trick is in the trig!
			V_PlotPixelWu(
			  0, x, y, (byte)color, finecosine[erroracc >> wu_fineshift] >> wu_fixedshift);
			V_PlotPixelWu(
			  0, x + xdir, y, (byte)color, finesine[erroracc >> wu_fineshift] >> wu_fixedshift);
		}
	}
	else
	{
    // line is x-axis major.
      ...
      // the trick is in the trig!
			V_PlotPixelWu(
			  0, x, y, (byte)color, finecosine[erroracc >> wu_fineshift] >> wu_fixedshift);
			V_PlotPixelWu(
			  0, x, y + 1, (byte)color, finesine[erroracc >> wu_fineshift] >> wu_fixedshift);
		}
	}
...
void V_FillBorder(int lump, byte color)
{
	...
	if (render_stretch_hud == patch_stretch_full) return;
	...
	if (lump >= 0)
	{
      ...
      // Left
			V_FillFlat(
			  lump, 0, 0, bordtop, bordleft, SCREENHEIGHT - bordbottom - bordtop, VPT_NONE);
      // Right
			V_FillFlat(lump, 0, SCREENWIDTH - bordright, bordtop, bordright,
			  SCREENHEIGHT - bordbottom - bordtop, VPT_NONE);
		}
	}
	else
	{
      ...
      // Right
			V_FillRect(0, SCREENWIDTH - bordright, bordtop, bordright,
			  SCREENHEIGHT - bordbottom - bordtop, color);
		}
	}
}
...
void SetRatio(int width, int height)
{
  ...
  // ratio came from Wikipedia.  SAR x PAR = DAR
	...
	else { lprintf(LO_INFO, "SetRatio: assuming square pixels\n"); }
  ...
  // custom scaling when "not adjusted" is used
	if (render_patches_scalex > 0) patches_scalex = MIN(render_patches_scalex, patches_scalex);
	if (render_patches_scaley > 0) patches_scaley = MIN(render_patches_scaley, patches_scaley);
	...
	SCREEN_320x200 = (SCREENWIDTH == 320) && (SCREENHEIGHT == 200) && (WIDE_SCREENWIDTH == 320)
	              && (WIDE_SCREENHEIGHT == 200);
...
void V_ToggleFullscreen(void)
{
	...
	else { desired_fullscreen = (desired_fullscreen ? 0 : 1); }

	I_UpdateVideoMode();

#ifdef GL_DOOM
	if (V_GetMode() == VID_MODEGL) gld_PreprocessLevel();
#endif
}
