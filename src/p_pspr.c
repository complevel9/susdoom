...
static void P_BringUpWeapon(player_t *player)
{
  ...
  // killough 12/98: prevent pistol from starting visibly at bottom of screen:
	player->psprites[ps_weapon].sy = mbf_features ? WEAPONBOTTOM + FRACUNIT * 2 : WEAPONBOTTOM;
  ...
  // killough 2/8/98: follow preferences and fix BFG/SSG bugs
	...
	do switch (*prefer++)
		...
		case 2:
			...
			break;
		case 3:
			if (player->weaponowned[wp_shotgun] && player->ammo[am_shell]) newweapon = wp_shotgun;
			break;
		case 4:
			if (player->weaponowned[wp_chaingun] && player->ammo[am_clip]) newweapon = wp_chaingun;
			break;
		case 5:
			if (player->weaponowned[wp_missile] && player->ammo[am_misl]) newweapon = wp_missile;
			break;
		case 6:
...
void A_Punch(player_t *player, pspdef_t *psp)
{
  ...
  /* killough 8/2/98: make autoaiming prefer enemies */
	if (!mbf_features
	    || (slope = P_AimLineAttack(player->mo, angle, MELEERANGE, MF_FRIEND), !linetarget))
...
void A_Saw(player_t *player, pspdef_t *psp)
{
  ...
  /* Use meleerange + 1 so that the puff doesn't skip the flash
	 * killough 8/2/98: make autoaiming prefer enemies */
	if (!mbf_features
	    || (slope = P_AimLineAttack(player->mo, angle, MELEERANGE + 1, MF_FRIEND), !linetarget))
...
void A_FireOldBFG(player_t *player, pspdef_t *psp)
{
	...
	if (weapon_recoil && !(player->mo->flags & MF_NOCLIP))
		P_Thrust(player, ANG180 + player->mo->angle, 512 * recoil_values[wp_plasma]);
	...
	do {
      ...
      // killough 8/2/98: make autoaiming prefer enemies
		...
		th
		  = P_SpawnMobj(mo->x, mo->y, mo->z + 62 * FRACUNIT - player->psprites[ps_weapon].sy, type);
...
//  player->ammo[weaponinfo[player->readyweapon].ammo]--; // infinite ammo
	...
	for (i = -1024; i < 1024; i++)
	{
		angle_t angle = pmo->angle + (i << ANGLETOFINESHIFT);
		{
			...
			P_PathNoTraverse(
			  pmo->x, pmo->y, x2, y2, PT_ADDLINES | PT_ADDTHINGS | (i & 63 ? PT_NOTRACE : 0));
		}
	}
}
...
void A_BFGSpray(mobj_t *mo)
{
	...
	for (i = 0; i < 40; i++)  // offset angles from its attack angle
	{
      ...
      // killough 8/2/98: make autoaiming prefer enemies
		if (!mbf_features
		    || (P_AimLineAttack(mo->target, an, 16 * 64 * FRACUNIT, MF_FRIEND), !linetarget))
			P_AimLineAttack(mo->target, an, 16 * 64 * FRACUNIT, 0);

		if (!linetarget) continue;

		P_SpawnMobj(
		  linetarget->x, linetarget->y, linetarget->z + (linetarget->height >> 2), MT_EXTRABFG);
