        ...
        // will be set when unarc thinker
			...
			for (j = 0; j < NUMPSPRITES; j++)
				if (players[i].psprites[j].state)
					players[i].psprites[j].state = &states[(size_t)players[i].psprites[j].state];
		}
}
...
void P_ArchiveWorld(void)
{
  ...
  // killough 10/98: adjust size for changes below
	size_t size
	  = (sizeof(short) * 5 + sizeof sec->floorheight + sizeof sec->ceilingheight) * numsectors
...
void P_ThinkerToIndex(void)
{
  ...
  // the prev field as a placeholder, since it can be restored later.
	...
	for (th = thinkercap.next; th != &thinkercap; th = th->next)
		if (th->function == P_MobjThinker) th->prev = (thinker_t *)(intptr_t)++number_of_thinkers;
}
...
void P_IndexToThinker(void)
{
  // killough 2/14/98: restore prev pointers
	...
	for (th = thinkercap.next; th != &thinkercap; prev = th, th = th->next) th->prev = prev;
}
...
void P_ArchiveThinkers(void)
{
  ...
  /* check that enough room is available in savegame buffer
	 ...
	 */
	CheckSaveGame(
	  number_of_thinkers * (sizeof(mobj_t) - 3 * sizeof(fixed_t) + 4 + 3 * sizeof(void *)) + 1);
  ...
  // save off the current thinkers
	for (th = thinkercap.next; th != &thinkercap; th = th->next)
        ...
        // seeing player anymore.
			...
			if (mobj->lastenemy)
				mobj->lastenemy = mobj->lastenemy->thinker.function == P_MobjThinker
        ...
        // killough 2/14/98: end changes
			...
			if (mobj->player) mobj->player = (player_t *)((mobj->player - players) + 1);
		}
...
void P_UnArchiveThinkers(void)
{
  ...
  // read in saved thinkers
	for (size = 1; *save_p++ == tc_mobj; size++)    // killough 2/14/98
	{
      ...
      // killough 2/14/98 -- insert pointers to thinkers into table, in order:
		...
		if (mobj->player) (mobj->player = &players[(size_t)mobj->player - 1])->mo = mobj;
      ...
      //      mobj->ceilingz = mobj->subsector->sector->ceilingheight;
		...
		if (!((mobj->flags ^ MF_COUNTKILL) & (MF_FRIEND | MF_COUNTKILL | MF_CORPSE))) totallive++;
	}
  ...
  // killough 11/98: use P_SetNewTarget() to set fields
	...
	for (th = thinkercap.next; th != &thinkercap; th = th->next)
	{
		P_SetNewTarget(&((mobj_t *)th)->target, mobj_p[P_GetMobj(((mobj_t *)th)->target, size)]);

		P_SetNewTarget(&((mobj_t *)th)->tracer, mobj_p[P_GetMobj(((mobj_t *)th)->tracer, size)]);

		P_SetNewTarget(
		  &((mobj_t *)th)->lastenemy, mobj_p[P_GetMobj(((mobj_t *)th)->lastenemy, size)]);
	}
	...
	while ((tclass = *save_p++) != tc_endspecials)  // killough 2/14/98
		...
		case tc_ceiling:
			PADSAVEP();
			{
				...
				if (ceiling->thinker.function) ceiling->thinker.function = T_MoveCeiling;
		...
		case tc_plat:
			PADSAVEP();
			{
				...
				if (plat->thinker.function) plat->thinker.function = T_PlatRaise;
...
void P_UnArchiveMap(void)
{
	...
	if (automapmode & am_active) AM_Start();
