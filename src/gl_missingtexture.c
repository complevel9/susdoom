  ...
  /* free memory if allocated by previous maps */
    ...
    // the following is for specialeffects. see r_bsp.c in R_Subsector
		...
		for (i = 0; i < sectors[num].linecount; i++)
		{
			...
			if (side0 && side1)
			{
				if (side0->toptexture != NO_TEXTURE) sectors[num].flags &= ~NO_TOPTEXTURES;
				if (side0->bottomtexture != NO_TEXTURE) sectors[num].flags &= ~NO_BOTTOMTEXTURES;
				if (side1->toptexture != NO_TEXTURE) sectors[num].flags &= ~NO_TOPTEXTURES;
				if (side1->bottomtexture != NO_TEXTURE) sectors[num].flags &= ~NO_BOTTOMTEXTURES;
        ...
        /* sides should not have null sectors, but we check anyway */
				if (side0->sector && side1->sector)
				{
					...
					dboolean needs_back_lower
					  = !(front_floor_is_sky)
					 && (side0->sector->floorheight > side1->sector->floorheight);
					dboolean needs_front_lower
					  = !(back_floor_is_sky)
					 && (side0->sector->floorheight < side1->sector->floorheight);

					dboolean needs_front_upper
					  = !(back_ceil_is_sky)
					 && (side0->sector->ceilingheight > side1->sector->ceilingheight);
					dboolean needs_back_upper
					  = !(front_ceil_is_sky)
					 && (side0->sector->ceilingheight < side1->sector->ceilingheight);
...
static void gld_RegisterBleedthroughSector(sector_t *source, sector_t *target, int ceiling)
{
	...
	for (i = 0; i < numbleedsectors && source_idx == -1; i++)
		if (bleedsectors[i].source == source && bleedsectors[i].ceiling == ceiling) source_idx = i;

	if (source_idx == -1)
	{
		/* allocate memory for new sector */
		bleedsectors
		  = (bleedthrough_t *)realloc(bleedsectors, (numbleedsectors + 1) * sizeof(bleedthrough_t));
  ...
  // precalculate NO_TOPTEXTURES and NO_BOTTOMTEXTURES flags
	...
	for (ceiling = 0; ceiling <= 1; ceiling++)
	{
		...
		do {
			for (i = 0; i < numsectors; i++)
			{
				if (!(sectors[i].flags & no_texture_flag) && (sectors[i].fakegroup[ceiling] == -1))
				{
...
sector_t *GetBestFake(sector_t *sector, int ceiling, int validcount)
{
	...
	if (fakeplanes[groupid].sector)
	{
		if (fakeplanes[groupid].ceiling)
		{
			if (sector->ceilingheight < fakeplanes[groupid].sector->ceilingheight) return sector;
		}
		else
		{
			if (sector->floorheight > fakeplanes[groupid].sector->floorheight) return sector;
		}
	}
...
void gld_SetupFloodedPlaneLight(GLWall *wall)
{
	...
	else { gld_StaticLightAlpha(wall->light, wall->alpha); }
}
