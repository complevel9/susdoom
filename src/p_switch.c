  ...
  // jff 3/23/98 read the switch table from a predefined lump
	...
	for (i = 0;; i++)
	{
		if (index + 1 >= max_numswitches)
			switchlist = realloc(switchlist,
			  sizeof *switchlist * (max_numswitches = max_numswitches ? max_numswitches * 2 : 8));
		if (LittleShort(alphSwitchList[i].episode) <= episode) // jff 5/11/98 endianess
		{
			int texture1, texture2;

			if (!LittleShort(alphSwitchList[i].episode)) break;
...
void P_ChangeSwitchTexture(line_t *line, int useAgain)
{
  ...
  /* search for a texture to change */
	...
	if (texture == NULL) return; /* no switch texture was found to change */
	*texture = switchlist[i ^ 1];

	S_StartSound(soundorg, sound);

	if (useAgain) P_StartButton(line, position, switchlist[i], BUTTONTIME);
}
...
//
dboolean P_UseSpecialLine(mobj_t *thing, line_t *line, int side, dboolean bossaction)
{
    ...
    // check each range of generalized linedefs
      ...
      // Out of range for GenFloors
		...
		else if ((unsigned)line->special >= GenLockedBase)
		{
			...
			if (!P_CanUnlockGenDoor(line, thing->player)) return false;
			if (!comperr(comperr_zerotag) && !line->tag
			    && ((line->special & 6) != 6)) // e6y //jff 2/27/98 all non-manual
				return false;                         // generalized types require tag
		...
		{
			if (!thing->player && !bossaction)
				if (!(line->special & LiftMonster)) return false; // monsters disallowed
			if (!comperr(comperr_zerotag) && !line->tag
			    && ((line->special & 6) != 6)) // e6y //jff 2/27/98 all non-manual
				return false;                         // generalized types require tag
		...
		{
			if (!thing->player && !bossaction)
				if (!(line->special & StairMonster)) return false; // monsters disallowed
			if (!comperr(comperr_zerotag) && !line->tag
			    && ((line->special & 6) != 6)) // e6y //jff 2/27/98 all non-manual
				return false;                         // generalized types require tag
		...
		{
			if (!thing->player && !bossaction)
				if (!(line->special & CrusherMonster)) return false; // monsters disallowed
			if (!comperr(comperr_zerotag) && !line->tag
			    && ((line->special & 6) != 6)) // e6y //jff 2/27/98 all non-manual
				return false;                         // generalized types require tag
		...
		if (linefunc) switch ((line->special & TriggerType) >> TriggerTypeShift)
			...
			case PushOnce:
				if (!side)
					if (linefunc(line)) line->special = 0;
				return true;
			case PushMany:
  ...
  // Switches that other things can activate.
	...
	case 50:
		// Close Door
		if (EV_DoDoor(line, closeDoor)) P_ChangeSwitchTexture(line, 0);
		break;
	...
	case 103:
		// Open Door
		if (EV_DoDoor(line, openDoor)) P_ChangeSwitchTexture(line, 0);
		break;
	...
	default:
		if (!demo_compatibility) switch (line->special)
			...
			case 158:
				...
				if (EV_DoFloor(line, raiseToTexture)) P_ChangeSwitchTexture(line, 0);
				break;

			case 159:
				// Raise Floor to shortest lower texture
				// 159 S1  EV_DoFloor(lowerAndChange)
				if (EV_DoFloor(line, lowerAndChange)) P_ChangeSwitchTexture(line, 0);
				break;

			case 160:
				// Raise Floor 24 and change
				// 160 S1  EV_DoFloor(raiseFloor24AndChange)
				if (EV_DoFloor(line, raiseFloor24AndChange)) P_ChangeSwitchTexture(line, 0);
				break;

			case 161:
				// Raise Floor 24
				// 161 S1  EV_DoFloor(raiseFloor24)
				if (EV_DoFloor(line, raiseFloor24)) P_ChangeSwitchTexture(line, 0);
				break;

			case 162:
				// Moving floor min n to max n
				// 162 S1  EV_DoPlat(perpetualRaise,0)
				if (EV_DoPlat(line, perpetualRaise, 0)) P_ChangeSwitchTexture(line, 0);
				break;
			...
			case 164:
				...
				if (EV_DoCeiling(line, fastCrushAndRaise)) P_ChangeSwitchTexture(line, 0);
				break;

			case 165:
				// Start slow silent crusher
				// 165 S1  EV_DoCeiling(silentCrushAndRaise)
				if (EV_DoCeiling(line, silentCrushAndRaise)) P_ChangeSwitchTexture(line, 0);
				break;
			...
			case 167:
				...
				if (EV_DoCeiling(line, lowerAndCrush)) P_ChangeSwitchTexture(line, 0);
				break;

			case 168:
				// Stop crusher
				// 168 S1 EV_CeilingCrushStop()
				if (EV_CeilingCrushStop(line)) P_ChangeSwitchTexture(line, 0);
				break;
			...
			case 174:
				...
				if (EV_Teleport(line, side, thing)) P_ChangeSwitchTexture(line, 0);
				break;

			case 175:
				// Close Door, Open in 30 secs
				// 175 S1  EV_DoDoor(close30ThenOpen)
				if (EV_DoDoor(line, close30ThenOpen)) P_ChangeSwitchTexture(line, 0);
				break;

			case 189: // jff 3/15/98 create texture change no motion type
				// Texture Change Only (Trigger)
				// 189 S1 Change Texture/Type Only
				if (EV_DoChange(line, trigChangeOnly)) P_ChangeSwitchTexture(line, 0);
				break;

			case 203:
				// Lower ceiling to lowest surrounding ceiling
				// 203 S1 EV_DoCeiling(lowerToLowest)
				if (EV_DoCeiling(line, lowerToLowest)) P_ChangeSwitchTexture(line, 0);
				break;

			case 204:
				// Lower ceiling to highest surrounding floor
				// 204 S1 EV_DoCeiling(lowerToMaxFloor)
				if (EV_DoCeiling(line, lowerToMaxFloor)) P_ChangeSwitchTexture(line, 0);
				break;

			case 209:
				// killough 1/31/98: silent teleporter
				// jff 209 S1 SilentTeleport
				if (EV_SilentTeleport(line, side, thing)) P_ChangeSwitchTexture(line, 0);
				break;

			case 241: // jff 3/15/98 create texture change no motion type
				// Texture Change Only (Numeric)
				// 241 S1 Change Texture/Type Only
				if (EV_DoChange(line, numChangeOnly)) P_ChangeSwitchTexture(line, 0);
				break;

			case 221:
				// Lower floor to next lowest floor
				// 221 S1 Lower Floor To Nearest Floor
				if (EV_DoFloor(line, lowerFloorToNearest)) P_ChangeSwitchTexture(line, 0);
				break;
			...
			case 78: // jff 3/15/98 create texture change no motion type
				...
				if (EV_DoChange(line, numChangeOnly)) P_ChangeSwitchTexture(line, 1);
				break;
			...
			case 177:
				...
				if (EV_DoFloor(line, lowerAndChange)) P_ChangeSwitchTexture(line, 1);
				break;

			case 178:
				// Raise Floor 512
				// 178 SR  EV_DoFloor(raiseFloor512)
				if (EV_DoFloor(line, raiseFloor512)) P_ChangeSwitchTexture(line, 1);
				break;
			...
			case 183:
				...
				if (EV_DoCeiling(line, fastCrushAndRaise)) P_ChangeSwitchTexture(line, 1);
				break;

			case 184:
				// Start slow crusher
				// 184 SR  EV_DoCeiling(crushAndRaise)
				if (EV_DoCeiling(line, crushAndRaise)) P_ChangeSwitchTexture(line, 1);
				break;
			...
			case 187:
				...
				if (EV_DoCeiling(line, lowerAndCrush)) P_ChangeSwitchTexture(line, 1);
				break;

			case 188:
				// Stop crusher
				// 188 SR EV_CeilingCrushStop()
				if (EV_CeilingCrushStop(line)) P_ChangeSwitchTexture(line, 1);
				break;

			case 190: // jff 3/15/98 create texture change no motion type
				// Texture Change Only (Trigger)
				// 190 SR Change Texture/Type Only
				if (EV_DoChange(line, trigChangeOnly)) P_ChangeSwitchTexture(line, 1);
				break;

			case 191:
				// Lower Pillar, Raise Donut
				// 191 SR  EV_DoDonut()
				if (EV_DoDonut(line)) P_ChangeSwitchTexture(line, 1);
				break;
			...
			case 195:
				...
				if (EV_Teleport(line, side, thing)) P_ChangeSwitchTexture(line, 1);
				break;

			case 196:
				// Close Door, Open in 30 secs
				// 196 SR  EV_DoDoor(close30ThenOpen)
				if (EV_DoDoor(line, close30ThenOpen)) P_ChangeSwitchTexture(line, 1);
				break;
			...
			case 206:
				...
				if (EV_DoCeiling(line, lowerToMaxFloor)) P_ChangeSwitchTexture(line, 1);
				break;

			case 210:
				// killough 1/31/98: silent teleporter
				// jff 210 SR SilentTeleport
				if (EV_SilentTeleport(line, side, thing)) P_ChangeSwitchTexture(line, 1);
				break;

			case 211: // jff 3/14/98 create instant toggle floor type
				// Toggle Floor Between C and F Instantly
				// 211 SR Toggle Floor Instant
				if (EV_DoPlat(line, toggleUpDn, 0)) P_ChangeSwitchTexture(line, 1);
				break;

			case 222:
				// Lower floor to next lowest floor
				// 222 SR Lower Floor To Nearest Floor
				if (EV_DoFloor(line, lowerFloorToNearest)) P_ChangeSwitchTexture(line, 1);
				break;
			...
			case 258:
				...
				if (EV_BuildStairs(line, build8)) P_ChangeSwitchTexture(line, 1);
				break;

			case 259:
				// Build stairs, step 16
				// 259 SR EV_BuildStairs(turbo16)
				if (EV_BuildStairs(line, turbo16)) P_ChangeSwitchTexture(line, 1);
				break;
	...
	case 42:
		// Close Door
		if (EV_DoDoor(line, closeDoor)) P_ChangeSwitchTexture(line, 1);
		break;
	...
	case 61:
		// Open Door
		if (EV_DoDoor(line, openDoor)) P_ChangeSwitchTexture(line, 1);
		break;
