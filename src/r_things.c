...
static void R_InstallSpriteLump(int lump, unsigned frame, char rot, dboolean flipped)
{
	...
	{    // the lump should be used for all rotations
		...
		for (r = 14; r >= 0; r -= 2)
			if (sprtemp[frame].lump[r] == -1)
			{
				...
				if (flipped) sprtemp[frame].flip |= (1 << r);
  ...
  // the lump is only used for one rotation
	...
	if (sprtemp[frame].lump[rotation] == -1)
	{
		...
		if (flipped) sprtemp[frame].flip |= (1 << rotation);
  ...
  //  noting the highest frame letter.
	...
	for (i = 0; i < numsprites; i++)
	{
		...
		if (j >= 0)
		{
			...
			for (k = 0; k < MAX_SPRITE_FRAMES; k++) sprtemp[k].flip = 0;
			...
			do {
              ...
              // (using int pointer cast is nonportable):
				...
				{
					R_InstallSpriteLump(
					  j + firstspritelump, lump->name[4] - 'A', lump->name[5], false);
					if (lump->name[6])
						R_InstallSpriteLump(
						  j + firstspritelump, lump->name[6] - 'A', lump->name[7], true);
				}
			...
			if ((sprites[i].numframes = ++maxframe))  // killough 1/31/98
			{
				...
				for (frame = 0; frame < maxframe; frame++)
				{
					...
					case 0:
						...
						if (sprtemp[frame].flip & 1) sprtemp[frame].flip = 0xFFFF;
						break;
...
void R_DrawMaskedColumn(const rpatch_t *patch, R_DrawColumn_f colfunc, draw_column_vars_t *dcvars,
  const rcolumn_t *column, const rcolumn_t *prevcolumn, const rcolumn_t *nextcolumn)
{
	int i;
	...
	for (i = 0; i < column->numPosts; i++)
	{
		...
		if (dcvars->yh >= mfloorclip[dcvars->x]) dcvars->yh = mfloorclip[dcvars->x] - 1;

		if (dcvars->yl <= mceilingclip[dcvars->x]) dcvars->yl = mceilingclip[dcvars->x] + 1;
...
static void R_DrawVisSprite(vissprite_t *vis)
{
	...
	if (!dcvars.colormap) // NULL colormap = shadow draw
		...
		else if (vis->mobjflags & MF_TRANSLUCENT && general_translucency) // phares
		{
			...
			colfunc = R_GetDrawColumnFunc(
			  RDC_PIPELINE_STANDARD, filter, filterz); // killough 3/14/98, 4/11/98

	// proff 11/06/98: Changed for high-res
	dcvars.iscale = FixedDiv(FRACUNIT, vis->scale);
	dcvars.texturemid = vis->texturemid;
	frac = vis->startfrac;
	if (filter == RDRAW_FILTER_LINEAR) frac -= (FRACUNIT >> 1);
	spryscale = vis->scale;
	...
	for (dcvars.x = vis->x1; dcvars.x <= vis->x2; dcvars.x++, frac += vis->xiscale)
	{
		...
		R_DrawMaskedColumn(patch, colfunc, &dcvars, R_GetPatchColumnClamped(patch, texturecolumn),
...
#ifdef GL_DOOM
	if ((V_GetMode() == VID_MODEGL) && (HaveMouseLook() || (render_fov > FOV90))
...
static void R_ProjectSprite(mobj_t *thing, int lightlevel)
{
	...
	if (FixedMul(globaluclip, tz2) > viewz - gzb || FixedMul(globaldclip, tz2) < viewz - gzt)
		return;
...
void R_AddSprites(subsector_t *subsec, int lightlevel)
{
	...
	if (compatibility_level <= boom_202_compatibility) lightlevel = sec->lightlevel;
	...
	if (show_alive)
	{
		if (show_alive == 1)
		{
			for (thing = sec->thinglist; thing; thing = thing->snext)
				if (!ALIVE(thing)) R_ProjectSprite(thing, lightlevel);
		}
	}
	else
#endif
...
static void R_ApplyWeaponBob(fixed_t *sx, dboolean bobx, fixed_t *sy, dboolean boby)
{
	...
	if (sx)
	{
		*sx = FRACUNIT;
		...
		if (bobx) *sx += FixedMul(viewplayer->bob, finecosine[angle]);
	}
	...
	if (sy)
	{
		*sy = 32 * FRACUNIT; // [crispy] WEAPONTOP
		...
		if (boby) *sy += FixedMul(viewplayer->bob, finesine[angle & (FINEANGLES / 2 - 1)]);
	}
}
...
static void R_DrawPSprite(pspdef_t *psp)
{
	...
	if ((psp->state->frame & FF_FRAMEMASK) >= sprdef->numframes)
		I_Error(
		  "R_ProjectSprite: Invalid sprite frame %i : %li", psp->state->sprite, psp->state->frame);
#endif
	...
	if (weapon_attack_alignment && viewplayer->attackdown && !psp->state->misc1)
	{
		...
		if (weapon_attack_alignment >= CENTERWEAPON_HORVER && state != winfo->downstate
		    && state != winfo->upstate)
		{
	...
	vis->texturemid = (BASEYCENTER << FRACBITS) /* +  FRACUNIT/2 */ - (psp_sy - topoffset);
	...
	if (movement_smooth)
	{
		...
		if (lump == psp_inter.lump)
		{
			...
			vis->texturemid = psp_inter.texturemid
			                + FixedMul(tic_vars.frac, (vis->texturemid - psp_inter.texturemid));
		}
		else
		{
			psp_inter.x1 = vis->x1;
	...
	if (V_GetMode() != VID_MODEGL) R_DrawVisSprite(vis);
#ifdef GL_DOOM
	else
	{
		int lightlevel;
		...
		{
			//      lightlevel = (viewplayer->mo->subsector->sector->lightlevel) + (extralight <<
			//      LIGHTSEGSHIFT);
			R_FakeFlat(viewplayer->mo->subsector->sector, &tmpsec, &floorlightlevel,
			  &ceilinglightlevel, false);
			lightlevel
			  = ((floorlightlevel + ceilinglightlevel) >> 1) + (extralight << LIGHTSEGSHIFT);
			if (!gl_hardware_gamma) lightlevel += usegamma * 16;
...
void R_SortVisSprites(void)
{
	if (num_vissprite)
	{
		...
		if (num_vissprite_ptrs < num_vissprite * 2)
		{
			...
			vissprite_ptrs
			  = malloc((num_vissprite_ptrs = num_vissprite_alloc * 2) * sizeof *vissprite_ptrs);
		}

		if (sprites_doom_order)
			while (--i >= 0) vissprite_ptrs[num_vissprite - i - 1] = vissprites + i;
		else
			while (--i >= 0) vissprite_ptrs[i] = vissprites + i;
...
static void R_DrawSprite(vissprite_t *spr)
{
	...
	for (x = spr->x1; x <= spr->x2; x++) clipbot[x] = -2;
	for (x = spr->x1; x <= spr->x2; x++) cliptop[x] = -2;
	...
	if (drawsegs_xrange_size)
	{
		...
		while (++curr <= last)
		{
			...
			if (curr->x1 > spr->x2 || curr->x2 < spr->x1) continue; // does not cover sprite
		    ...
		    && (h >>= FRACBITS) < viewheight)
		{
			...
			{ // clip bottom
				for (x = spr->x1; x <= spr->x2; x++)
					if (clipbot[x] == -2 || h < clipbot[x]) clipbot[x] = h;
			}
			else // clip top
				if (phs != -1 && viewz <= sectors[phs].floorheight) // killough 11/98
					for (x = spr->x1; x <= spr->x2; x++)
						if (cliptop[x] == -2 || h > cliptop[x]) cliptop[x] = h;
		}
...
void R_DrawMasked(void)
{
	...
	for (i = 0; i < DS_RANGES_COUNT; i++) drawsegs_xranges[i].count = 0;
	...
	if (num_vissprite > 0)
	{
		if (drawsegs_xrange_size < maxdrawsegs)
		{
			...
			{
				drawsegs_xranges[i].items = realloc(drawsegs_xranges[i].items,
	...
	if (!viewangleoffset && !viewpitchoffset) R_DrawPlayerSprites();
}
