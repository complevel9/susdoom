...
DIR *opendir(const _TCHAR *szPath)
{
   ...
   /* Allocate enough space to store DIR structure and the complete
	...
	nd = (DIR *)(malloc(
	  sizeof(DIR) + (_tcslen(szFullPath) + _tcslen(SLASH) + _tcslen(SUFFIX) + 1) * sizeof(_TCHAR)));
   ...
   /* Add on a slash if the path does not end with one. */
	if (nd->dd_name[0] != _T('\0')
	    && _tcsrchr(nd->dd_name, _T('/')) != nd->dd_name + _tcslen(nd->dd_name) - 1
	    && _tcsrchr(nd->dd_name, _T('\\')) != nd->dd_name + _tcslen(nd->dd_name) - 1)
	{
      ...
      /* Get the next search entry. */
		if (_tfindnext(dirp->dd_handle, &(dirp->dd_dta)))
		{
         /* We are off the end or otherwise error.
			...
			if (winerr == ERROR_NO_MORE_FILES) errno = 0;
...
int closedir(DIR *dirp)
{
	...
	if (dirp->dd_handle != -1) rc = _findclose(dirp->dd_handle);
...
void rewinddir(DIR *dirp)
{
	...
	if (dirp->dd_handle != -1) _findclose(dirp->dd_handle);
...
void seekdir(DIR *dirp, long lPos)
{
	...
	else
	{
      /* Rewind and read forward to the appropriate index. */
		...
		while ((dirp->dd_stat < lPos) && readdir(dirp)); /* do-nothing loop */
	}
}
