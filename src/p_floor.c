...
result_e T_MovePlane(
  sector_t *sector, fixed_t speed, fixed_t dest, dboolean crush, int floorOrCeiling, int direction)
{
	dboolean flag;
	fixed_t lastpos;
...
#ifdef GL_DOOM
	if (V_GetMode() == VID_MODEGL) gld_UpdateSplitData(sector);
#endif
      ...
      /* cph - make more compatible with original Doom, by
		...
		case 1:
          ...
          // jff 2/22/98 weaken check to demo_compatibility
			destheight
			  = (comp[comp_floors] || dest < sector->ceilingheight) ? dest : sector->ceilingheight;
        ...
        /* jff 1/25/98 fix floor crusher */
                ...
                // e6y: warning about potential desynch
						...
						{
							lprintf(LO_WARN, "T_MovePlane: Stairs which can potentially crush may "
							                 "lead to desynch in compatibility mode.\n");
							lprintf(LO_WARN, " gametic: %d, sector: %d, complevel: %d\n", gametic,
							  sector->iSectorID, compatibility_level);
						}
	...
	case 1:
      // moving a ceiling
		...
		case -1:
          ...
          // jff 2/22/98 weaken check to demo_compatibility
			destheight
			  = (comp[comp_floors] || dest > sector->floorheight) ? dest : sector->floorheight;
...
void T_MoveFloor(floormove_t *floor)
{
	...
	res = T_MovePlane       // move the floor
	  (floor->sector, floor->speed, floor->floordestheight, floor->crush, 0, floor->direction);
...
void T_MoveElevator(elevator_t *elevator)
{
	...
	if (elevator->direction < 0)      // moving down
	{
		...
		if (res == ok || res == pastdest) // jff 4/7/98 don't move ceil if blocked
			T_MovePlane(elevator->sector, elevator->speed, elevator->floordestheight, 0,
			  0,			            // move ceiling
			  elevator->direction);
	}
	else // up
	{
		...
		if (res == ok || res == pastdest) // jff 4/7/98 don't move floor if blocked
			T_MovePlane(elevator->sector, elevator->speed, elevator->ceilingdestheight, 0,
			  1,			            // move floor
			  elevator->direction);
	}

  // make floor move sound
	if (!(leveltime & 7)) S_StartSound((mobj_t *)&elevator->sector->soundorg, sfx_stnmov);
    ...
    /* jff 3/13/98 no ovf */
			...
			for (i = 0; i < sec->linecount; i++)
			{
              ...
              // jff 8/14/98 don't scan texture 0, its not real
					if (side->bottomtexture > 0 || (comp[comp_model] && !side->bottomtexture))
   ...
   // don't start a stair if the first step's floor is already moving
    ...
    // set up the speed and stepsize according to the stairs type
        ...
        // http://www.doomworld.com/idgames/index.php?id=5191
				else if (!prboom_comp[PC_UNINITIALIZE_CRUSH_FIELD_FOR_STAIRS].state)
...
int EV_DoDonut(line_t *line)
{
  ...
  // do function on all sectors with same tag as linedef
    ...
    /* do not start the donut if the pool is already moving
		...
		if (!comp[comp_floors] && P_SectorActive(floor_special, s2)) continue; // jff 5/7/98
