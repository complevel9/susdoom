...
mline_t player_arrow[] = {{{-R + R / 8, 0}, {R, 0}}, // -----
  {{R, 0}, {R - R / 2, R / 4}},  // ----->
  {{R, 0}, {R - R / 2, -R / 4}}, {{-R + R / 8, 0}, {-R - R / 8, R / 4}}, // >---->
  {{-R + R / 8, 0}, {-R - R / 8, -R / 4}}, {{-R + 3 * R / 8, 0}, {-R + R / 8, R / 4}}, // >>--->
  {{-R + 3 * R / 8, 0}, {-R + R / 8, -R / 4}}};
#undef R
...
static void AM_changeWindowLoc(void)
{
	...
	if (m_paninc.x || m_paninc.y) automapmode &= ~am_follow;
	...
	if (automapmode & am_rotate) AM_rotate(&incx, &incy, viewangle - ANG90);
...
// Passed nothing, returns nothing
//
void AM_clearMarks(void) { markpointnum = 0; }
...
static void AM_drawGrid(int color)
{
	...
	if (map_grid_size == -1)
	{
		...
		if (gridsize - oprtimal_gridsize > oprtimal_gridsize - (gridsize >> 1)) gridsize >>= 1;
	}
    ...
    // if line has been seen or IDDT has been used
        ...
        /* cph - show keyed doors and lines */
			...
			if /* jff 4/23/98 add exit lines to automap */
			  (mapcolor_exit
			    && (lines[i].special == 11 || lines[i].special == 52 || lines[i].special == 197
			        || lines[i].special == 51 || lines[i].special == 124
			        || lines[i].special == 198))
			{
        ...
        // jff 1/10/98 add color change for all teleporter types
				if (mapcolor_tele && !(lines[i].flags & ML_SECRET)
				    && (lines[i].special == 39 || lines[i].special == 97 || lines[i].special == 125
				        || lines[i].special == 126))
				{ // teleporters
				...
				else if (mapcolor_clsd && !(lines[i].flags & ML_SECRET) && // non-secret closed door
		...
		else if (plr->powers[pw_allmap]) // computermap visible lines
		{
			...
			{
				if (mapcolor_flat || !lines[i].backsector
...
static void AM_drawLineCharacter(
  mline_t *lineguy, int lineguylines, fixed_t scale, angle_t angle, int color, fixed_t x, fixed_t y)
{
	int i;
	mline_t l;
	...
	if (!paused && movement_smooth)
	{
		...
		if (mo->player)
			*angle = mo->player->prev_viewangle
			       + FixedMul(
			         tic_vars.frac, R_SmoothPlaying_Get(mo->player) - mo->player->prev_viewangle);
		else
			*angle = mo->angle;
	}
	else
...
static void AM_drawPlayers(void)
{
	...
	if (!netgame)
	{
		...
		if (ddt_cheating)
			AM_drawLineCharacter(
			  cheat_player_arrow, NUMCHEATPLYRLINES, scale, viewangle, mapcolor_sngl, pt.x, pt.y);
		else
			AM_drawLineCharacter(
			  player_arrow, NUMPLYRLINES, scale, viewangle, mapcolor_sngl, pt.x, pt.y);
		return;
	}
...
static void AM_ProcessNiceThing(mobj_t *mobj, angle_t angle, fixed_t x, fixed_t y)
{
	...
	static const map_nice_icon_param_t icons[] = {{SPR_STIM, am_icon_health, 12, 0, 100, 100, 200},
	  {SPR_MEDI, am_icon_health, 16, 0, 100, 100, 200},
	  {SPR_BON1, am_icon_health, 10, 0, 0, 0, 200},

	  {SPR_BON2, am_icon_armor, 10, 0, 0, 200, 0}, {SPR_ARM1, am_icon_armor, 16, 0, 100, 200, 100},
	  {SPR_ARM2, am_icon_armor, 16, 0, 100, 100, 200},

	  {SPR_CLIP, am_icon_ammo, 10, 0, 180, 150, 50}, {SPR_AMMO, am_icon_ammo, 16, 0, 180, 150, 50},
	  {SPR_ROCK, am_icon_ammo, 10, 0, 180, 150, 50}, {SPR_BROK, am_icon_ammo, 16, 0, 180, 150, 50},

	  {SPR_CELL, am_icon_ammo, 10, 0, 180, 150, 50}, {SPR_CELP, am_icon_ammo, 16, 0, 180, 150, 50},
	  {SPR_SHEL, am_icon_ammo, 10, 0, 180, 150, 50}, {SPR_SBOX, am_icon_ammo, 16, 0, 180, 150, 50},
	  {SPR_BPAK, am_icon_ammo, 16, 0, 180, 150, 50},

	  {SPR_BKEY, am_icon_key, 10, 0, 0, 0, 255}, {SPR_BSKU, am_icon_key, 10, 0, 0, 0, 255},
	  {SPR_YKEY, am_icon_key, 10, 0, 255, 255, 0}, {SPR_YSKU, am_icon_key, 10, 0, 255, 255, 0},
	  {SPR_RKEY, am_icon_key, 10, 0, 255, 0, 0}, {SPR_RSKU, am_icon_key, 10, 0, 255, 0, 0},
	  ...
	  {SPR_BLUD, am_icon_bullet, 8, 0, 255, 0, 0}, {SPR_PUFF, am_icon_bullet, 8, 0, 255, 255, 115},
	  {SPR_MISL, am_icon_bullet, 8, 0, 91, 71, 43}, {SPR_PLSS, am_icon_bullet, 8, 0, 115, 115, 255},
	...
	if (fx + shadow_radius < 0 || fx - shadow_radius > (float)SCREENWIDTH || fy + shadow_radius < 0
	...
	if (need_shadow) gld_AddNiceThing(am_icon_shadow, fx, fy, shadow_radius, rot, 0, 0, 0, 128);
#endif
}
...
static void AM_DrawNiceThings(void)
{
	...
	// marked locations on the automap
	{
		...
		if (anim_flash_level >= 16) anim_flash_level = 32 - anim_flash_level;
		...
		for (pass = 0; pass < 2; pass += (enemies ? 1 : 2))
		{
			...
			while (t) // for all things in that sector
			{
				...
				if (mapcolor_rkey || mapcolor_ykey || mapcolor_bkey)
				{
					...
					{
						AM_drawLineCharacter(
						  cross_mark, NUMCROSSMARKLINES, scale, t->angle, color, p.x, p.y);
						t = t->snext;
						continue;
					}
				}
				...
				AM_drawLineCharacter(arrowhead_guy, NUMARROWHEADGUYLINES, scale, angle,
				  t->flags & MF_FRIEND && !t->player
				    ? mapcolor_frnd
				    :
				...
				AM_drawLineCharacter(square_guy, NUMSQUAREGUYLINES, t->radius >> FRACTOMAPBITS, 0,
		...
		if (markpoints[i].x != -1)
		{
			      ...
			      : p.y < f_y + f_h && p.y >= f_y)
			{
				...
				for (k = 0; k < (int)strlen(markpoints[i].label); k++)
				{
					...
					if (p.x < f_x + f_w && p.x + markpoints[i].widths[k] * SCREENWIDTH / 320 >= f_x)
					{
						float fx, fy;
						int x, y, flags;
					...
					if (am_frame.precise) p.fx += (float)w * SCREENWIDTH / 320.0f;
				}
			}
		}
	}
...
static void AM_drawLineTraces(void)
{
	for (unsigned short i = 0; i < NUMAMLINETRACES; i++)
	{
		...
		{
			int color;
			mline_t pathline = {{p->x1 >> FRACTOMAPBITS, p->y1 >> FRACTOMAPBITS},
...
static void AM_drawRectTraces(void)
{
	for (unsigned short i = 0; i < NUMAMLINETRACES; i++)
	{
		...
		{
			int color;
			mpoint_t rectpts[4] = {{p->x1 >> FRACTOMAPBITS, p->y1 >> FRACTOMAPBITS},
			  {p->x2 >> FRACTOMAPBITS, p->y1 >> FRACTOMAPBITS},
			  {p->x2 >> FRACTOMAPBITS, p->y2 >> FRACTOMAPBITS},
			  {p->x1 >> FRACTOMAPBITS, p->y2 >> FRACTOMAPBITS}};

			if (automapmode & am_rotate)
				for (short j = 0; j < 4; j++) AM_rotatePoint(&rectpts[j]);
			else
				for (short j = 0; j < 4; j++) AM_SetMPointFloatValue(&rectpts[j]);
...
{
	if (map_grid_size > 0) map_grid_size = MAX(map_grid_size, 1);
}

void M_ChangeMapTextured(void)
{
#ifdef GL_DOOM
	if (V_GetMode() == VID_MODEGL) gld_ProcessTexturedMap();
#endif
}

void M_ChangeMapMultisamling(void)
{
	if (map_use_multisamling && V_GetMode() != VID_MODEGL) V_InitFlexTranTable();
}
...
void AM_Drawer(void)
{
	...
	if (!(automapmode & am_overlay)) // cph - If not overlay mode, clear background for the automap
		V_FillRect(
		  FB, f_x, f_y, f_w, f_h, (byte)mapcolor_back); // jff 1/5/98 background default color

	if (map_textured) AM_drawSubsectors();
