  ...
  // early out?
    ...
    // initialize, or handle moving sector
      ...
      // calculate adjustment
			while ((height >>= 1)) frontsector->scaleindex++;
		}
...
void R_RenderMaskedSegRange(drawseg_t *ds, int x1, int x2)
{
  ...
  // killough 4/11/98: draw translucent 2s normal textures
	...
	{
		colfunc
		  = R_GetDrawColumnFunc(RDC_PIPELINE_TRANSLUCENT, drawvars.filterwall, drawvars.filterz);
  ...
  // cph 2001/11/25 - middle textures did not animate in v1.2
	...
	if (!comp[comp_maskedanim]) texnum = texturetranslation[texnum];
  ...
  // draw the columns
	for (dcvars.x = x1; dcvars.x <= x2; dcvars.x++, spryscale += rw_scalestep)
        ...
        // calculate texture offset - POPE
			...
			if (drawvars.filterwall == RDRAW_FILTER_LINEAR) dcvars.texu -= (FRACUNIT >> 1);

			if (!fixedcolormap) dcvars.z = spryscale; // for filtering -- POPE
        ...
        // mapping to screen coordinates is totally out of range:
			...
			{
				int_64_t t
				  = ((int_64_t)centeryfrac << FRACBITS) - (int_64_t)dcvars.texturemid * spryscale;
  ...
  // Except for main_tranmap, mark others purgable at this point
	...
	curline = NULL; /* cph 2001/11/18 - must clear curline now we're done with it, so R_ColourMap
	                   doesn't try using it for other things */
}
...
static void R_RenderSegLoop(void)
{
	...
	R_DrawColumn_f colfunc
	  = R_GetDrawColumnFunc(RDC_PIPELINE_STANDARD, drawvars.filterwall, drawvars.filterz);
	draw_column_vars_t dcvars;
...
//      yh = bottomfrac>>HEIGHTBITS;
      ...
      // texturecolumn and lighting are independent of wall tiers
		if (segtextured)
		{
          // calculate texture offset
			...
			if (drawvars.filterwall == RDRAW_FILTER_LINEAR) texturecolumn -= (FRACUNIT >> 1);
    ...
    // add this info to the solid columns array for r_bsp.c
          ...
          // save texturecol for backdrawing of masked mid texture
			if (maskedtexture) maskedtexturecol[rw_x] = texturecolumn;
		}
...
static fixed_t R_PointToDist(fixed_t x, fixed_t y)
{
	...
	return FixedDiv(
	  dx, finesine[(tantoangle[FixedDiv(dy, dx) >> DBITS] + ANG90) >> ANGLETOFINESHIFT]);
}
...
#ifdef RANGECHECK
	if (start >= viewwidth || start > stop) I_Error("Bad R_RenderWallRange: %i to %i", start, stop);
#endif
  ...
  // shift right to avoid possibility of int64 overflow in rw_distance calculation
	...
	{     // killough 1/6/98, 2/1/98: remove limit on openings
		...
		if (need > maxopenings)
		{
			...
			do maxopenings = maxopenings ? maxopenings * 2 : 16384;
...
#undef ADJUST
  ...
  // from being displayed on the automap.
      ...
      // hack to allow height changes in outdoor areas
		...
		markfloor = worldlow != worldbottom || backsector->floorpic != frontsector->floorpic
		...
		markceiling = worldhigh != worldtop || backsector->ceilingpic != frontsector->ceilingpic
		           || backsector->lightlevel != frontsector->lightlevel
		...
		if (worldhigh < worldtop)   // top texture
		{
			...
			toptexheight
			  = (linedef->r_flags & RF_TOP_TILE) ? 0 : textureheight[toptexture] >> FRACBITS;
			rw_toptexturemid
			  = linedef->flags & ML_DONTPEGTOP
			    ? worldtop
		...
		if (worldlow > worldbottom) // bottom texture
		{
			...
			bottomtexheight
			  = (linedef->r_flags & RF_BOT_TILE) ? 0 : textureheight[bottomtexture] >> FRACBITS;
  ...
  // calculate incremental stepping values for texture edges
	...
	topfrac = ((int_64_t)centeryfrac >> invhgtbits)
	        - (((int_64_t)worldtop * rw_scale) >> FRACBITS); // R_WiggleFix

	bottomstep = -FixedMul(rw_scalestep, worldbottom);
	bottomfrac = ((int_64_t)centeryfrac >> invhgtbits)
	           - (((int_64_t)worldbottom * rw_scale) >> FRACBITS); // R_WiggleFix
	...
	if (backsector)
	{
		...
		{
			pixhigh = ((int_64_t)centeryfrac >> invhgtbits)
			        - (((int_64_t)worldhigh * rw_scale) >> FRACBITS); // R_WiggleFix
			pixhighstep = -FixedMul(rw_scalestep, worldhigh);
		}
		if (worldlow > worldbottom)
		{
			pixlow = ((int_64_t)centeryfrac >> invhgtbits)
			       - (((int_64_t)worldlow * rw_scale) >> FRACBITS); // R_WiggleFix
