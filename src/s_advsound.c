...
void S_ParseMusInfo(const char *mapid)
{
    ...
    /* don't restart music that is already playing */
		if (mus_playing && mus_playing->lumpnum == S_music[NUMMUSIC].lumpnum)
		...
		while (SC_GetString())
		{
			if (inMap || SC_Compare(mapid))
			{
				...
				if (sc_String[0] == 'E' || sc_String[0] == 'e' || sc_String[0] == 'M'
				    || sc_String[0] == 'm')
				{
					break;
				}
        ...
        // Check number in range
				...
				{
					lprintf(
					  LO_ERROR, "S_ParseMusInfo: Number not in range 1 to %d", MAX_MUS_ENTRIES - 1);
				}
			}
		}
...
{
	if (musinfo.tics < 0 || !musinfo.mapthing) return;

	if (musinfo.tics > 0) { musinfo.tics--; }
	else if (!musinfo.tics && musinfo.lastmapthing != musinfo.mapthing)
	{
		...
		if (arraypt >= 0 && arraypt < MAX_MUS_ENTRIES)
		{
			...
			if (lumpnum >= 0 && lumpnum < numlumps) S_ChangeMusInfoMusic(lumpnum, true);
		}
