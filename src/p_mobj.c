...
static void P_XYMovement(mobj_t *mo)
{
  ...
  fixed_t   oldx,oldy; // phares 9/10/98: reducing bobbing/momentum on ice
    ...
    // Add ability for objects other than players to bounce on ice
			...
			if (!(mo->flags & MF_MISSILE) && mbf_features
			    && (mo->flags & MF_BOUNCES
			        || (!player && blockline && variable_friction && mo->z <= mo->floorz
  ...
  /* no friction for missiles or skulls ever, no friction when airborne */
	...
	if (mo->z > mo->floorz && !(mo->flags & MF_FLY)) return;
  ...
  // moving corresponding player, except in old demos:
	        ...
	        || (player->mo != mo && compatibility_level >= lxdoom_1_compatibility)))
	{
      ...
      /* killough 10/98: kill any bobbing momentum too (except in voodoo dolls)
		 ...
		 */
		if (player && player->mo == mo) player->momx = player->momy = 0;
	}
	else
	{
      /* phares 3/17/98
...
static void P_ZMovement(mobj_t *mo)
{
  /* killough 7/11/98:
    ...
    /* Bring it to rest below a certain speed */
					if (D_abs(mo->momz) <= mo->info->mass * (GRAVITY * 4 / 256)) mo->momz = 0;
				}
...
floater:
  ...
  /* killough 11/98: touchy objects explode on impact */
			...
			{
				if (mo->player) mo->player->jumpTics = 7;
        ...
        // but can be applied globally for all demo_compatibility complevels,
				    ...
				    (demo_compatibility || mo->player->mo == mo) && mo->momz < -GRAVITY * 8)
				{
					...
					if (default_comp[comp_sound]
					    || (mo->health > 0)) /* cph - prevent "oof" when dead */
...
void P_MobjThinker(mobj_t *mobj)
{
	...
	if (mobj->tics != -1)
	{
		...
		if (!mobj->tics)
			if (!P_SetMobjState(mobj, mobj->state->nextstate)) return; // freed itself
	}
	else
	{
		...
		if (P_Random(pr_respawn) > 4) return;
...
mobj_t *P_SpawnMobj(fixed_t x, fixed_t y, fixed_t z, mobjtype_t type)
{
	...
	if (!((mobj->flags ^ MF_COUNTKILL) & (MF_FRIEND | MF_COUNTKILL))) totallive++;
	return mobj;
}
...
void P_RemoveMobj(mobj_t *mobj)
{
	...
	if ((compatibility_level >= lxdoom_1_compatibility) || (!demorecording && !demoplayback))
	{
...
static PUREFUNC int P_FindDoomedNum(unsigned type)
{
	...
	if (!hash)
	{
		...
		for (i = 0; i < NUMMOBJTYPES; i++) hash[i].first = NUMMOBJTYPES;
	...
	while ((i < NUMMOBJTYPES) && ((unsigned)mobjinfo[i].doomednum != type)) i = hash[i].next;
	return i;
}
...
void P_SpawnPlayer(int n, const mapthing_t *mthing)
{
	...
	if (PlayeringameOverrun(mthing)) return;

	// not playing?

	if (!playeringame[n]) return;

	p = &players[n];

	if (p->playerstate == PST_REBORN) G_PlayerReborn(n);
...
dboolean P_IsDoomnumAllowed(int doomnum)
{
	...
	if (gamemode != commercial) switch (doomnum)
		{
		case 64: // Archvile
...
mobj_t *P_SpawnMapThing(const mapthing_t *mthing, int index)
{
	...
	if (thingtype == 11)
	{
		if (!(!compatibility || deathmatch_p - deathmatchstarts < 10)) { return NULL; }
		else
		{
			...
			if (compatibility && deathmatch_p - deathmatchstarts >= 10) return NULL; // e6y
			if (offset >= num_deathmatchstarts)
			{
				num_deathmatchstarts = num_deathmatchstarts ? num_deathmatchstarts * 2 : 16;
				deathmatchstarts
				  = realloc(deathmatchstarts, num_deathmatchstarts * sizeof(*deathmatchstarts));
	...
	if (thingtype <= 4 && thingtype > 0) // killough 2/26/98 -- fix crashes
	{
		...
		{ // use secretcount to avoid multiple dogs in case of multiple starts
			...
			if (HelperThing != -1) // haleyjd 9/22/99: deh substitution
			{
				...
				if (type >= 0 && type < NUMMOBJTYPES) { i = type; }
				else
				{
		...
		if (!deathmatch) P_SpawnPlayer(thingtype - 1, &playerstarts[thingtype - 1]);
		return NULL;
	}
	...
	if (!coop_spawns && !netgame && options & MTF_NOTSINGLE) return NULL;

	// jff 3/30/98 implement "not deathmatch" thing flag

	if (netgame && deathmatch && options & MTF_NOTDM) return NULL;

	// jff 3/30/98 implement "not cooperative" thing flag

	if ((coop_spawns || netgame) && !deathmatch && options & MTF_NOTCOOP) return NULL;
	...
	if (deathmatch && mobjinfo[i].flags & MF_NOTDMATCH) return NULL;

	// don't spawn any monsters if -nomonsters

	if (nomonsters && (i == MT_SKULL || (mobjinfo[i].flags & MF_COUNTKILL))) return NULL;
...
spawnit:
	...
	if (mobj->tics > 0) mobj->tics = 1 + (P_Random(pr_spawnthing) % mobj->tics);
	...
	if (!((mobj->flags ^ MF_COUNTKILL) & (MF_FRIEND | MF_COUNTKILL))) totalkills++;

	if (mobj->flags & MF_COUNTITEM) totalitems++;

	mobj->angle = ANG45 * (mthing->angle / 45);
	if (options & MTF_AMBUSH) mobj->flags |= MF_AMBUSH;
...
void P_CheckMissileSpawn(mobj_t *th)
{
	...
	if (!(th->flags & MF_MISSILE) && mbf_features) return;

	// killough 3/15/98: no dropoff (really = don't care for missiles)

	if (!P_TryMove(th, th->x, th->y, false)) P_ExplodeMissile(th);
}
