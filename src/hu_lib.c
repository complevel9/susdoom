  ...
  // draw the new stuff
	...
	for (i = 0; i < l->len; i++)
	{
		...
		{			        // jff 3/26/98 changed to actual escape char
			if (++i < l->len)
				if (l->l[i] >= '0' && l->l[i] <= '9') l->cm = l->l[i] - '0';
		}
		else if (c != ' ' && c >= l->sc && c <= 127)
		{
			w = l->f[c - l->sc].width;
			if (x + w - l->f[c - l->sc].leftoffset > BASE_WIDTH) break;
      // killough 1/18/98 -- support multiple lines:
...
// Returns nothing
//
void HUlib_initSText(hu_stext_t *s, int x, int y, int h, const patchnum_t *font, int startchar,
  ...
  enum patch_translation_e flags, dboolean *on)
{
	...
	s->cl = 0;
	for (i = 0; i < h; i++)
		HUlib_initTextLine(&s->l[i], x, y - i * (font[0].height + 1), font, startchar, cm, flags);
}
...
static void HUlib_addLineToSText(hu_stext_t *s)
{
  ...
  // everything needs updating
	for (i = 0; i < s->h; i++) s->l[i].needsupdate = 4;
}
...
void HUlib_addMessageToSText(hu_stext_t *s, const char *prefix, const char *msg)
{
	...
	if (prefix)
		while (*prefix) HUlib_addCharToTextLine(&s->l[s->cl], *(prefix++));
  ...
  int startchar, int cm, const patchnum_t *bgfont, enum patch_translation_e flags, dboolean *on)
{
	...
	{
		HUlib_initTextLine(&m->l[i], x, y + (hud_list_bgon ? i + 1 : i) * HU_REFRESHSPACING, font,
		  startchar, cm, flags);
	}
}
...
{
  // add a clear line
	if (++m->cl == hud_msg_lines) m->cl = 0;
	HUlib_clearTextLine(&m->l[m->cl]);

	if (m->nl < hud_msg_lines) m->nl++;
...
void HUlib_addMessageToMText(hu_mtext_t *m, const char *prefix, const char *msg)
{
	...
	if (prefix)
		while (*prefix) HUlib_addCharToTextLine(&m->l[m->cl], *(prefix++));

	while (*msg) HUlib_addCharToTextLine(&m->l[m->cl], *(msg++));
}
...
void HUlib_drawMText(hu_mtext_t *m)
{
	...
	if (!*m->on) return; // if not on, don't draw

  // draw everything
	if (hud_list_bgon) HUlib_drawMBg(m->x, m->y, m->w, m->h, m->bg);
	y = m->y + HU_REFRESHSPACING;
	for (i = 0; i < m->nl; i++)
	{
		idx = m->cl - i;
		if (idx < 0) idx += m->nl; // handle queue of lines
...
void HUlib_eraseMText(hu_mtext_t *m)
{
	...
	if (hud_list_bgon) HUlib_eraseMBg(m);
...
// Returns nothing
//
void HUlib_initIText(hu_itext_t *it, int x, int y, const patchnum_t *font, int startchar,
  int cm,   // jff 2/16/98 add color range parameter
  enum patch_translation_e flags, dboolean *on)
{
...
{
	while (*str) HUlib_addCharToTextLine(&it->l, *(str++));
	it->lm = it->l.len;
}
