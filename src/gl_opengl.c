...
void gld_InitOpenGL(dboolean compatibility_mode)
{
  ...
  //
	...
	gl_arb_multitexture
	  = gl_arb_multitexture_default && isExtensionSupported("GL_ARB_multitexture") != NULL;
  ...
  //
	...
	if (gl_arb_texture_compression) lprintf(LO_INFO, "using GL_ARB_texture_compression\n");
  ...
  //
	...
	if (gl_ext_framebuffer_object) lprintf(LO_INFO, "using GL_EXT_framebuffer_object\n");

	gl_ext_packed_depth_stencil = gl_ext_packed_depth_stencil_default
	                           && isExtensionSupported("GL_EXT_packed_depth_stencil") != NULL;
	if (gl_ext_packed_depth_stencil) lprintf(LO_INFO, "using GL_EXT_packed_depth_stencil\n");
  ...
  //
	...
	gl_ext_blend_color
	  = gl_ext_blend_color_default && isExtensionSupported("GL_EXT_blend_color") != NULL;
  ...
  // VBO
	...
	if (gl_ext_arb_vertex_buffer_object)
	{
		...
		if (!GLEXT_glGenBuffersARB || !GLEXT_glDeleteBuffersARB || !GLEXT_glBindBufferARB
		    || !GLEXT_glBufferDataARB)
			gl_ext_arb_vertex_buffer_object = false;
	}
	if (gl_ext_arb_vertex_buffer_object) lprintf(LO_INFO, "using GL_ARB_vertex_buffer_object\n");
#else
	gl_ext_arb_vertex_buffer_object = false;
#endif
	...
	if (gl_arb_pixel_buffer_object)
	{
		...
		if (!GLEXT_glGenBuffersARB || !GLEXT_glBindBufferARB || !GLEXT_glBufferDataARB
		    || !GLEXT_glBufferSubDataARB || !GLEXT_glDeleteBuffersARB
		    || !GLEXT_glGetBufferParameterivARB || !GLEXT_glMapBufferARB || !GLEXT_glUnmapBufferARB)
			gl_arb_pixel_buffer_object = false;
	}
	if (gl_arb_pixel_buffer_object) lprintf(LO_INFO, "using GL_ARB_pixel_buffer_object\n");
  ...
  //
#ifdef USE_SHADERS
	gl_arb_shader_objects = gl_arb_shader_objects_default && (gl_version >= OPENGL_VERSION_2_0)
	...
	if (gl_arb_shader_objects)
	{
		...
		if (!GLEXT_glDeleteObjectARB || !GLEXT_glGetHandleARB || !GLEXT_glDetachObjectARB
		    || !GLEXT_glCreateShaderObjectARB || !GLEXT_glShaderSourceARB
		    || !GLEXT_glCompileShaderARB || !GLEXT_glCreateProgramObjectARB
		    || !GLEXT_glAttachObjectARB || !GLEXT_glLinkProgramARB || !GLEXT_glUseProgramObjectARB
		    || !GLEXT_glValidateProgramARB || !GLEXT_glUniform1fARB || !GLEXT_glUniform2fARB
		    || !GLEXT_glUniform1iARB || !GLEXT_glGetObjectParameterfvARB
...
void gld_EnableTexture2D(GLenum texture, int enable)
{
...
#ifdef RANGECHECK
	if (arb < 0 || arb > 31) I_Error("gld_EnableTexture2D: wronge ARB texture unit %d", arb);
#endif
	...
	else if (active_texture_enabled[arb])
	{
		if (arb != 0)
		{
			GLEXT_glActiveTextureARB(texture);
...
void gld_EnableClientCoordArray(GLenum texture, int enable)
{
	...
	else if (clieant_active_texture_enabled[arb])
	{
		GLEXT_glClientActiveTextureARB(texture);
...
void gld_EnableMultisample(int enable)
{
	...
	else if (multisample_is_enabled)
	{
...
{
	if (gl_compatibility_mode) type = TM_MODULATE;
