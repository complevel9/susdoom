...
static dboolean WriteTime(unsigned int time, MEMFILE *midioutput)
{
	...
	for (;;)
	{
		...
		if (mem_fwrite(&writeval, 1, 1, midioutput) != 1) return true;
...
static dboolean WriteEndTrack(MEMFILE *midioutput)
{
	...
	if (WriteTime(queuedtime, midioutput)) return true;

	if (mem_fwrite(endtrack, 1, 3, midioutput) != 3) return true;
...
static dboolean WritePressKey(byte channel, byte key, byte velocity, MEMFILE *midioutput)
{
	...
	if (mem_fwrite(&working, 1, 1, midioutput) != 1) return true;
...
static int GetMIDIChannel(int mus_channel)
{
  ...
  // MUS channel 15 is the percusssion channel.
	...
	if (mus_channel == MUS_PERCUSSION_CHAN) { return MIDI_PERCUSSION_CHAN; }
	else
	{
    // If a MIDI channel hasn't been allocated for this MUS channel
    // yet, allocate the next free MIDI channel.

		if (channel_map[mus_channel] == -1) channel_map[mus_channel] = AllocateMIDIChannel();
...
dboolean mus2mid(MEMFILE *musinput, MEMFILE *midioutput)
{
  ...
  // Initialise channel map to mark all channels as unused.
	...
	for (channel = 0; channel < NUM_CHANNELS; ++channel) channel_map[channel] = -1;

  // Grab the header

	if (!ReadMusHeader(musinput, &musfileheader)) return true;
  ...
  // Seek to where the data is held
	if (mem_fseek(musinput, (long)musfileheader.scorestart, MEM_SEEK_SET) != 0) return true;
  ...
  // Now, process the MUS file:
    ...
    // Handle a block of events:
      ...
      // Fetch channel number and event code:
			...
			if (mem_fread(&eventdescriptor, 1, 1, musinput) != 1) return true;
			...
			case mus_presskey:
				...
				if (key & 0x80)
				{
					if (mem_fread(&channelvelocities[channel], 1, 1, musinput) != 1) return true;
			...
			case mus_changecontroller:
				...
				{
					if (WriteChangePatch(channel, controllervalue, midioutput)) return true;
				}
				else
				{
					if (controllernumber < 1 || controllernumber > 9) return true;
    ...
    // Now we need to read the time code:
		if (!hitscoreend)
		{
			...
			for (;;)
			{
				if (mem_fread(&working, 1, 1, musinput) != 1) return true;
  ...
  // End of track
	if (WriteEndTrack(midioutput)) return true;

  // Write the track size into the stream
	if (mem_fseek(midioutput, 18, MEM_SEEK_SET)) return true;
	...
	if (mem_fwrite(tracksizebuffer, 1, 4, midioutput) != 4) return true;
