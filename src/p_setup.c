...
{
	if (!samelevel || !p) return malloc(size);
	return p;
}
...
{
	if (!samelevel) { return calloc(n1, n2); }
	else
	{
		memset(p, 0, n1 * n2);
		return p;
...
static dboolean CheckForIdentifier(int lumpnum, const byte *id, size_t length)
{
	...
	if (W_LumpLength(lumpnum) >= length)
	{
		...
		if (!memcmp(data, id, length)) result = true;
...
static int P_CheckForZDoomUncompressedNodes(int lumpnum, int gl_lumpnum)
{
	...
	else
	{
		...
		{
			lprintf(
			  LO_INFO, "P_CheckForZDoomUncompressedNodes: compressed ZDoom nodes are detected\n");
...
static void P_GetNodesVersion(int lumpnum, int gl_lumpnum)
{
	    ...
	    && (compatibility_level >= prboom_2_compatibility))
	{
		...
		{
			if (CheckForIdentifier(gl_lumpnum + ML_GL_SEGS, "gNd3", 4)) { ver = 3; }
			else
			{
				nodesVersion = 2;
		...
		else if (CheckForIdentifier(gl_lumpnum + ML_GL_VERTS, "gNd4", 4)) { ver = 4; }
		else if (CheckForIdentifier(gl_lumpnum + ML_GL_VERTS, "gNd5", 4)) { ver = 5; }
...
{
	if (num & 0x8000) num = (num & 0x7FFF) + firstglvertex;
	return num;
}
...
static void P_LoadSegs(int lump)
{
	...
	if ((!data) || (!numsegs)) I_Error("P_LoadSegs: no segs in level");
	...
	for (i = 0; i < numsegs; i++)
	{
      ...
      // e6y: check for wrong indexes
		...
		{
			I_Error(
			  "P_LoadSegs: seg %d references a non-existent linedef %d", i, (unsigned)linedef);
		}
      ...
      /* cph 2006/09/30 - our frontsector can be the second side of the
		...
		if (ldef->flags & ML_TWOSIDED)
		{
          ...
          // this is wrong
			...
			else { li->backsector = sides[sidenum].sector; }
		}
      ...
      // http://www.doomworld.com/idgames/index.php?id=12647
		if (v1 >= numvertexes || v2 >= numvertexes)
		{
			...
			if (v1 >= numvertexes) lprintf(LO_WARN, str, i, v1);
			if (v2 >= numvertexes) lprintf(LO_WARN, str, i, v2);
...
static void P_LoadSegs_V4(int lump)
{
	...
	if ((!data) || (!numsegs)) I_Error("P_LoadSegs_V4: no segs in level");
    ...
    // e6y: check for wrong indexes
		...
		{
			I_Error(
			  "P_LoadSegs_V4: seg %d references a non-existent linedef %d", i, (unsigned)linedef);
		}
...
static void P_LoadGLSegs(int lump)
{
	...
	if ((!ml) || (!numsegs)) I_Error("P_LoadGLSegs: no glsegs in level");
...
{
	/* cph 2006/07/29 - make data a const mapsubsector_t *, so the loop below is simpler & gives no
	 * constness warnings */
	...
	if ((!data) || (!numsubsectors)) I_Error("P_LoadSubsectors: no subsectors in level");
...
static void P_LoadSubsectors_V4(int lump)
{
	...
	if ((!data) || (!numsubsectors)) I_Error("P_LoadSubsectors_V4: no subsectors in level");
...
static void P_LoadNodes(int lump)
{
	...
	for (i = 0; i < numnodes; i++)
	{
		...
		for (j = 0; j < 2; j++)
		{
			...
			// e6y: support for extended nodes
			if (no->children[j] == 0xFFFF) { no->children[j] = -1; }
			...
			for (k = 0; k < 4; k++) no->bbox[j][k] = LittleShort(mn->bbox[j][k]) << FRACBITS;
		}
	}
...
static void CheckZNodesOverflow(int *size, int count)
{
	...
	if ((*size) < 0) I_Error("P_LoadZNodes: incorrect nodes");
}
...
static void P_LoadZSegs(const byte *data)
{
	...
	for (i = 0; i < numsegs; i++)
	{
		...
		{
			I_Error(
			  "P_LoadZSegs: seg %d references a non-existent linedef %d", i, (unsigned)linedef);
		}
		...
		// li->angle = (int)((float)atan2(li->v2->y - li->v1->y,li->v2->x - li->v1->x) * (ANG180 /
		// M_PI));
	}
}
...
static void P_LoadZNodes(int lump, int glnodes, int compressed)
{
	...
	if (compressed == ZDOOM_ZNOD_NODES)
	{
		...
		if (err != Z_STREAM_END) I_Error("P_LoadZNodes: Error during ZDoom nodes decompression!");
	...
	if (!samelevel)
	{
		if (orgVerts + newVerts == (unsigned int)numvertexes) { newvertarray = vertexes; }
		else
		{
		...
		CheckZNodesOverflow(
		  &len, newVerts * (sizeof(newvertarray[0].x) + sizeof(newvertarray[0].y)));
	...
	if (numsubsectors <= 0) I_Error("P_LoadZNodes: no subsectors in level");
	subsectors = calloc_IfSameLevel(subsectors, numsubsectors, sizeof(subsector_t));
	...
	if (numSegs != currSeg) I_Error("P_LoadZNodes: Incorrect number of segs in nodes.");
...
static void P_LoadThings(int lump)
{
	...
	if ((!data) || (!numthings)) I_Error("P_LoadThings: no things in level");
	...
	for (i = 0; i < numthings; i++)
	{
		...
		if (!P_IsDoomnumAllowed(mt.type)) continue;
		...
		if (bfgedition && singleplayer && mt.type == 84) mt.type = 3004;

		// Do spawn all other stuff.
		mobj = P_SpawnMapThing(&mt, i);
		if (mobj && mobj->info->speed == 0) mobjlist[mobjcount++] = mobj;
	}
		...
		if (!no_overlapped_sprites)
		{
			...
			while (i < mobjcount)
			{
				...
				if (GETXY(m1) == GETXY(m2))
				{
					...
					{
						if (mobjlist[i]->index < mo->index) mo = mobjlist[i];
						i++;
					}
...
static void P_LoadLineDefs(int lump)
{
	...
	for (i = 0; i < numlines; i++)
	{
		...
		if (ld->sidenum[0] != NO_INDEX && ld->special) sides[*ld->sidenum].special = ld->special;
	}
...
static void P_LoadSideDefs2(int lump)
{
	...
	for (i = 0; i < numsides; i++)
	{
		...
		case 242: // variable colormap via 242 linedef
			...
			sd->midtexture = (sec->midmap = R_ColormapNumForName(msd->midtexture)) < 0 ? sec->midmap
			                                                                             = 0,
			R_TextureNumForName(msd->midtexture)                                       : 0;
			sd->toptexture = (sec->topmap = R_ColormapNumForName(msd->toptexture)) < 0 ? sec->topmap
			                                                                             = 0,
			...
			for (j = 0; j < ncols; j++)
			{
				...
				if (yp == 0) // intersection at a corner
				{
					if (sneg) //   \ - blocks x,y-, x-,y
					{
						if (yb > 0 && miny < y)
							AddBlockLine(
							  blocklists, blockcount, blockdone, ncols * (yb - 1) + j, i);
						if (j > 0 && minx < x)
					...
					else if (spos) //   / - block x-,y-
					{
						if (yb > 0 && j > 0 && minx < x)
							AddBlockLine(
							  blocklists, blockcount, blockdone, ncols * (yb - 1) + j - 1, i);
					}
			...
			for (j = 0; j < nrows; j++)
			{
				...
				if (xp == 0) // intersection at a corner
				{
					if (sneg) //   \ - blocks x,y-, x-,y
					{
						if (j > 0 && miny < y)
							AddBlockLine(
							  blocklists, blockcount, blockdone, ncols * (j - 1) + xb, i);
						if (xb > 0 && minx < x)
					...
					else if (spos) //   / - block x-,y-
					{
						if (xb > 0 && j > 0 && miny < y)
							AddBlockLine(
							  blocklists, blockcount, blockdone, ncols * (j - 1) + xb - 1, i);
					}
				}
...
static void P_LoadBlockMap(int lump)
{
	...
	{ // e6y
		...
		if (W_LumpLength(lump) >= 0x20000)
			lprintf(LO_WARN,
			  "P_LoadBlockMap: This is due to BLOCKMAP lump having size larger than 65536.\n");
	...
	else
	{
		...
		if (!P_VerifyBlockMap(count))
		{
			...
			lprintf(
			  LO_INFO, "P_LoadBlockMap: use \"-blockmap\" command line switch for rebuilding\n");
		}
	}
...
static void P_LoadReject(int lumpnum, int totallines)
{
	...
	if (rejectlump != -1) W_UnlockLumpNum(rejectlump);
...
static int P_GroupLines(void)
{
	...
	for (i = 0; i < numsubsectors; i++)
	{
		...
		if (subsectors[i].sector == NULL) I_Error("P_GroupLines: Subsector a part of no sector!\n");
	}
...
static void P_RemoveSlimeTrails(void) // killough 10/98
{
	...
	int apply_for_real_vertexes = (compatibility_level >= lxdoom_1_compatibility
				...
				if (!hit[v - vertexes]) // If we haven't processed vertex
				{
					...
					{ // Project the vertex back onto the parent linedef
						...
						if (!apply_for_real_vertexes
						    && (D_abs(v->px - v->x) > 8 * FRACUNIT
						        || D_abs(v->py - v->y) > 8 * FRACUNIT))
						{
							v->px = v->x;
							v->py = v->y;
						}
...
dboolean P_CheckLumpsForSameSource(int lump1, int lump2)
{
	...
	if (!wad1 || !wad2) return false;

	wad1_index = (int)(wad1 - wadfiles);
	wad2_index = (int)(wad2 - wadfiles);

	if (wad1_index != wad2_index) return false;

	if ((wad1_index < 0) || ((size_t)wad1_index >= numwadfiles)) return false;

	if ((wad2_index < 0) || ((size_t)wad2_index >= numwadfiles)) return false;
...
void P_CheckLevelWadStructure(const char *mapname)
{
	...
	if (!mapname) I_Error("P_SetupLevel: Wrong map name");

	lumpnum = W_CheckNumForName(mapname);

	if (lumpnum < 0) I_Error("P_SetupLevel: There is no %s map.", mapname);

	for (i = ML_THINGS + 1; i <= ML_SECTORS; i++)
	{
		if (!P_CheckLumpsForSameSource(lumpnum, lumpnum + i))
		{
			I_Error("P_SetupLevel: Level wad structure is incomplete. There is no %s lump.",
			  ml_labels[i]);
		}
	}
...
void P_InitSubsectorsLines(void)
{
	...
	for (num = 0; num < numsubsectors; num++)
	{
		...
		for (seg = segs + subsectors[num].firstline; seg < seg_last; seg++)
		{
			...
			if (seg->linedef->validcount == 1) continue;
	...
	for (num = 0; num < numlines; num++) lines[num].validcount = 0;
}
...
void P_SetupLevel(int episode, int map, int playermask, skill_t skill)
{
	...
	else { memset(blocklinks, 0, bmapwidth * bmapheight * sizeof(*blocklinks)); }
	...
	if (!samelevel) P_InitSubsectorsLines();

#ifdef GL_DOOM
	map_subsectors = calloc_IfSameLevel(map_subsectors, numsubsectors, sizeof(map_subsectors[0]));
#endif
	...
	for (i = 0; i < MAXPLAYERS; i++) players[i].mo = NULL;
	TracerClearStarts();
	...
	if (gamemode == commercial) P_SpawnBrainTargets();

	if (gamemode != shareware) S_ParseMusInfo(lumpname);
