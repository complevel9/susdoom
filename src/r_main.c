...
#endif
{
  ...
  // Try to quickly decide by looking at sign bits.
	if ((node->dy ^ node->dx ^ x ^ y) < 0) return (node->dy ^ x) < 0;  // (left is negative)
...
{
	return (y -= y1, (x -= x1) || y)
	       ? x >= 0 ? y >= 0 ? (x > y) ? tantoangle[slope_div(y, x)] : // octant 0
	                             ANG90 - 1 - tantoangle[slope_div(x, y)]
	                         : // octant 1
	                    x > (y = -y) ? 0 - tantoangle[slope_div(y, x)]
	                                 : // octant 8
	           ...
	           y >= 0 ? (x = -x) > y ? ANG180 - 1 - tantoangle[slope_div(y, x)] : // octant 3
	                  ...
	                  : // octant 2
	           (x = -x) > (y = -y) ? ANG180 + tantoangle[slope_div(y, x)]
	                               : // octant 4
...
angle_t R_PointToAngleEx(fixed_t x, fixed_t y) { return R_PointToAngleEx2(viewx, viewy, x, y); }
...
{
	// Note: float won't work here as it's less precise than the BAM values being passed as
	// parameters
...
static void R_InitTextureMapping(void)
{
	...
	{ // wide_centerx is what centerx would be if the display was not widescreen
		FieldOfView = (int)(atan((double)centerx * tan((double)FieldOfView * M_PI / FINEANGLES)
		                         / (double)wide_centerx)
		                    * FINEANGLES / M_PI);
		if (FieldOfView > 160 * FINEANGLES / 360) FieldOfView = 160 * FINEANGLES / 360;
	}
...
static void GenLookup(short *lookup1, short *lookup2, int size, int max, int step)
{
	...
	for (i = 1; i < max; i++)
	{
		if (lookup1[i] == 0 && lookup1[i - 1] != 0) lookup1[i] = lookup1[i - 1];
		if (lookup2[i] == 0 && lookup2[i - 1] != 0) lookup2[i] = lookup2[i - 1];
	}
}
...
static void InitStretchParam(stretch_param_t *offsets, int stretch, enum patch_translation_e flags)
{
	...
	if (flags == VPT_ALIGN_BOTTOM || flags == VPT_ALIGN_LEFT_BOTTOM
	    || flags == VPT_ALIGN_RIGHT_BOTTOM)
	{
	...
	if (flags == VPT_ALIGN_WIDE && !tallscreen) offsets->deltay1 = 0;
}
...
void R_SetupViewScaling(void)
{
	...
	for (i = 0; i < 3; i++)
		for (k = 0; k < VPT_ALIGN_MAX; k++) InitStretchParam(&stretch_params_table[i][k], i, k);
	...
	GenLookup(video_stretch.x1lookup, video_stretch.x2lookup, video_stretch.width, 320,
	  video_stretch.xstep);
	GenLookup(video_stretch.y1lookup, video_stretch.y2lookup, video_stretch.height, 200,
	  video_stretch.ystep);
