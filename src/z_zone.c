...
static void Z_DrawStats(void)            // Print allocation statistics
{
	...
	if (memory_size > 0)
	{
		...
		doom_printf("%-5i\t%6.01f%%\tstatic\n"
		            ...
		            "%-5i\t%6.01f%%\tfree\n"
		            "%-5li\t\ttotal\n",
		  active_memory, active_memory * s, purgable_memory, purgable_memory * s,
		  (free_memory + memory_size), (free_memory + memory_size) * s, total_memory);
	}
	else
	{
...
)
{
	...
	if (!size) return user ? *user = NULL : NULL;           // malloc(0) returns NULL
