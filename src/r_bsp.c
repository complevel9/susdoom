...
static void R_ClipWallSegment(int first, int last, dboolean solid)
{
	...
	while (first < last)
	{
		...
		else
		{
			...
			if (solid) memset(solidcol + first, 1, to - first);
			first = to;
		}
	}
}
...
void R_ClearClipSegs(void) { memset(solidcol, 0, SCREENWIDTH); }
...
static void R_RecalcLineFlags(line_t *linedef)
{
  ...
  /* First decide if the line is closed, normal, or invisible */
	if (!(linedef->flags & ML_TWOSIDED) || backsector->ceilingheight <= frontsector->floorheight
    ...
    // CPhipps - recode for speed, not certain if this is portable though
		if (backsector->ceilingheight != frontsector->ceilingheight
		    || backsector->floorheight != frontsector->floorheight || curline->sidedef->midtexture
...
sector_t *R_FakeFlat(
  sector_t *sec, sector_t *tempsec, int *floorlightlevel, int *ceilinglightlevel, dboolean back)
{
	if (floorlightlevel)
		*floorlightlevel
		  = sec->floorlightsec == -1 ? sec->lightlevel : sectors[sec->floorlightsec].lightlevel;
...
static dboolean CheckClip(seg_t *seg, sector_t *frontsector, sector_t *backsector)
{
  ...
  // check for closed sectors!
	...
	{
		if (seg->sidedef->toptexture == NO_TEXTURE) return false;
	...
	{
		if (seg->sidedef->bottomtexture == NO_TEXTURE) return false;
...
static void R_AddLine(seg_t *line)
{
    ...
    // Back side, i.e. backface culling	- read: endAngle >= startAngle!
		if (angle2 - angle1 < ANG180 || !line->linedef) return;
		if (!gld_clipper_SafeCheckRange(angle2, angle1)) return;

		map_subsectors[currentsubsectornum] = 1;

		if (!line->backsector) { gld_clipper_SafeAddClipRange(angle2, angle1); }
		else
		{
  ...
  /* cph - roll up linedef properties in flags */
	if ((linedef = curline->linedef)->r_validcount != gametic) R_RecalcLineFlags(linedef);
...
static const int checkcoord[12][4] = // killough -- static const
  {{3, 0, 2, 1}, {3, 0, 2, 0}, {3, 1, 2, 0}, {0}, {2, 0, 2, 1}, {0, 0, 0, 0}, {3, 1, 3, 0}, {0},
...
static dboolean R_CheckBBox(const fixed_t *bspcoord)
{
  ...
  // that define the edges from current viewpoint.
	boxpos = (viewx <= bspcoord[BOXLEFT]    ? 0
  ...
  // when passed into gld_AddPlane().
...
#ifdef RANGECHECK
	if (num >= numsubsectors) I_Error("R_Subsector: ss %i with numss = %i", num, numsubsectors);
#endif
    ...
    // killough 3/8/98, 4/4/98: Deep water / fake ceiling effect
		frontsector = R_FakeFlat(
		  frontsector, &tempsec, &floorlightlevel, &ceilinglightlevel, false); // killough 4/11/98
		...
		ceilingplane = frontsector->ceilingheight > viewz || frontsector->ceilingpic == skyflatnum
		                || (frontsector->heightsec != -1
...
#ifdef GL_DOOM
		if (V_GetMode() == VID_MODEGL) gld_AddPlane(num, floorplane, ceilingplane);
#endif
	}
	...
	while (count--)
	{
		if (line->miniseg == false) R_AddLine(line);
		line++;
		curline = NULL; /* cph 2001/11/18 - must clear curline now we're done with it, so
...
void R_RenderBSPNode(int bspnum)
{
	while (!(bspnum & NF_SUBSECTOR)) // Found a subsector?
	{
		...
		if (!R_CheckBBox(bsp->bbox[side ^ 1])) return;
