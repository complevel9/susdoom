...
void gld_ResetDrawInfo(void)
{
	...
	for (i = 0; i < gld_drawinfo.maxsize; i++) gld_drawinfo.data[i].size = 0;
	gld_drawinfo.size = 0;

	for (i = 0; i < GLDIT_TYPES; i++) gld_drawinfo.num_items[i] = 0;
}
...
static void gld_AddDrawRange(int size)
{
	...
	gld_drawinfo.data
	  = realloc(gld_drawinfo.data, gld_drawinfo.maxsize * sizeof(gld_drawinfo.data[0]));
...
void gld_AddDrawItem(GLDrawItemType itemtype, void *itemdata)
{
	...
	static int itemsizes[GLDIT_TYPES] = {0, SIZEOF8(GLWall), SIZEOF8(GLWall), SIZEOF8(GLWall),
	  SIZEOF8(GLWall), SIZEOF8(GLWall), SIZEOF8(GLWall), SIZEOF8(GLWall), SIZEOF8(GLFlat),
	  SIZEOF8(GLFlat), SIZEOF8(GLFlat), SIZEOF8(GLFlat), SIZEOF8(GLSprite), SIZEOF8(GLSprite),
	  SIZEOF8(GLSprite), SIZEOF8(GLShadow), SIZEOF8(GLHealthBar)};

	itemsize = itemsizes[itemtype];
	if (itemsize == 0) I_Error("gld_AddDrawItem: unknown GLDrawItemType %d", itemtype);

	if (gld_drawinfo.maxsize == 0) gld_AddDrawRange(NEWSIZE);

	if (gld_drawinfo.data[gld_drawinfo.size].size + itemsize
	    >= gld_drawinfo.data[gld_drawinfo.size].maxsize)
	{
		gld_drawinfo.size++;
		if (gld_drawinfo.size >= gld_drawinfo.maxsize) gld_AddDrawRange(NEWSIZE);
	}

	item_p = gld_drawinfo.data[gld_drawinfo.size].data + gld_drawinfo.data[gld_drawinfo.size].size;
	...
	if (gld_drawinfo.num_items[itemtype] >= gld_drawinfo.max_items[itemtype])
	{
		...
		gld_drawinfo.items[itemtype] = realloc(gld_drawinfo.items[itemtype],
