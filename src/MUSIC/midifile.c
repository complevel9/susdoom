...
static dboolean ReadChannelEvent(
  midi_event_t *event, byte event_type, dboolean two_param, midimem_t *mf)
{
	byte b;
...
static dboolean ReadSysExEvent(midi_event_t *event, int event_type, midimem_t *mf)
{
...
static dboolean ReadEvent(midi_event_t *event, unsigned int *last_event_type, midimem_t *mf)
{
	byte event_type;
	...
	else { *last_event_type = event_type; }
...
static dboolean ReadTrackHeader(midi_track_t *track, midimem_t *mf)
{
	...
	if (records_read < 1) return false;

	if (!CheckChunkHeader(&chunk_header, TRACK_CHUNK_ID)) return false;
...
static dboolean ReadTrack(midi_track_t *track, midimem_t *mf)
{
	...
	if (!ReadTrackHeader(track, mf)) return false;
		...
		*/
		...
		{ // depending on the state of the heap and the malloc implementation, realloc()
          // one more event at a time can be VERY slow.  10sec+ in MSVC
			...
			new_events
			  = (midi_event_t *)realloc(track->events, sizeof(midi_event_t) * track->num_event_mem);
		}
		...
		if (!ReadEvent(event, &last_event_type, mf)) return false;
...
static void FreeTrack(midi_track_t *track)
{
	...
	for (i = 0; i < track->num_events; ++i) FreeEvent(&track->events[i]);
...
static dboolean ReadAllTracks(midi_file_t *file, midimem_t *mf)
{
	...
	if (file->tracks == NULL) return false;
	...
	for (i = 0; i < file->num_tracks; ++i)
		if (!ReadTrack(&file->tracks[i], mf)) return false;
...
void MIDI_FreeFile(midi_file_t *file)
{
	...
	if (file->tracks != NULL)
	{
		for (i = 0; i < file->num_tracks; ++i) FreeTrack(&file->tracks[i]);
...
void MIDI_RestartIterator(midi_track_iter_t *iter) { iter->position = 0; }
...
midi_event_t **MIDI_GenerateFlatList(midi_file_t *file)
{
	...
	for (i = 0; i < file->num_tracks; i++) totalevents += file->tracks[i].num_events;
	...
	while (trackactive)
	{
		...
		{
			if (trackpos[i] != -1
			    && file->tracks[i].events[trackpos[i]].delta_time - tracktime[i] < delta)
			{
...
void MIDI_DestroyFlatList(midi_event_t **evs) { free(evs); }
...
midi_file_t *MIDI_LoadFileSpecial(midimem_t *mf)
{
	...
	while (1)
	{
		...
		if (ret->tracks->num_events == ret->tracks->num_event_mem)
		{
			...
			ret->tracks->events = (midi_event_t *)realloc(
			  ret->tracks->events, sizeof(midi_event_t) * ret->tracks->num_event_mem);
		}
