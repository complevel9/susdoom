...
static const char *vorb_name(void) { return "vorbis player (DISABLED)"; }


static int vorb_init(int samplerate) { return 0; }
...
static size_t vread(void *dst, size_t s, size_t n, void *src)
{
	...
	if (vorb_pos + size >= vorb_len) size = vorb_len - vorb_pos;
...
static const char *vorb_name(void) { return "vorbis player"; }
...
static void vorb_setvolume(int v) { vorb_volume = v; }

static void vorb_pause(void) { vorb_paused = 1; }

static void vorb_resume(void) { vorb_paused = 0; }
...
static void vorb_stop(void) { vorb_playing = 0; }
...
const music_player_t vorb_player = {vorb_name, vorb_init, vorb_shutdown, vorb_setvolume, vorb_pause,
  vorb_resume, vorb_registersong, vorb_unregistersong, vorb_play, vorb_stop, vorb_render};
