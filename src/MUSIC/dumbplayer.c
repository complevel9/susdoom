...
const music_player_t db_player
  = {db_name, db_init, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL};
...
static const char *db_name(void) { return "dumb tracker player"; }
...
static void db_shutdown(void) { dumb_exit(); }

static void db_setvolume(int v) { db_volume = (float)v / 15.0f; }
...
static void db_pause(void) { db_paused = 1; }

static void db_resume(void) { db_paused = 0; }
...
static void db_render(void *dest, unsigned nsamp)
{
	...
	if (db_playing && !db_paused)
	{
		...
		nsampwrit = duh_render_int(
		  dsren, &sig_samples, &sig_samples_size, 16, 0, db_volume, db_delta, nsamp, dest);
