...
static void LoadOperatorData(int Operator, const genmidi_op_t *data, dboolean max_level)
{
	int level;
...
void I_OPL_SetMusicVolume(int volume)
{
	...
	for (i = 0; i < OPL_NUM_VOICES; ++i)
		if (voices[i].channel != NULL) SetVoiceVolume(&voices[i], voices[i].note_volume);
}
...
static opl_voice_t *ReplaceExistingVoice(opl_channel_data_t *channel)
{
	...
	for (rover = voice_alloced_list; rover != NULL; rover = rover->next)
	{
		if (rover->current_instr_voice != 0
		    || (rover->channel > channel && CompareChannelPriorities(channel, rover->channel) > 0))
		{
			result = rover;
			break;
		}
	...
	if (result == NULL) result = voice_alloced_list;
...
static void VoiceKeyOn(opl_channel_data_t *channel, const genmidi_instr_t *instrument,
...
static void KeyOnEvent(opl_track_data_t *track, midi_event_t *event)
{
	...
	*/
	...
	else { instrument = channel->instrument; }
...
static void PitchBendEvent(opl_track_data_t *track, midi_event_t *event)
{
	...
	for (i = 0; i < OPL_NUM_VOICES; ++i)
		if (voices[i].channel == channel) UpdateVoiceFrequency(&voices[i]);
}
...
static void TrackTimerCallback(void *arg)
{
	...
	if (!MIDI_GetNextEvent(track->iter, &event)) return;
	...
	if (event->event_type == MIDI_EVENT_META && event->data.meta.type == MIDI_META_END_OF_TRACK)
	{
		...
		if (running_tracks <= 0 && song_looping) RestartSong();
...
static void StartTrack(const midi_file_t *file, unsigned int track_num)
{
	...
	for (i = 0; i < MIDI_CHANNELS_PER_TRACK; ++i) InitChannel(track, &track->channels[i]);
...
static void I_OPL_PlaySong(const void *handle, int looping)
{
	...
	for (i = 0; i < num_tracks; ++i) StartTrack(file, i);
}
...
static void I_OPL_StopSong(void)
{
	...
	for (i = 0; i < num_tracks; ++i) MIDI_FreeIterator(tracks[i].iter);
...
static void I_OPL_UnRegisterSong(const void *handle)
{
	...
	if (handle != NULL) MIDI_FreeFile((midi_file_t *)handle);
}

// Determine whether memory block is a .mid file

static dboolean IsMid(byte *mem, int len) { return len > 4 && !memcmp(mem, "MThd", 4); }
...
static const void *I_OPL_RegisterSong(const void *data, unsigned len)
{
	...
	if (result == NULL) lprintf(LO_WARN, "I_OPL_RegisterSong: Failed to load MID.\n");
...
const char *I_OPL_SynthName(void) { return "opl2 synth player"; }

void I_OPL_RenderSamples(void *dest, unsigned nsamp) { OPL_Render_Samples(dest, nsamp); }

const music_player_t opl_synth_player = {I_OPL_SynthName, I_OPL_InitMusic, I_OPL_ShutdownMusic,
  I_OPL_SetMusicVolume, I_OPL_PauseSong, I_OPL_ResumeSong, I_OPL_RegisterSong, I_OPL_UnRegisterSong,
