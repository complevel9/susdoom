...
/*
  ...
  Can choose different types of generators, using muls and bigger tables, try different ones for
  slower platforms The generation was based on the MAME implementation but tried to have it use less
  memory and be faster in general MAME uses much bigger envelope tables and this will be the biggest
  cause of it sounding different at times

  //TODO Don't delay first operator 1 sample in opl3 mode
  //TODO Maybe not use class method pointers but a regular function pointers with operator as first
  parameter
  //TODO Fix panning for the Percussion channels, would any opl3 player use it and actually really
  change it though?
...
static const Bit8u FreqCreateTable[16] = {M(0.5), M(1), M(2), M(3), M(4), M(5), M(6), M(7), M(8),
  M(9), M(10), M(10), M(12), M(12), M(15), M(15)};
#undef M
...
static const Bit8s VibratoTable[8]
  = {1 - 0x00, 0 - 0x00, 1 - 0x00, 30 - 0x00, 1 - 0x80, 0 - 0x80, 1 - 0x80, 30 - 0x80};
...
static const WaveHandler WaveHandlerTable[8]
  = {WaveForm0, WaveForm1, WaveForm2, WaveForm3, WaveForm4, WaveForm5, WaveForm6, WaveForm7};
  ...
  Channel *self, Chip *chip, Bit32u samples, Bit32s *output, SynthMode mode)
{
	...
	case sm2AM:
	case sm3AM:
		if (Operator__Silent(Channel__Op(self, 0)) && Operator__Silent(Channel__Op(self, 1)))
		{
			self->old[0] = self->old[1] = 0;
			return (self + 1);
		}
	...
	case sm3FMFM:
		...
		break;
	case sm3AMFM:
		if (Operator__Silent(Channel__Op(self, 0)) && Operator__Silent(Channel__Op(self, 3)))
		{
			self->old[0] = self->old[1] = 0;
			return (self + 2);
		}
		break;
	case sm3FMAM:
		if (Operator__Silent(Channel__Op(self, 1)) && Operator__Silent(Channel__Op(self, 3)))
		{
			self->old[0] = self->old[1] = 0;
			return (self + 2);
		}
		break;
	case sm3AMAM:
		if (Operator__Silent(Channel__Op(self, 0)) && Operator__Silent(Channel__Op(self, 2))
...
static void Chip__WriteBD(Chip *self, Bit8u val)
{
	...
	if (val & 0x20)
	{
		...
		{
			if (self->opl3Active)
				self->chan[6].synthHandler = Channel__BlockTemplate_sm3Percussion;
			else
				self->chan[6].synthHandler = Channel__BlockTemplate_sm2Percussion;
		}
		// Bass Drum
		if (val & 0x10)
		{
...
void Chip__WriteReg(Chip *self, Bit32u reg, Bit8u val)
{
	...
	case 0x00 >> 4:
		...
		else if (reg == 0x105)
		{
			...
			// MAME says the real opl3 doesn't reset anything on opl3 disable/enable till the next
			// write in another register
			if (!((self->opl3Active ^ val) & 1)) return;
			self->opl3Active = (val & 1) ? 0xff : 0;
			// Update the 0xc0 register for all channels to signal the switch to mono/stereo
			// handlers
...
void Chip__Setup(Chip *self, Bit32u rate)
{
	...
	for (i = 0; i < 76; i++)
	{
		...
		self->linearRates[i]
		  = (Bit32u)(scale * (EnvelopeIncreaseTable[index] << (RATE_SH + ENV_EXTRA - shift - 3)));
	}
...
void DBOPL_InitTables(void)
{
	...
	{
		WaveTable[0x700 + i]
		  = (Bit16s)(0.5 + (pow(2.0, -1.0 + (255 - i * 8) * (1.0 / 256))) * 4085);
	...
	{
		WaveTable[0x0200 + i]
		  = (Bit16s)(0.5 - log10(sin((i + 0.5) * (PI / 512.0))) / log10(2.0) * 256);
