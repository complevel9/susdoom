...
static const char *mp_name(void) { return "mad mp3 player (DISABLED)"; }


static int mp_init(int samplerate) { return 0; }

const music_player_t mp_player
  = {mp_name, mp_init, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL};
...
static const char *mp_name(void) { return "mad mp3 player"; }
...
static void mp_setvolume(int v) { mp_volume = v; }

static void mp_pause(void) { mp_paused = 1; }

static void mp_resume(void) { mp_paused = 0; }
...
static void mp_stop(void) { mp_playing = 0; }
...
static void mp_render_ex(void *dest, unsigned nsamp)
{
    ...
    // decode next valid MP3 frame
        ...
        // FIXME: in order to not drop the last frame, there must be at least
				// MAD_BUFFER_GUARD of extra bytes (with value 0) at the end of the file.  current
				// implementation drops last frame
				if (mp_looping)
