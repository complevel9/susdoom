...
static const char *alsa_name(void) { return "alsa midi player (DISABLED)"; }


static int alsa_init(int samplerate) { return 0; }

const music_player_t alsa_player
  = {alsa_name, alsa_init, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL};
...
void alsaplay_refresh_outputs(void)
{
  // port type and capabilities required from valid MIDI output
	...
	if (!seq_handle)
	{
		lprintf(LO_WARN, "alsaplay_refresh_outputs: Can't list ALSA output ports: seq_handle is "
		                 "not initialized\n");
		return;
	}
  ...
  // clear client info
    ...
    // clear port info
      ...
      // check if port is valid midi output
			...
			if (!(snd_seq_port_info_get_type(pinfo) & SND_SEQ_PORT_TYPE_MIDI_GENERIC)) continue;
      ...
      // client name only up to 100 chars, so it always fits within a 120 byte buffer
			sprintf(alsaplayer_outputs[out_ind].name, "%.*s (%d:%d)", 100, client_name, client_num,
			  port_num);
		}
	}
...
int alsaplay_connect_output(int which)
{
	...
	{
		lprintf(LO_WARN,
		  "alsaplay_connect_output: tried to connect to output listing at index out of bounds: "
		  "%d\n",
		  which);
		return -1;
	}
...
{
	if (seq_handle == NULL) return NULL;

	if (which >= alsaplayer_num_outs) return NULL;
...
int alsa_midi_default_dest(void)
{
  ...
  // port type and capabilities required from valid MIDI output
	...
	{
		lprintf(LO_WARN,
		  "alsa_midi_default_dest: Can't list ALSA output ports: seq_handle is not initialized\n");
		return 0;
	}
    ...
    // list ports of each client
      ...
      // check for and skip loopback (eg MIDI Through)
			...
			for (i = 0; i < loopback_check_len; i++)
			{
				...
				if (a != loopback_check_name[i]) break;
			}
    ...
    // clear port info
      ...
      // check if port is valid midi output
        ...
        // connect to this port
				...
				{
					lprintf(LO_WARN,
					  "alsa_midi_default_dest: error connecting to default port %i:%i (%s): %s\n",
  ...
  // try midi through as last resort fallback
	...
	if (loopback_cl != -1)
	{
		...
		{
			lprintf(LO_WARN,
			  "alsa_midi_default_dest: (fallback) error connecting to default port %i:%i (%s): "
			  "%s\n",
		...
		lprintf(LO_INFO,
		  "alsa_midi_default_dest: (fallback) connected to default port %i:%i (%s)\n", loopback_cl,
		  loopback_prt, loopback_name);
...
{
	CHK_RET(
	  snd_seq_open(&seq_handle, "default", SND_SEQ_OPEN_OUTPUT, 0), "could not open sequencer")

	CHK_RET(snd_seq_set_client_name(seq_handle, "PrBoom+ MIDI"), "could not set client name")

	out_id = snd_seq_client_id(seq_handle);

	CHK_RET(out_port = snd_seq_create_simple_port(seq_handle, "Music",
...
int alsa_midi_set_dest(int client, int port)
{
  ...
  // connects to a destination alsa-midi client and port
	...
	CHK_LPRINT_ERR_RET(snd_seq_connect_to(seq_handle, out_port, client, port), -3, LO_WARN,
...
static unsigned long alsa_now(void)
{
  ...
  // update queue status
	CHK_LPRINT_ERR_RET(snd_seq_get_queue_status(seq_handle, out_queue, queue_status), 0, LO_WARN,
	...
	{
		lprintf(
		  LO_WARN, "alsaplayer: alsa_now(): error getting realtime position from queue status\n");
		return 0;
	}
...
static void alsa_midi_evt_start(unsigned long when)
{
  ...
  // schedule
	...
	else { snd_seq_ev_schedule_real(&seq_ev, out_queue, 0, alsa_now_realtime()); }
...
{
	CHK_LPRINT_ERR(snd_seq_event_output(seq_handle, &seq_ev), LO_WARN,
	  "alsa_midi_evt_finish: could not output alsa midi event: %s\n");
}

static void alsa_midi_evt_flush()
{
	CHK_LPRINT_ERR(snd_seq_drain_output(seq_handle), LO_WARN,
	  "alsa_midi_evt_finish: could not drain alsa sequencer output: %s\n");
}

static void alsa_midi_write_event(
  unsigned long when, midi_event_type_t type, int channel, int v1, int v2)
{
...
static void alsa_midi_all_notes_off(void)
{
  // sends All Notes Off event in all channels
	for (int i = 0; i < 16; i++) alsa_midi_all_notes_off_chan(i);
}
...
static int alsa_midi_init_connect_default_port(void)
{
  // load MIDI device specified in config
	...
	if (snd_mididev && strlen(snd_mididev))
	{
		...
		CHK_LPRINT_ERR(snd_seq_parse_address(seq_handle, &seqaddr, snd_mididev), LO_WARN,
...
static const char *alsa_name(void) { return "alsa midi player"; }
...
static void alsa_refreshvolume(void)
{
	...
	for (i = 0; i < 16; i++) alsa_midi_write_control_now(i, 7, channelvol[i] * alsa_volume / 15);
...
static void alsa_play(const void *handle, int looping)
{
  ...
  // set queue resolution
	...
	snd_seq_queue_timer_set_resolution(
	  timer, 1000000 / 32); // 1000000 ns = 1 ms, so this is 1/32 ms
  ...
  // timerpos = newtime;
    ...
    // process event
		...
		case MIDI_EVENT_SYSEX_SPLIT:
			alsa_midi_writesysex(when, currevent->event_type, currevent->data.sysex.data,
		...
		case MIDI_EVENT_CONTROLLER:
			...
			{ // volume event
				alsa_setchvolume(
				  currevent->data.channel.channel, currevent->data.channel.param2, when);
				break;
			} // fall through
		default:
		...
		// if the event was a "reset all controllers", we need to additionally re-fix the volume
		// (which itself was reset)
...
const music_player_t alsa_player = {alsa_name, alsa_init, alsa_shutdown, alsa_setvolume, alsa_pause,
  alsa_resume, alsa_registersong, alsa_unregistersong, alsa_play, alsa_stop, alsa_render};
