...
static const char *pm_name(void) { return "portmidi midi player (DISABLED)"; }


static int pm_init(int samplerate) { return 0; }

const music_player_t pm_player
  = {pm_name, pm_init, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL};
...
static const char *pm_name(void) { return "portmidi midi player"; }
...
static void init_reset_buffer(void)
{
  ...
  // if no reverb specified and no SysEx reset selected, then use GM default
	if (reverb == -1 && sysex_reset == NULL) reverb = 40;

	if (reverb > -1)
		for (i = 0; i < 16; ++i) event_reverb[i].message = Pm_Message(0xB0 | i, 0x5B, reverb);

  // if no chorus specified and no SysEx reset selected, then use GM default
	if (chorus == -1 && sysex_reset == NULL) chorus = 0;

	if (chorus > -1)
		for (i = 0; i < 16; ++i) event_chorus[i].message = Pm_Message(0xB0 | i, 0x5D, chorus);
}
...
static int pm_init(int samplerate)
{
  ...
  // look for a device that matches the user preference
	...
	if (Pm_OpenOutput(&pm_stream, outputdevice, NULL, DRIVER_BUFFER, NULL, NULL, DRIVER_LATENCY)
	    != pmNoError)
	{
	...
	for (int i = 0; i < 16; i++) channel_volume[i] = DEFAULT_VOLUME;
...
static void pm_shutdown(void)
{
    ...
    /* ugly deadlock in portmidi win32 implementation:
		...
		"Applications should not call any multimedia functions from inside the callback function,
		 as doing so can cause a deadlock. Other system functions can safely be called from the
		callback."

		winmm_streamout_callback calls midiOutUnprepareHeader.  oops?


		since timestamps are slightly in the future, it's very possible to have some messages still
		in the windows midi queue when Pm_Close is called.  this is normally no problem, but if one
		so happens to dequeue and call winmm_streamout_callback at the exact right moment...

		fix: at this point, we've stopped generating midi messages.  sleep for more than
		DRIVER_LATENCY to ensure all messages are flushed.
...
{
	for (int i = 0; i < 16; i++) write_volume(0, i, channel_volume[i]);
}

static void reset_volume(void)
{
	for (int i = 0; i < 16; i++) write_volume(0, i, DEFAULT_VOLUME);
}

static void pm_setvolume(int v)
{
	if (pm_volume == v) return;
	...
	case 0x41: // roland
		...
		case 0x42: // gs
			...
			case 0x12: // dt1
				...
				else if (len == 11 && msg[5] == 0x40 && // address msb
				         msg[6] == 0x00 && // address
				         msg[7] == 0x7F && // address lsb
...
static void writesysex(unsigned long when, int etype, byte *data, int len)
{
  ...
  // process message if it's complete, otherwise do nothing yet
	if (sysexbuff[sysexbufflen - 1] == MIDI_EVENT_SYSEX_SPLIT)
	{
		...
		if (is_sysex_reset(sysexbuff, sysexbufflen)) reset_volume();
...
static void pm_render(void *vdest, unsigned bufflen)
{
  // wherever you see samples in here, think milliseconds
    ...
    // delay after reset, for real devices only (e.g. roland sc-55)
		if (use_reset_delay) eventdelta += mus_portmidi_reset_delay;
      ...
      // overshoot; render some samples without processing an event
		...
		case MIDI_EVENT_SYSEX_SPLIT:
			if (!mus_portmidi_filter_sysex)
				writesysex(when, currevent->event_type, currevent->data.sysex.data,
        ...
        // fall through
		default:
			writeevent(when, currevent->event_type, currevent->data.channel.channel,
