...
void OPL_Queue_Destroy(opl_callback_queue_t *queue) { free(queue); }

int OPL_Queue_IsEmpty(opl_callback_queue_t *queue) { return queue->num_entries == 0; }

void OPL_Queue_Clear(opl_callback_queue_t *queue) { queue->num_entries = 0; }

void OPL_Queue_Push(
  opl_callback_queue_t *queue, opl_callback_t callback, void *data, unsigned int time)
{
	int entry_id;
	int parent_id;
	...
	while (entry_id > 0)
	{
		...
		if (time >= queue->entries[parent_id].time) break;
...
int OPL_Queue_Pop(opl_callback_queue_t *queue, opl_callback_t *callback, void **data)
{
	...
	if (queue->num_entries <= 0) return 0;
...
static void PrintQueueNode(opl_callback_queue_t *queue, int node, int depth)
{
	...
	if (node >= queue->num_entries) return;
...
static void PrintQueue(opl_callback_queue_t *queue) { PrintQueueNode(queue, 0, 0); }
