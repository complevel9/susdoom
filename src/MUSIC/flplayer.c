...
static const char *fl_name(void) { return "fluidsynth midi player (DISABLED)"; }


static int fl_init(int samplerate) { return 0; }

const music_player_t fl_player
  = {fl_name, fl_init, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL};
...
static const char *fl_name(void) { return "fluidsynth midi player"; }
...
static int fl_init(int samplerate)
{
  ...
  // fluidsynth 1.1.4 supports sample rates as low as 8000hz.  earlier versions only go down to
	// 22050hz since the versions are ABI compatible, detect at runtime, not compile time
	if (1)
	{
		int sratemin;
		int major;
...
static void fl_resume(void) { f_paused = 0; }
...
static void fl_setvolume(int v) { f_volume = v; }
...
static void writesysex(unsigned char *data, int len)
{
  ...
  // until it hits an 0xf7 terminator)
	...
	{ // pass len-1 because fluidsynth does NOT want the final F7
		fluid_synth_sysex(
		  f_syn, (const char *)sysexbuff, sysexbufflen - 1, NULL, NULL, &didrespond, 0);
		sysexbufflen = 0;
	}
	if (!didrespond) lprintf(LO_WARN, "fluidplayer: SYSEX message received but not understood\n");
}
...
static void fl_render(void *vdest, unsigned length)
{
    ...
    // process event
		...
		case MIDI_EVENT_NOTE_OFF:
			fluid_synth_noteoff(
			  f_syn, currevent->data.channel.channel, currevent->data.channel.param1);
			break;
		...
		case MIDI_EVENT_CONTROLLER:
			fluid_synth_cc(f_syn, currevent->data.channel.channel, currevent->data.channel.param1,
