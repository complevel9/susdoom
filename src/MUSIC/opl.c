...
{
	OPL_Queue_Push(
	  callback_queue, callback, data, current_time - pause_offset + (ms * opl_sample_rate) / 1000);
}

void OPL_ClearCallbacks(void) { OPL_Queue_Clear(callback_queue); }
...
static void OPL_AdvanceTime(unsigned int nsamples)
{
	...
	if (opl_paused) pause_offset += nsamples;
...
void OPL_Render_Samples(void *dest, unsigned buffer_len)
{
	...
	while (filled < buffer_len)
	{
		...
		if (opl_paused || OPL_Queue_IsEmpty(callback_queue)) { nsamples = buffer_len - filled; }
		else
		{
			...
			if (nsamples > buffer_len - filled) nsamples = buffer_len - filled;
		}
...
void OPL_InitRegisters(void)
{
	...
	for (r = 1; r < OPL_REGS_LEVEL; ++r) OPL_WriteRegister(r, 0x00);
