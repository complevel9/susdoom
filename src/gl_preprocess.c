...
static void gld_CalcIntersectionVertex(vertex_t *s, vertex_t *e, divline_t *d, vertex_t *i)
{
	...
	double cx = FIX2DBL(d->x), cy = FIX2DBL(d->y), dx = cx + FIX2DBL(d->dx),
	       dy = cy + FIX2DBL(d->dy);
  ...
  int *numpoints, vertex_t *points, int numclippers, divline_t *clippers)
{
  ...
  // each divline is discarded.
    ...
    // to be on the line.
		for (k = 0; k < num; k++) sidelist[k] = gld_PointOnSide(&points[k], curclip);
      ...
      // Clipping will happen when the ends are on different sides.
        ...
        // Add the new vertex. Also modify the sidelist.
				...
				if (num >= MAX_CC_SIDES) I_Error("gld_FlatEdgeClipper: Too many points in carver");

        // Make room for the new vertex.
				memmove(
				  &points[endIdx + 1], &points[endIdx], (num - endIdx - 1) * sizeof(vertex_t));
  ...
  // Screen out consecutive identical points.
	for (i = 0; i < num; i++)
	{
		...
		if (points[i].x == points[previdx].x && points[i].y == points[previdx].y)
		{
...
static void CALLBACK ntessBegin(GLenum type)
{
  ...
  // PU_LEVEL is used, so this gets freed before a new level is loaded
	sectorloops[currentsector].loops = Z_Realloc(sectorloops[currentsector].loops,
  ...
  // currentloop is -> sectorloops[currentsector].loopcount-1
	...
	sectorloops[currentsector].loops[sectorloops[currentsector].loopcount - 1].vertexindex
	  = gld_num_vertexes;
}
		...
		if (sectors[num].lines[i]->sidenum[0] != NO_INDEX)
			if (sectors[num].lines[i]->sidenum[1] != NO_INDEX)
				    ...
				    == sides[sectors[num].lines[i]->sidenum[1]].sector)
				{
					...
					if (levelinfo)
						fprintf(levelinfo,
						  "line %4i (iLineID %4i) has both sides in same sector (removed)\n", i,
						  sectors[num].lines[i]->iLineID);
				}
	}
  // e6y
			...
			for (i = 0; i < sectors[num].linecount; i++)
				if (!lineadded[i])
				{
					...
					if ((sectors[num].lines[currentline]->sidenum[0] != NO_INDEX)
					      ? (sides[sectors[num].lines[currentline]->sidenum[0]].sector
					          == &sectors[num])
					      : false)
...
//            gluNextContour(tess, GLU_CW);
    ...
    // check if currentsector is on the front side of the line ...
      ...
      // calculate the angle of this line for use below
			lineangle = R_PointToAngle2(sectors[num].lines[currentline]->v1->x,
			  sectors[num].lines[currentline]->v1->y, sectors[num].lines[currentline]->v2->x,
      ...
      // calculate the angle of this line for use below
			lineangle = R_PointToAngle2(sectors[num].lines[currentline]->v2->x,
			  sectors[num].lines[currentline]->v2->y, sectors[num].lines[currentline]->v1->x,
    ...
    // calculate coordinates for the glu tesselation functions
		...
		if (levelinfo)
			fprintf(levelinfo, "\t\tgluTessVertex(%i, %i)\n", currentvertex->x >> FRACBITS,
			  currentvertex->y >> FRACBITS);
		...
		// find the next line
		oldline
		  = currentline; // if this isn't changed at the end of the search, a new loop will start
		...
		for (i = 0; i < sectors[num].linecount; i++)
			if (!lineadded[i]) // if the line isn't already added ...
				...
				if ((sectors[num].lines[i]->v1 == currentvertex)
				    || (sectors[num].lines[i]->v2 == currentvertex))
				{
					...
					if ((sectors[num].lines[i]->sidenum[0] != NO_INDEX)
						...
						angle = R_PointToAngle2(sectors[num].lines[i]->v1->x,
						  sectors[num].lines[i]->v1->y, sectors[num].lines[i]->v2->x,
						  sectors[num].lines[i]->v2->y);
					else
						angle = R_PointToAngle2(sectors[num].lines[i]->v2->x,
						  sectors[num].lines[i]->v2->y, sectors[num].lines[i]->v1->x,
					...
					if ((sectors[num].lines[i]->sidenum[0] != NO_INDEX)
					      ...
					      : false)
					{
						// when the line is not flipped and startvertex is not the currentvertex
						// then skip this line
						if (sectors[num].lines[i]->v1 != currentvertex) continue;
					}
					else
					{
						// when the line is flipped and endvertex is not the currentvertex then skip
						// this line
						if (sectors[num].lines[i]->v2 != currentvertex) continue;
					}
					...
					if (bestline == -1) // if this is the first one ...
					{
						...
						// check if the angle between the current line and this best line candidate
						// is smaller then the angle of the last candidate e6y: for finding an angle
						// between AB and BC vectors we should subtract (BC - BA) == (BC - (180 -
						// AB)) == (angle-(180-lineangle))
...
static void gld_GetSubSectorVertices(void)
{
	...
	for (i = 0; i < numsubsectors; i++)
	{
		...
		if ((ssector->sector->flags & SECTOR_IS_CLOSED) && !triangulate_subsectors) continue;
		...
		if (flats_vbo)
		{
			...
			{
				flats_vbo[gld_num_vertexes].u
				  = ((float)(segs[ssector->firstline + j].v1->x) / FRACUNIT) / 64.0f;
				flats_vbo[gld_num_vertexes].v
				  = (-(float)(segs[ssector->firstline + j].v1->y) / FRACUNIT) / 64.0f;
				flats_vbo[gld_num_vertexes].x
				  = -(float)(segs[ssector->firstline + j].v1->x) / MAP_SCALE;
				flats_vbo[gld_num_vertexes].y = 0.0f;
				flats_vbo[gld_num_vertexes].z
				  = (float)(segs[ssector->firstline + j].v1->y) / MAP_SCALE;
				gld_num_vertexes++;
			}
		}
	}
	...
	for (i = 0; i < numsectors; i++)
	{
		...
		for (loopnum = 0; !fail && loopnum < sectorloops[i].loopcount; loopnum++)
		{
			...
			for (vertexnum = currentloop->vertexindex;
			  !fail && vertexnum < (currentloop->vertexindex + currentloop->vertexcount);
			  vertexnum++)
			{
				vbo_xyz_uv_t *vbo;
...
static void gld_PreprocessSectors(void)
{
	...
	if (numsectors)
	{
		...
		if (!sectorloops) I_Error("gld_PreprocessSectors: Not enough memory for array sectorloops");
	...
	if (numsegs)
	{
		...
		if (!segrendered) I_Error("gld_PreprocessSectors: Not enough memory for array segrendered");
	}
	...
	gld_max_vertexes = 0;
	gld_num_vertexes = 0;
	if (numvertexes) gld_AddGlobalVertexes(numvertexes * 2);
	...
	for (i = 0; i < numsectors; i++)
	{
		...
		for (j = 0; j < sectors[i].linecount; j++)
		{
			...
			if ((v1num >= numvertexes) || (v2num >= numvertexes)) continue;
		...
		if (sectors[i].flags & SECTOR_IS_CLOSED) gld_PrecalculateSector(i);
	}
	free(vertexcheck);
	free(vertexcheck2);
#endif /* USE_GLU_TESS */
...
void gld_PreprocessLevel(void)
{
	...
	if (!gl_preprocessed)
	{
		...
		for (i = 0; i < numsectors_prev; i++) free(sectorloops[i].loops);
		free(sectorloops);
		for (i = 0; i < numsubsectors_prev; i++) free(subsectorloops[i].loops);
		free(subsectorloops);
	...
	if (!gl_preprocessed)
	{
		if (gl_ext_arb_vertex_buffer_object)
		{
			...
			// upload data to VBO
			GLEXT_glBufferDataARB(GL_ARRAY_BUFFER, gld_num_vertexes * sizeof(flats_vbo[0]),
	...
	if (!gl_preprocessed)
	{
		if (gl_use_display_lists) gld_InitDisplayLists();
	}
#endif
