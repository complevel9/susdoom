...
void R_InitInterpolation(void) { tic_vars.msec = realtic_clock_rate * TICRATE / 100000.0f; }
...
void R_InterpolateView(player_t *player, fixed_t frac)
{
	...
	if (!paused && movement_smooth)
	{
		...
		if (didInterp)
			for (i = numinterpolations - 1; i >= 0; i--) R_DoAnInterpolation(i, tic_vars.frac);
	}
}
...
static void R_SetInterpolation(interpolation_type_e type, void *posptr)
{
	...
	if (numinterpolations >= interpolations_max)
	{
		...
		if (interpolations_max == prevmax) return;
...
void R_StopAllInterpolations(void)
{
	...
	for (i = 0; i < numsides; i++) sides[i].INTERP_WallPanning = 0;
}
...
void R_RestoreInterpolations(void)
{
	...
	{
		didInterp = false;
		for (i = numinterpolations - 1; i >= 0; --i) R_CopyBakToInterp(i);
	}
}
...
static void R_InterpolationGetData(thinker_t *th, interpolation_type_e *type1,
  interpolation_type_e *type2, void **posptr1, void **posptr2)
{
	*posptr1 = NULL;
	*posptr2 = NULL;
