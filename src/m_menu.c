...
static int allow_changes(void) { return !(demoplayback || demorecording || netgame); }
...
{
	if (menu_background) V_DrawBackground(flat, scrn);
}
...
void M_DrawTitle(int x, int y, const char *patch, int cm, const char *alttext, int altcm);
...
menuitem_t MainMenu[] = {{1, "M_NGAME", M_NewGame, 'n'}, {1, "M_OPTION", M_Options, 'o'},
...
menu_t ReadDef1 = {read1_end, &MainDef, ReadMenu1, M_DrawReadThis1, 330, 175,
...
menu_t HelpDef =           // killough 10/98
  {help_end, &HelpDef, HelpMenu, M_DrawHelp, 330, 175, 0};
...
menuitem_t EpisodeMenu[] = // added a few free entries for UMAPINFO
  {{1, "M_EPI1", M_Episode, 'k'}, {1, "M_EPI2", M_Episode, 't'}, {1, "M_EPI3", M_Episode, 'i'},
    {1, "M_EPI4", M_Episode, 't'}, {1, "", M_Episode, '0'}, {1, "", M_Episode, '0'},
    {1, "", M_Episode, '0'}, {1, "", M_Episode, '0'}};
...
void M_ClearEpisodes(void) { EpiDef.numitems = 0; }
...
void M_AddEpisode(const char *map, const char *gfx, const char *txt, const char *alpha)
{
	if (!EpiCustom)
	{
		...
		if (gamemode == commercial || gamemission == chex) EpiDef.numitems = 0;
	}
...
menu_t LoadDef = {load_end, &MainDef, LoadMenue, M_DrawLoad, 80, 34, // jff 3/15/98 move menu up
  0};
...
void M_DrawLoad(void)
{
  ...
  //  CPhipps - patch drawing updated
	...
	if (delete_verify) M_DrawDelVerify();
}
...
menu_t SaveDef = {load_end, // same number of slots as the Load Game screen
  &MainDef, SaveMenu, M_DrawSave, 80, 34, // jff 3/15/98 move menu up
  0};
...
static void SetDefaultSaveName(int slot)
{
	...
	{
		snprintf(savegamestrings[itemOn], SAVESTRINGSIZE, "%s", lumpinfo[maplumpnum].name);
	}
	else
	{
		...
		while (basename > wadname && *basename != '/' && *basename != '\\') basename--;
		if (*basename == '/' || *basename == '\\') basename++;

		snprintf(
		  savegamestrings[itemOn], SAVESTRINGSIZE, "%s (%s)", lumpinfo[maplumpnum].name, basename);
		(free)(wadname);
	}
...
static dboolean StartsWithMapIdentifier(char *str)
{
	...
	if (strlen(str) >= 5 && str[0] == 'M' && str[1] == 'A' && str[2] == 'P' && isdigit(str[3])
	    && isdigit(str[4]))
	{
		return true;
	}
  ...
  // e6y: Optional removal of a quit sound
    ...
    // wait till all sounds stopped or 3 seconds are over
		...
		while (i > 0)
		{
			...
			if (!I_AnySoundStillPlaying()) break;
			i--;
		}
	}
...
menuitem_t MouseMenu[] = {{2, "M_HORSEN", M_MouseHoriz, 'h', "HORIZONTAL"}, {-1, "", 0},
  {2, "M_VERSEN", M_MouseVert, 'v', "VERTICAL"},
  {-1, "", 0}

  // e6y
  ,
  {2, "M_LOKSEN", M_MouseMLook, 'l', "MOUSE LOOK"}, {-1, "", 0},
  {2, "M_ACCEL", M_MouseAccel, 'a', "ACCELERATION"}, {-1, "", 0}};

menu_t MouseDef = {mouse_end, &OptionsDef, MouseMenu, M_DrawMouse, 60, 37,// e6y
  0};
...
void M_MouseHoriz(int choice) { M_Mouse(choice, &mouseSensitivity_horiz); }
...
menuitem_t Generic_Setup[] = {{1, "", M_DoNothing, 0}};
...
menu_t KeybndDef
  = {generic_setup_end, &SetupDef, Generic_Setup, M_DrawKeybnd, 34, 5,      // skull drawn here
    0};

menu_t WeaponDef
  = {generic_setup_end, &SetupDef, Generic_Setup, M_DrawWeapons, 34, 5,      // skull drawn here
    0};

menu_t StatusHUDDef
  = {generic_setup_end, &SetupDef, Generic_Setup, M_DrawStatusHUD, 34, 5,      // skull drawn here
    0};

menu_t AutoMapDef
  = {generic_setup_end, &SetupDef, Generic_Setup, M_DrawAutoMap, 34, 5,      // skull drawn here
    0};

menu_t EnemyDef =                                           // phares 4/08/98
  {generic_setup_end, &SetupDef, Generic_Setup, M_DrawEnemy, 34, 5,      // skull drawn here
    0};

menu_t MessageDef =                                         // phares 4/08/98
  {generic_setup_end, &SetupDef, Generic_Setup, M_DrawMessages, 34, 5,      // skull drawn here
    0};

menu_t ChatStrDef =                                         // phares 4/10/98
  {generic_setup_end, &SetupDef, Generic_Setup, M_DrawChatStrings, 34, 5,      // skull drawn here
    0};

menu_t GeneralDef =                                           // killough 10/98
  {generic_setup_end, &OptionsDef, Generic_Setup, M_DrawGeneral, 34, 5,      // skull drawn here
    0};

menu_t CompatDef =                                           // killough 10/98
  {generic_setup_end, &SetupDef, Generic_Setup, M_DrawCompat, 34, 5,      // skull drawn here
    0};
...
void M_Setup(int choice) { M_SetupNextMenu(&SetupDef); }
...
static void M_DrawItem(const setup_menu_t *s)
{
	...
	{ // Draw the item string
		char *p, *t;
		int w = 0;
		int color = flags & S_DISABLE ? CR_DISABLE : // e6y
...
static void M_DrawSetting(const setup_menu_t *s)
{
  ...
  // depending on whether the item is a text string or not.
	...
	color = flags & S_DISABLE ? CR_DISABLE : // e6y
	          flags & S_SELECT ? CR_SELECT
	      : flags & S_HILITE   ? CR_HILITE
	                           : CR_SET;
  ...
  // Is the item a key binding?
    ...
    // Draw the key bound to the action
		...
		if (key)
		{
			...
			if (s->m_joy) sprintf(menu_buffer + strlen(menu_buffer), "/JSB%d", *s->m_joy + 1);
  ...
  // Is the item a paint chip?
	...
	if (flags & S_STRING)
	{
		...
		if (setup_select && (s->m_flags & (S_HILITE | S_SELECT)))
		{
			...
			*c = text[chat_index]; // hold temporarily
			...
			if (char_width == 1) char_width = 7; // default for end of line
	...
	if (flags & S_CHOICE)
	{
		...
		if (s->var.def->type == def_str) sprintf(menu_buffer, "%s", *s->var.def->location.ppsz);
...
static void M_DrawInstructions(void)
{
	...
	{
		switch (flags
		        & (S_KEY | S_YESNO | S_WEAP | S_NUM | S_COLOR | S_CRITEM | S_CHAT | S_RESET | S_FILE
		           | S_CHOICE))
		{
		case S_KEY:
...
setup_menu_t *keys_settings[] = {keys_settings1, keys_settings2, keys_settings3, keys_settings4,
  keys_settings5, keys_settings6, keys_settings7, NULL};
...
setup_menu_t keys_settings7[] = {{"MENUS", S_SKIP | S_TITLE, m_null, KB_X, KB_Y + 0 * 8},
...
enum
{ // killough 10/98: enum for y-offset info
	weap_recoil,
	weap_bobbing,
...
setup_menu_t weap_settings1[] = // Weapons Settings screen
  {{"ENABLE RECOIL", S_YESNO, m_null, WP_X, WP_Y + weap_recoil * 8, {"weapon_recoil"}},
    ...
    {"WEAPON ATTACK ALIGNMENT", S_CHOICE, m_null, WP_X, WP_Y + weap_attack_alignment * 8,
      {"weapon_attack_alignment"}, 0, 0, NULL, weapon_attack_alignment_strings},
...
setup_menu_t stat_settings2[]
  = {{"ADVANCED HUD SETTINGS", S_SKIP | S_TITLE, m_null, ADVHUD_X, SB_Y + 1 * 8},
    ...
    {"SHOW PROGRESS BAR DURING DEMO PLAYBACK", S_YESNO, m_null, ADVHUD_X, SB_Y + 7 * 8,
    ...
    {"ENABLE CROSSHAIR", S_CHOICE, m_null, ADVHUD_X, SB_Y + 11 * 8, {"hudadd_crosshair"}, 0, 0, 0,
      crosshair_str},
    {"SCALE CROSSHAIR", S_YESNO, m_null, ADVHUD_X, SB_Y + 12 * 8, {"hudadd_crosshair_scale"}},
    {"CHANGE CROSSHAIR COLOR BY PLAYER HEALTH", S_YESNO, m_null, ADVHUD_X, SB_Y + 13 * 8,
      {"hudadd_crosshair_health"}},
    {"CHANGE CROSSHAIR COLOR ON TARGET", S_YESNO, m_null, ADVHUD_X, SB_Y + 14 * 8,
      {"hudadd_crosshair_target"}},
    {"LOCK CROSSHAIR ON TARGET", S_YESNO, m_null, ADVHUD_X, SB_Y + 15 * 8,
      {"hudadd_crosshair_lock_target"}},
    {"DEFAULT CROSSHAIR COLOR", S_CRITEM, m_null, ADVHUD_X, SB_Y + 16 * 8,
      {"hudadd_crosshair_color"}},
    {"TARGET CROSSHAIR COLOR", S_CRITEM, m_null, ADVHUD_X, SB_Y + 17 * 8,
      {"hudadd_crosshair_target_color"}},
...
setup_menu_t *auto_settings[] = {auto_settings1, auto_settings2, auto_settings3, NULL};
...
setup_menu_t auto_settings1[] = // 1st AutoMap Settings screen
  {{"Show Kills/Secrts/Items statistics", S_YESNO, m_null, AU_X, AU_Y + 0 * 8, {"map_level_stat"}},
    {"Show coordinates of automap pointer", S_YESNO, m_null, AU_X, AU_Y + 1 * 8,
      {"map_point_coord"}}, // killough 10/98
    {"Show Secrets only after entering", S_YESNO, m_null, AU_X, AU_Y + 2 * 8, {"map_secret_after"}},
    {"Update unexplored parts in automap mode", S_YESNO, m_null, AU_X, AU_Y + 3 * 8,
      {"map_always_updates"}},
    {"Grid cell size 1..256, -1 for autosize", S_NUM, m_null, AU_X, AU_Y + 4 * 8, {"map_grid_size"},
      0, 0, M_ChangeMapGridSize},
    ...
    {"Apply multisampling", S_YESNO, m_null, AU_X, AU_Y + 8 * 8, {"map_use_multisamling"}, 0, 0,
      M_ChangeMapMultisamling},
#ifdef GL_DOOM
    {"Enable textured display", S_YESNO, m_null, AU_X, AU_Y + 9 * 8, {"map_textured"}, 0, 0,
      M_ChangeMapTextured},
    {"Things appearance", S_CHOICE, m_null, AU_X, AU_Y + 10 * 8, {"map_things_appearance"}, 0, 0,
      NULL, map_things_appearance_list},
    ...
    {"Textured automap in overlay mode", S_NUM, m_null, AU_X, AU_Y + 13 * 8,
...
setup_menu_t auto_settings2[] = // 2st AutoMap Settings screen
  {{"background", S_COLOR, m_null, AU_X, AU_Y, {"mapcolor_back"}},
    ...
    {"Automap Statistics Titles Color", S_CRITEM, m_null, AU_X, AU_Y + 14 * 8,
      {"hudcolor_mapstat_title"}},
    {"Automap Statistics Values Color", S_CRITEM, m_null, AU_X, AU_Y + 15 * 8,
      {"hudcolor_mapstat_value"}},
    {"Automap Statistics Time Color", S_CRITEM, m_null, AU_X, AU_Y + 16 * 8,
...
setup_menu_t auto_settings3[] = // 3nd AutoMap Settings screen
  {{"teleporter line", S_COLOR, m_null, AU_X, AU_Y, {"mapcolor_tele"}},
    ...
    {"countable enemy sprite", S_COLOR, m_null, AU_X, AU_Y + 6 * 8,
      {"mapcolor_enemy"}}, // cph 2006/06/30
    {"countable item sprite", S_COLOR, m_null, AU_X, AU_Y + 7 * 8,
      {"mapcolor_item"}}, // mead 3/4/2003
...
void M_DrawAutoMap(void)
{
	...
	if (colorbox_active) M_DrawColPal();
...
setup_menu_t enem_settings1[] = // Enemy Settings screen
  {
    // killough 7/19/98
    {"Monster Infighting When Provoked", S_YESNO, m_null, E_X, E_Y + enem_infighting * 8,
      {"monster_infighting"}},

    {"Remember Previous Enemy", S_YESNO, m_null, E_X, E_Y + enem_remember * 8,
      {"monsters_remember"}},
    ...
    // killough 9/9/98
    {"Intelligently Avoid Hazards", S_YESNO, m_null, E_X, E_Y + enem_avoid_hazards * 8,
    ...
    // killough 7/19/98
    {"Number Of Single-Player Helper Dogs", S_NUM | S_LEVWARN, m_null, E_X, E_Y + enem_helpers * 8,
      {"player_helpers"}},

    // killough 8/8/98
    {"Distance Friends Stay Away", S_NUM, m_null, E_X, E_Y + enem_distfriend * 8,
      {"friend_distance"}},

    {"Allow dogs to jump down", S_YESNO, m_null, E_X, E_Y + enem_dog_jumping * 8, {"dog_jumping"}},

    {"Colored blood and gibs", S_YESNO, m_null, E_X, E_Y + enem_colored_blood * 8,
      {"colored_blood"}},
...
setup_menu_t *gen_settings[] = {gen_settings1, gen_settings2, gen_settings3, gen_settings4,
  gen_settings5, gen_settings6, gen_settings7, gen_settings8, NULL};
...
static const char *videomodes[] = {"8bit", "15bit", "16bit", "32bit",
#ifdef GL_DOOM
  "OpenGL",
#endif
  NULL};
...
setup_menu_t gen_settings1[] = { // General Settings screen1
  ...
  {"Video mode", S_CHOICE, m_null, G_X, G_Y + 2 * 8, {"videomode"}, 0, 0, M_ChangeVideoMode,
    videomodes},
  {"Screen Resolution", S_CHOICE, m_null, G_X, G_Y + 3 * 8, {"screen_resolution"}, 0, 0,
    M_ChangeVideoMode, screen_resolutions_list},
  {"Aspect Ratio", S_CHOICE, m_null, G_X, G_Y + 4 * 8, {"render_aspect"}, 0, 0, M_ChangeAspectRatio,
    render_aspects_list},
  {"Fullscreen Video mode", S_YESNO, m_null, G_X, G_Y + 5 * 8, {"use_fullscreen"}, 0, 0,
    M_ChangeFullScreen},
  {"Status Bar and Menu Appearance", S_CHOICE, m_null, G_X, G_Y + 6 * 8, {"render_stretch_hud"}, 0,
    0, M_ChangeStretch, render_stretch_list},
  ...
  {"Translucency filter percentage", S_NUM, m_null, G_X, G_Y + 10 * 8, {"tran_filter_pct"}, 0, 0,
    M_Trans},
  {"Uncapped Framerate", S_YESNO, m_null, G_X, G_Y + 11 * 8, {"uncapped_framerate"}, 0, 0,
  ...
  {"Preferred MIDI player", S_CHOICE | S_PRGWARN, m_null, G_X, G_Y + 17 * 8, {"snd_midiplayer"}, 0,
    0, M_ChangeMIDIPlayer, midiplayers},
  {"disable sound cutoffs", S_YESNO, m_null, G_X, G_Y + 18 * 8, {"full_sounds"}},
  //{"Low-pass filter",                S_YESNO,            m_null, G_X, G_Y+19*8,
  //{"lowpass_filter"}},
...
static const char *gen_compstrings[] = {"17/Default", "0/Doom v1.2", "1/Doom v1.666",
  "2/Doom (2) v1.9", "3/Ultimate v1.9", "4/Final Doom", "5/DosDoom", "6/TASDoom", "7/Boom vanilla",
  "8/Boom v2.01", "9/Boom", "10/LxDoom", "11/MBF", "12/PrB 2.03b", "13/PrB 2.1.x", "14/PrB 2.2.x",
  "15/PrB 2.3.x", "16/PrB 2.4.0", "17/PrB+ Latest", NULL};
...
setup_menu_t gen_settings2[] = { // General Settings screen2
  ...
  {"Maximum number of player corpses", S_NUM | S_PRGWARN, m_null, G_X, G_Y + 12 * 8,
    {"max_player_corpse"}},
  {"Game speed, percentage of normal", S_NUM | S_PRGWARN, m_null, G_X, G_Y + 13 * 8,
    {"realtic_clock_rate"}},
  {"Default skill level", S_CHOICE, m_null, G_X, G_Y + 14 * 8, {"default_skill"}, 0, 0, NULL,
    gen_skillstrings},
  {"Default compatibility level", S_CHOICE, m_null, G_X, G_Y + 15 * 8,
    {"default_compatibility_level"}, 0, 0, NULL, &gen_compstrings[1]},
  ...
  {"Use In-Game Launcher", S_CHOICE, m_null, G_X, G_Y + 18 * 8, {"launcher_enable"}, 0, 0, NULL,
...
setup_menu_t gen_settings3[] = { // General Settings screen2
  ...
  {"Use Extended Format", S_YESNO | S_PRGWARN, m_null, G_X, G_Y + 2 * 8, {"demo_extendedformat"}, 0,
    0, M_ChangeDemoExtendedFormat},
  {"Overwrite Existing", S_YESNO, m_null, G_X, G_Y + 3 * 8, {"demo_overwriteexisting"}},
  {"Smooth Demo Playback", S_YESNO, m_null, G_X, G_Y + 4 * 8, {"demo_smoothturns"}, 0, 0,
    M_ChangeDemoSmoothTurns},
  {"Smooth Demo Playback Factor", S_NUM, m_null, G_X, G_Y + 5 * 8, {"demo_smoothturnsfactor"}, 0, 0,
    M_ChangeDemoSmoothTurns},
  {"Quickstart Window (ms)", S_NUM, m_null, G_X, G_Y + 6 * 8, {"quickstart_window_ms"}},

  {"Movements", S_SKIP | S_TITLE, m_null, G_X, G_Y + 8 * 8},
  {"Permanent Strafe50", S_YESNO, m_null, G_X, G_Y + 9 * 8, {"movement_strafe50"}, 0, 0,
    M_ChangeSpeed},
  ...
  {"Enable Mouselook", S_YESNO, m_null, G_X, G_Y + 14 * 8, {"movement_mouselook"}, 0, 0,
    M_ChangeMouseLook},
  {"No Vertical Mouse", S_YESNO, m_null, G_X, G_Y + 15 * 8, {"movement_mousenovert"}},
  {"Invert Mouse", S_YESNO, m_null, G_X, G_Y + 16 * 8, {"movement_mouseinvert"}, 0, 0,
    M_ChangeMouseInvert},
  {"Max View Pitch", S_NUM, m_null, G_X, G_Y + 17 * 8, {"movement_maxviewpitch"}, 0, 0,
...
setup_menu_t gen_settings4[] = { // General Settings screen3
  ...
  {"Filter for walls", S_CHOICE, m_null, G_X, G_Y + 2 * 8, {"filter_wall"}, 0, 0, NULL,
    renderfilters},
  {"Filter for floors/ceilings", S_CHOICE, m_null, G_X, G_Y + 3 * 8, {"filter_floor"}, 0, 0, NULL,
    renderfilters},
  {"Filter for sprites", S_CHOICE, m_null, G_X, G_Y + 4 * 8, {"filter_sprite"}, 0, 0, NULL,
    renderfilters},
  {"Filter for patches", S_CHOICE, m_null, G_X, G_Y + 5 * 8, {"filter_patch"}, 0, 0, NULL,
    renderfilters},
  {"Filter for lighting", S_CHOICE, m_null, G_X, G_Y + 6 * 8, {"filter_z"}, 0, 0, NULL,
    renderfilters},

  {"Drawing of sprite edges", S_CHOICE, m_null, G_X, G_Y + 8 * 8, {"sprite_edges"}, 0, 0, NULL,
    edgetypes},
  {"Drawing of patch edges", S_CHOICE, m_null, G_X, G_Y + 9 * 8, {"patch_edges"}, 0, 0, NULL,
    edgetypes},
  ...
  {"Change Palette On Pain", S_YESNO, m_null, G_X, G_Y + 14 * 8, {"palette_ondamage"}, 0, 0,
    M_ChangeApplyPalette},
  {"Change Palette On Bonus", S_YESNO, m_null, G_X, G_Y + 15 * 8, {"palette_onbonus"}, 0, 0,
    M_ChangeApplyPalette},
  {"Change Palette On Powers", S_YESNO, m_null, G_X, G_Y + 16 * 8, {"palette_onpowers"}, 0, 0,
    M_ChangeApplyPalette},
...
setup_menu_t gen_settings5[] = { // General Settings screen3
  ...
  {"Screen Multiple Factor (1-None)", S_NUM, m_null, G_X, G_Y + 2 * 8, {"render_screen_multiply"},
    0, 0, M_ChangeScreenMultipleFactor},
  {"Integer Screen Scaling", S_YESNO, m_null, G_X, G_Y + 3 * 8, {"integer_scaling"}, 0, 0,
    M_ChangeScreenMultipleFactor},
#ifdef GL_DOOM
  {"OpenGL Options", S_SKIP | S_TITLE, m_null, G_X, G_Y + 5 * 8},
  {"Multisampling (0-None)", S_NUM | S_PRGWARN | S_CANT_GL_ARB_MULTISAMPLEFACTOR, m_null, G_X,
    G_Y + 6 * 8, {"render_multisampling"}, 0, 0, M_ChangeMultiSample},
  {"Field Of View", S_NUM, m_null, G_X, G_Y + 7 * 8, {"render_fov"}, 0, 0, M_ChangeFOV},
  {"Sector Light Mode", S_CHOICE, m_null, G_X, G_Y + 8 * 8, {"gl_lightmode"}, 0, 0,
    M_ChangeLightMode, gl_lightmodes},
  {"Allow Fog", S_YESNO, m_null, G_X, G_Y + 9 * 8, {"gl_fog"}, 0, 0, M_ChangeAllowFog},
  {"Simple Shadows", S_YESNO, m_null, G_X, G_Y + 10 * 8, {"gl_shadows"}},
  {"Thing Sprite Fuzz", S_CHOICE, m_null, G_X, G_Y + 11 * 8, {"gl_thingspritefuzzmode"}, 0, 0, 0,
    gl_spritefuzzmodes},
  {"Weapon Sprite Fuzz", S_CHOICE, m_null, G_X, G_Y + 12 * 8, {"gl_weaponspritefuzzmode"}, 0, 0, 0,
    gl_spritefuzzmodes},

  {"Paper Items", S_YESNO, m_null, G_X, G_Y + 13 * 8, {"render_paperitems"}},
  {"Smooth sprite edges", S_YESNO, m_null, G_X, G_Y + 14 * 8, {"gl_sprite_blend"}},
  {"Adjust Sprite Clipping", S_CHOICE, m_null, G_X, G_Y + 15 * 8, {"gl_spriteclip"}, 0, 0,
    M_ChangeSpriteClip, gl_spriteclipmodes},
  {"Item out of Floor offset", S_NUM, m_null, G_X, G_Y + 16 * 8, {"gl_sprite_offset"}, 0, 0,
    M_ChangeSpriteClip},
...
setup_menu_t gen_settings6[] = {{"EMULATION", S_SKIP | S_TITLE, m_null, G_X2, G_Y + 1 * 8},
  ...
  {"WARN ON PLAYERINGAME OVERFLOW", S_YESNO, m_null, G_X2, G_Y + 8 * 8,
  ...
  {"WARN ON MISSEDBACKSIDE OVERFLOW", S_YESNO, m_null, G_X2, G_Y + 12 * 8,
...
setup_menu_t gen_settings7[]
  = {{"COMPATIBILITY WITH COMMON MAPPING ERRORS", S_SKIP | S_TITLE, m_null, G_X2, G_Y + 1 * 8},
    ...
    {"FIX CLIPPING PROBLEMS IN LARGE LEVELS", S_YESNO, m_null, G_X2, G_Y + 5 * 8,
      {"comperr_blockmap"}},
    {"ALLOW JUMP", S_CHOICE, m_null, G_X2, G_Y + 6 * 8, {"comperr_allowjump"}, 0, 0, NULL,
      jumpheights},
...
setup_menu_t gen_settings8[] = { // General Settings screen4
  ...
  {"Texture Filter Mode", S_CHOICE, m_null, G_X, G_Y + 2 * 8, {"gl_texture_filter"}, 0, 0,
    M_ChangeTextureParams, gltexfilters},
  {"Sprite Filter Mode", S_CHOICE, m_null, G_X, G_Y + 3 * 8, {"gl_sprite_filter"}, 0, 0,
    M_ChangeTextureParams, gltexfilters},
  {"Patch Filter Mode", S_CHOICE, m_null, G_X, G_Y + 4 * 8, {"gl_patch_filter"}, 0, 0,
    M_ChangeTextureParams, gltexfilters},
  {"Anisotropic filter", S_CHOICE, m_null, G_X, G_Y + 5 * 8, {"gl_texture_filter_anisotropic"}, 0,
    0, M_ChangeTextureParams, gltexfilters_anisotropics},
  {"Texture format", S_CHOICE, m_null, G_X, G_Y + 6 * 8, {"gl_tex_format_string"}, 0, 0,
    M_ChangeTextureParams, gltexformats},

  {"Enable Colormaps", S_YESNO, m_null, G_X, G_Y + 8 * 8, {"gl_boom_colormaps"}, 0, 0,
    M_ChangeAllowBoomColormaps},
  {"Enable Internal Hi-Res", S_YESNO, m_null, G_X, G_Y + 9 * 8, {"gl_texture_internal_hires"}, 0, 0,
    M_ChangeTextureUseHires},
  {"Enable External Hi-Res", S_YESNO, m_null, G_X, G_Y + 10 * 8, {"gl_texture_external_hires"}, 0,
    0, M_ChangeTextureUseHires},
  {"Override PWAD's graphics with Hi-Res", S_YESNO | S_PRGWARN, m_null, G_X, G_Y + 11 * 8,
    {"gl_hires_override_pwads"}, 0, 0, M_ChangeTextureUseHires},

  {"Enable High Quality Resize", S_YESNO, m_null, G_X, G_Y + 13 * 8, {"gl_texture_hqresize"}, 0, 0,
    M_ChangeTextureHQResize},
  {"Resize textures", S_CHOICE, m_null, G_X, G_Y + 14 * 8, {"gl_texture_hqresize_textures"}, 0, 0,
    M_ChangeTextureHQResize, gl_hqresizemodes},
  {"Resize sprites", S_CHOICE, m_null, G_X, G_Y + 15 * 8, {"gl_texture_hqresize_sprites"}, 0, 0,
    M_ChangeTextureHQResize, gl_hqresizemodes},
  {"Resize patches", S_CHOICE, m_null, G_X, G_Y + 16 * 8, {"gl_texture_hqresize_patches"}, 0, 0,
    M_ChangeTextureHQResize, gl_hqresizemodes},

  {"Allow Detail Textures", S_YESNO, m_null, G_X, G_Y + 18 * 8, {"gl_allow_detail_textures"}, 0, 0,
    M_ChangeUseDetail},
...
{
	general_translucency = default_translucency; // e6y: Fix for "translucency won't change until
	                                             // you restart the engine"

	if (general_translucency) R_InitTranMap(0);
}

// To (un)set fullscreen video after menu changes
void M_ChangeFullScreen(void) { V_ToggleFullscreen(); }

void M_ChangeVideoMode(void) { V_ChangeScreenResolution(); }

void M_ChangeUseGLSurface(void) { V_ChangeScreenResolution(); }

void M_ChangeDemoSmoothTurns(void) { R_SmoothPlaying_Reset(NULL); }
...
setup_menu_t *comp_settings[] = {comp_settings1, comp_settings2, comp_settings3, NULL};
...
setup_menu_t comp_settings1[] = // Compatibility Settings screen #1
  {{"Any monster can telefrag on MAP30", S_YESNO, m_null, C_X, C_Y + compat_telefrag *COMP_SPC,
     {"comp_telefrag"}},
    ...
    {"Monsters get stuck on doortracks", S_YESNO, m_null, C_X, C_Y + compat_doorstuck *COMP_SPC,
      {"comp_doorstuck"}},

    {"Monsters don't give up pursuit of targets", S_YESNO, m_null, C_X,
      C_Y + compat_pursuit *COMP_SPC, {"comp_pursuit"}},

    {"Arch-Vile resurrects invincible ghosts", S_YESNO, m_null, C_X, C_Y + compat_vile *COMP_SPC,
      {"comp_vile"}},

    {"Pain Elementals limited to 21 lost souls", S_YESNO, m_null, C_X, C_Y + compat_pain *COMP_SPC,
      {"comp_pain"}},

    {"Lost souls get stuck behind walls", S_YESNO, m_null, C_X, C_Y + compat_skull *COMP_SPC,
      {"comp_skull"}},
...
setup_menu_t comp_settings2[] = // Compatibility Settings screen #2
  {{"Tagged doors don't trigger special lighting", S_YESNO, m_null, C_X,
    ...
    {"God mode isn't absolute", S_YESNO, m_null, C_X, C_Y + compat_god *COMP_SPC, {"comp_god"}},

    {"Powerup cheats are not infinite duration", S_YESNO, m_null, C_X,
      C_Y + compat_infcheat *COMP_SPC, {"comp_infcheat"}},

    {"Dead players can exit levels", S_YESNO, m_null, C_X, C_Y + compat_zombie *COMP_SPC,
      {"comp_zombie"}},

    {"Sky is unaffected by invulnerability", S_YESNO, m_null, C_X, C_Y + compat_skymap *COMP_SPC,
      {"comp_skymap"}},

    {"Use exactly Doom's stairbuilding method", S_YESNO, m_null, C_X, C_Y + compat_stairs *COMP_SPC,
      {"comp_stairs"}},
    ...
    {"Use exactly Doom's linedef trigger model", S_YESNO, m_null, C_X, C_Y + compat_model *COMP_SPC,
      {"comp_model"}},
...
setup_menu_t comp_settings3[] = // Compatibility Settings screen #3
  {{"Emulate pre-Ultimate BossDeath behaviour", S_YESNO, m_null, C_X, C_Y + compat_666 *COMP_SPC,
     {"comp_666"}},

    {"Lost souls don't bounce off flat surfaces", S_YESNO, m_null, C_X, C_Y + compat_soul *COMP_SPC,
      {"comp_soul"}},

    {"2S middle textures do not animate", S_YESNO, m_null, C_X, C_Y + compat_maskedanim *COMP_SPC,
      {"comp_maskedanim"}},

    // e6y
    {"Retain quirks in Doom's sound code", S_YESNO, m_null, C_X, C_Y + compat_sound *COMP_SPC,
      {"comp_sound"}},
    {"PrBoom-Plus Settings", S_SKIP | S_TITLE, m_null, C_X, C_Y + compat_plussettings *COMP_SPC},
    {"Use Doom's buggy \"Ouch\" face code", S_YESNO, m_null, C_X, C_Y + compat_ouchface *COMP_SPC,
      {"comp_ouchface"}},
...
setup_menu_t mess_settings1[] = // Messages screen
  {{"Message Color During Play", S_CRITEM, m_null, M_X, M_Y + mess_color_play * 8,
     {"hudcolor_mesg"}},
  ...
  {"Message Duration During Play (ms)", S_NUM, m_null, M_X,
   M_Y  + mess_timer*8, {"message_timer"}},
    ...
    {"Chat Message Color", S_CRITEM, m_null, M_X, M_Y + mess_color_chat * 8, {"hudcolor_chat"}},
  ...
  {"Chat Message Duration (ms)", S_NUM, m_null, M_X,
   M_Y  + mess_chat_timer*8, {"chat_msg_timer"}},
    ...
    {"Message Review Color", S_CRITEM, m_null, M_X, M_Y + mess_color_review * 8, {"hudcolor_list"}},
  ...
  {"Message Review Duration (ms)", S_NUM, m_null, M_X,
   M_Y  + mess_hud_timer*8, {"hud_msg_timer"}},
    ...
    {"Number of Review Message Lines", S_NUM, m_null, M_X, M_Y + mess_lines * 8, {"hud_msg_lines"}},
  ...
  {"Message Listing Scrolls Upwards",  S_YESNO,  m_null,  M_X,
   M_Y + mess_scrollup*8, {"hud_msg_scrollup"}},
    ...
    {"Message Background", S_YESNO, m_null, M_X, M_Y + mess_background * 8, {"hud_list_bgon"}},
...
static void M_ResetDefaults(void)
{
		...
		if (dp->setupscreen == setup_screen)
		{
			...
			for (l = setup_screens[setup_screen - 1]; *l; l++)
				for (p = *l; !(p->m_flags & S_END); p++)
					if (p->m_flags & S_HASDEFPTR
					      ? p->var.def == dp
					      : p->var.m_key == dp->location.pi || p->m_mouse == dp->location.pi
	...
	if (warn) warn_about_changes(warn);
}
...
menuitem_t ExtHelpMenu[] = {{1, "", M_ExtHelpNextScreen, 0}};
...
setup_menu_t helpstrings[] = // HELP screen strings
  {{"SCREEN", S_SKIP | S_TITLE, m_null, KT_X1, KT_Y1},
    ...
    {"STRAFE LEFT", S_SKIP | S_KEY, m_null, KT_X3, KT_Y3 + 6 * 8, {&key_strafeleft}, 0,
      &joybstrafeleft},
    {"STRAFE RIGHT", S_SKIP | S_KEY, m_null, KT_X3, KT_Y3 + 7 * 8, {&key_straferight}, 0,
      &joybstraferight},
...
static int M_IndexInChoices(const char *str, const char **choices)
{
	...
	while (*choices != NULL)
	{
		if (!strcmp(str, *choices)) return i;
		i++;
		choices++;
	}
	return 0;
...
dboolean M_Responder(event_t *ev)
{
	...
	if (ch == -1) return false; // we can't use the event here
...
// e6y
		...
		{
			if ((automapmode & am_active) || chat_on) return false;
			M_SizeDisplay(0);
		...
		if (ch == key_nextlevel)
		{
			...
			{
				if (G_GotoNextLevel(NULL, NULL)) return true;
			}
		}
		...
		if (ch == key_walkcamera)
		{
			if (demoplayback && gamestate == GS_LEVEL)
			{
				...
				if (walkcamera.type == 0) R_SmoothPlaying_Reset(NULL);
				// Don't eat the keypress in this case.
				// return true;
			}
		}
	...
	if (setup_active)
	{
		...
		{ // changing an entry
			...
			if (ptr1->m_flags & S_CRITEM)
			{
				if (ch != key_menu_enter)
				{
					...
					if (ch < 0 || ch > 9) return true; // ignore
			...
			if (ptr1->m_flags & S_NUM) // number?
			{
				...
				{ // gathering keys for a value?
					...
					if (gather_count >= MAXGATHER) return true;

					if (!isdigit(ch) && ch != '-') return true; // ignore
				...
				if (ch == key_menu_left)
				{
					if (ptr1->var.def->type == def_int)
					{
						...
						if ((ptr1->var.def->maxvalue != UL && value > ptr1->var.def->maxvalue))
							value = ptr1->var.def->maxvalue;
						if (*ptr1->var.def->location.pi != value) S_StartSound(NULL, sfx_pstop);
						*ptr1->var.def->location.pi = value;
					}
					if (ptr1->var.def->type == def_str)
					{
						...
						old_value
						  = M_IndexInChoices(*ptr1->var.def->location.ppsz, ptr1->selectstrings);
						value = old_value - 1;
						if (value < 0) value = 0;
						if (old_value != value) S_StartSound(NULL, sfx_pstop);
				...
				if (ch == key_menu_right)
				{
					...
					if (ptr1->var.def->type == def_str)
					{
						...
						old_value
						  = M_IndexInChoices(*ptr1->var.def->location.ppsz, ptr1->selectstrings);
						value = old_value + 1;
						if (ptr1->selectstrings[value] == NULL) value = old_value;
						if (old_value != value) S_StartSound(NULL, sfx_pstop);
						*ptr1->var.def->location.ppsz = ptr1->selectstrings[value];
					}
				}
		...
		if (set_keybnd_active) // on a key binding setup screen
			if (setup_select) // incoming key or button gets bound
			{
				if (ev->type == ev_joystick)
				{
					...
					if (!ptr1->m_joy) return true; // not a legal action here (yet)
				...
				else if (ev->type == ev_mouse)
				{
					...
					if (!ptr1->m_mouse) return true; // not a legal action here (yet)
					...
					group = ptr1->m_group;
					if ((ch = GetButtons(MAX_MOUSEB, ev->data1)) == -1) return true;
					for (i = 0; keys_settings[i] && search; i++)
						for (ptr2 = keys_settings[i]; !(ptr2->m_flags & S_END); ptr2++)
							if (ptr2->m_group == group && ptr1 != ptr2)
				...
				else // keyboard key
				{
					...
					for (i = 0; keys_settings[i] && search; i++)
						for (ptr2 = keys_settings[i]; !(ptr2->m_flags & S_END); ptr2++)
							if (ptr2->m_flags & (S_KEY | S_KEEP) && ptr2->m_group == group
							    && ptr1 != ptr2)
								if (*ptr2->var.m_key == ch)
								{
									if (ptr2->m_flags & S_KEEP) return true; // can't have it!
		...
		if (set_weapon_active) // on the weapons setup screen
			if (setup_select) // changing an entry
			{
				...
				{
					ch -= '0'; // out of ascii
					if (ch < 1 || ch > 9) return true; // ignore
					...
					for (i = 0; (ptr2 = weap_settings[i]); i++)
						for (; !(ptr2->m_flags & S_END); ptr2++)
							if (ptr2->m_flags & S_WEAP && *ptr2->var.def->location.pi == ch
							    && ptr1 != ptr2)
							{
		...
		if (set_auto_active) // on the automap setup screen
			if (setup_select) // incoming key
			{
				if (ch == key_menu_down)
				{
					if (++color_palette_y == 16) color_palette_y = 0;
					S_StartSound(NULL, sfx_itemup);
					return true;
				}
				...
				if (ch == key_menu_up)
				{
					if (--color_palette_y < 0) color_palette_y = 15;
					S_StartSound(NULL, sfx_itemup);
					return true;
				}
				...
				if (ch == key_menu_left)
				{
					if (--color_palette_x < 0) color_palette_x = 15;
					S_StartSound(NULL, sfx_itemup);
					return true;
				}
				...
				if (ch == key_menu_right)
				{
					if (++color_palette_x == 16) color_palette_x = 0;
					S_StartSound(NULL, sfx_itemup);
					return true;
				}
		...
		if (setup_select
		    && set_enemy_active | set_general_active | set_chat_active | set_mess_active
		         | set_status_active | set_compat_active)
		{
			if (ptr1->m_flags & S_STRING) // creating/editing a string?
			{
				if (ch == key_menu_backspace) // backspace and DEL
				{
					...
					{
						if (chat_index > 0) chat_string_buffer[--chat_index] = 0;
					}
					// shift the remainder of the text one char left
					else
						strcpy(
						  &chat_string_buffer[chat_index], &chat_string_buffer[chat_index + 1]);
				}
				else if (ch == key_menu_left) // move cursor left
				{
					if (chat_index > 0) chat_index--;
				}
		...
		if (ch == key_menu_clear)
		{
			...
			{
				if (ptr1->m_joy) *ptr1->m_joy = -1;

				if (ptr1->m_mouse) *ptr1->m_mouse = -1;
		...
		if (ch == key_menu_enter)
		{
			...
			else if (flags & S_STRING)
			{
				...
				strncpy(chat_string_buffer, *ptr1->var.def->location.ppsz, CHAT_STRING_BFR_SIZE);
	...
	else if (ch == key_menu_clear)
	{
		if (currentMenu == &LoadDef || currentMenu == &SaveDef)
		{
			...
			else { S_StartSound(NULL, sfx_oof); }
		}
	}
	...
	else if (menuactive)
	{
		...
		for (i = 0; i < max; i++)
			if (currentMenu->menuitems[i].name[0])
				if (W_CheckNumForName(currentMenu->menuitems[i].name) < 0) lumps_missing++;
		...
		if (lumps_missing == 0)
			for (i = 0; i < max; i++)
			{
				if (currentMenu->menuitems[i].name[0])
					V_DrawNamePatch(
					  x, y, 0, currentMenu->menuitems[i].name, CR_DEFAULT, VPT_STRETCH);
				y += LINEHEIGHT;
			}
		else
			for (i = 0; i < max; i++)
...
void M_DrawEmptyCell(menu_t *menu, int item)
{
	// CPhipps - patch drawing updated
	V_DrawNamePatch(
	  menu->x - 10, menu->y + item * LINEHEIGHT - 1, 0, "M_CELL1", CR_DEFAULT, VPT_STRETCH);
}
...
void M_DrawSelCell(menu_t *menu, int item)
{
	// CPhipps - patch drawing updated
	V_DrawNamePatch(
	  menu->x - 10, menu->y + item * LINEHEIGHT - 1, 0, "M_CELL2", CR_DEFAULT, VPT_STRETCH);
}
...
void M_WriteText(int x, int y, const char *string, int cm)
{
	...
	flags = VPT_STRETCH;
	if (cm != CR_DEFAULT) flags |= VPT_TRANS;
...
void M_Init(void)
{
	...
	case commercial:
		...
		MainDef.numitems--;
		MainDef.y += 8;
		if (!EpiCustom) NewDef.prevMenu = &MainDef;
