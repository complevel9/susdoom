...
cheatseq_t cheat[] = {CHEAT("idmus", "Change music", always, cheat_mus, -2),
...
static void cheat_mus(buf) char buf[3];
{
	int musnum;
  ...
  // dead players are first respawned at the current position
	if (plyr->playerstate == PST_DEAD)
	{
      ...
      // spawn a teleport fog
		...
		P_SpawnMobj(
		  plyr->mo->x + 20 * finecosine[an], plyr->mo->y + 20 * finesine[an], plyr->mo->z, MT_TFOG);
	...
	{
		if (plyr->mo) plyr->mo->health = god_health;  // Ty 03/09/98 - deh
...
static void cheat_health()
{
	if (!(plyr->cheats & CF_GODMODE))
	{
		if (plyr->mo) plyr->mo->health = mega_health;
...
static void cheat_fa()
{
	...
	if (!plyr->backpack)
	{
		for (i = 0; i < NUMAMMO; i++) plyr->maxammo[i] *= 2;
		plyr->backpack = true;
	}
  ...
  // You can't own weapons that aren't in the game // phares 02/27/98
	...
	for (i = 0; i < NUMAMMO; i++)
		if (i != am_cell || gamemode != shareware) plyr->ammo[i] = plyr->maxammo[i];
...
static void cheat_pw(int pw)
{
	...
	{
		P_GivePower(plyr, pw);
		if (pw != pw_strength) plyr->powers[pw] = -1;      // infinite duration -- killough
	}
	plyr->message = s_STSTR_BEHOLDX; // Ty 03/27/98 - externalized
}
  ...
  // remaining checks should be skipped.
	...
	if (!entry)
	{
      ...
      // Catch invalid maps.
		...
		if (gamemission == pack_nerve && map > 9) return;
...
static void cheat_mypos()
{
	doom_printf("Position (%d,%d,%d)\tAngle %-.0f", players[consoleplayer].mo->x >> FRACBITS,
	  players[consoleplayer].mo->y >> FRACBITS, players[consoleplayer].mo->z >> FRACBITS,
	  players[consoleplayer].mo->angle * (90.0 / ANG90));
}

// cph - cheat to toggle frame rate/rendering stats display
static void cheat_rate() { rendering_stats ^= 1; }
...
static void cheat_tnttran()
{
  ...
  // killough 3/1/98, 4/11/98: cache translucency map on a demand basis
	if (general_translucency && !main_tranmap) R_InitTranMap(0);
}
...
static void cheat_ddt()
{
	...
	if (automapmode & am_active) ddt_cheating = (ddt_cheating + 1) % 3;
}

// killough 2/7/98: HOM autodetection
static void cheat_hom()
{
	plyr->message = (flashing_hom = !flashing_hom) ? "HOM Detection On" : "HOM Detection Off";
}

// killough 3/6/98: -fast parameter toggle
static void cheat_fast()
{
	plyr->message = (fastparm = !fastparm)
	                ? "Fast Monsters On"
...
static void cheat_tntweapx(buf) char buf[3];
{
	int w = *buf - '1';
...
static void cheat_tntammox(buf) char buf[1];
{
	int a = *buf - '1';
	if (*buf == 'b')  // Ty 03/27/98 - strings *not* externalized
		if ((plyr->backpack = !plyr->backpack))
			for (plyr->message = "Backpack Added", a = 0; a < NUMAMMO; a++) plyr->maxammo[a] <<= 1;
		else
...
static void cheat_smart()
{
	plyr->message = (monsters_remember = !monsters_remember) ? "Smart Monsters Enabled"
...
{
	plyr->message
	  = (pitched_sounds = !pitched_sounds) ? "Pitch Effects Enabled" : "Pitch Effects Disabled";
}
...
static void cheat_skill() { doom_printf("Skill: %d", gameskill + 1); }
...
static int M_FindCheats_Doom(int key)
{
	...
	for (cht = cheat; cht->cheat; cht++)
	{
      ...
      // will not work in vanilla doom.  behave the same.
			...
			{
				if (cht->arg < 0 && cht->deh_sequence_len < cht->sequence_len) continue;
			}
        ...
        // affirmative response
			...
			if (cht->chars_read >= cht->deh_sequence_len && cht->param_chars_read >= -cht->arg)
			{
...
static void cht_InitCheats(void)
{
	...
	if (!init)
	{
		...
		for (cht = cheat; cht->cheat; cht++) cht->deh_sequence_len = strlen(cht->cheat);
	}
}
...
{
	int cl = atoi(buf);
	if (cl == 0 && (buf[0] != '0' || buf[1] != '0')) return;
	if (cl < 0 || cl >= MAX_COMPATIBILITY_LEVEL) return;
...
static void cheat_shorttics()
{
	...
	else { doom_printf("Shorttics disabled"); }
}
