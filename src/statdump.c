...
static void DiscoverGamemode(const wbstartstruct_t *stats, int num_stats)
{
	...
	if (discovered_gamemission != none) return;
...
static int GetNumPlayers(const wbstartstruct_t *stats)
{
	...
	for (i = 0; i < MAXPLAYERS; ++i)
		if (stats->plyr[i].in) ++num_players;
...
static void PrintStats(FILE *stream, const wbstartstruct_t *stats)
{
	...
	for (i = 0; i < MAXPLAYERS; ++i)
		if (stats->plyr[i].in) PrintPlayerStats(stream, stats, i);

	if (GetNumPlayers(stats) >= 2) PrintFragsTable(stream, stats);
...
void StatDump(void)
{
	...
	if (i > 0 && i < myargc - 1)
	{
		...
		for (i = 0; i < num_captured_stats; ++i) PrintStats(dumpfile, &captured_stats[i]);
