	...
	for (i = -1; (i = P_FindSectorFromLineTag(line, i)) >= 0;)
	{
		...
		while ((th = P_NextThinker(th, th_misc)) != NULL)
			if (th->function == P_MobjThinker)
			{
				...
				if (m->type == MT_TELEPORTMAN && m->subsector->sector->iSectorID == i) return m;
			}
	}
	return NULL;
}
...
int EV_Teleport(line_t *line, int side, mobj_t *thing)
{
  ...
  //  so you can get out of teleporter.
	if (side || thing->flags & MF_MISSILE) return 0;
  ...
  // P_FindSectorFromLineTag instead of simple linear search.
	...
	if ((m = P_TeleportDestination(line)) != NULL)
	{
		...
		if (player && player->mo != thing) player = NULL;

		if (!P_TeleportMove(thing, m->x, m->y, false)) /* killough 8/9/98 */
			return 0;

		if (compatibility_level != finaldoom_compatibility) thing->z = thing->floorz;
		thing->PrevZ = thing->z;

		if (player) player->viewz = thing->z + player->viewheight;
		...
		S_StartSound(P_SpawnMobj(m->x + 20 * finecosine[m->angle >> ANGLETOFINESHIFT],
		               m->y + 20 * finesine[m->angle >> ANGLETOFINESHIFT], thing->z, MT_TFOG),
		  sfx_telept);
    ...
    /* killough 10/98: kill all bobbing momentum too */
		if (player) player->momx = player->momy = 0;

     // e6y
		if (player && player->mo == thing) R_ResetAfterTeleport(player);
...
int EV_SilentLineTeleport(line_t *line, int side, mobj_t *thing, dboolean reverse)
{
	int i;
	line_t *l;
