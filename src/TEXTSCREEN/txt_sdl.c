...
static void ChooseFont(void)
{
	...
	if (desktop_info.w < 640 || desktop_info.h < 480) font = &small_font;
#ifdef _WIN32
...
int TXT_Init(void)
{
	...
	TXT_SDLWindow = SDL_CreateWindow("", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
	...
	screenbuffer
	  = SDL_CreateRGBSurface(0, TXT_SCREEN_W * font->w, TXT_SCREEN_H * font->h, 8, 0, 0, 0, 0);
...
static inline void UpdateCharacter(int x, int y)
{
	...
	if (bg & 0x8)
	{
		...
		if (((SDL_GetTicks() / BLINK_PERIOD) % 2) == 0) fg = bg;
	}
	...
	s = ((unsigned char *)screenbuffer->pixels) + (y * font->h * screenbuffer->pitch)
	  + (x * font->w);
...
void TXT_UpdateScreenArea(int x, int y, int w, int h)
{
	...
	for (y1 = y; y1 < y_end; ++y1)
		for (x1 = x; x1 < x_end; ++x1) UpdateCharacter(x1, y1);
	...
	SDL_BlitSurface(screenbuffer, &rect, SDL_GetWindowSurface(TXT_SDLWindow), &rect);
	SDL_UpdateWindowSurfaceRects(TXT_SDLWindow, &rect, 1);
}

void TXT_UpdateScreen(void) { TXT_UpdateScreenArea(0, 0, TXT_SCREEN_W, TXT_SCREEN_H); }
...
signed int TXT_GetChar(void)
{
	...
	while (SDL_PollEvent(&ev))
	{
		...
		{
			if (event_callback(&ev, event_callback_data)) continue;
		}
...
{
	if (mod < TXT_NUM_MODIFIERS) return modifier_state[mod] > 0;
...
void TXT_EnableKeyMapping(int enable) { key_mapping = enable; }

void TXT_SetWindowTitle(char *title) { SDL_SetWindowTitle(TXT_SDLWindow, title); }
...
void TXT_StringConcat(char *dest, const char *src, size_t dest_len)
{
	...
	offset = strlen(dest);
	if (offset > dest_len) offset = dest_len;
